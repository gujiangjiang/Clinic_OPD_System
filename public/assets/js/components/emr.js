...
Clinic.emr = (function () {
    ...
    var _ctx = {
        ...
        requireSaved: function (scene) { return requireSaved(scene); },
        isRecordComplete: function () { return isRecordComplete(); },
        hasEditableRecord: function () { return hasEditableRecord(); },
        currentRecordEditable: function () { return currentRecordEditable(); },
        loadOrders: function (visitId) { return loadOrders(visitId); },
        flowColumnHtml: function (steps, idx, title) { return flowColumnHtml(steps, idx, title); },
        renderEmrCard: function (d) { return renderEmrCard(d); },
        addProgressEditor: function () { return addProgressEditor(); },
        createProgressEditor: function () { return createProgressEditor(); },
    };
...
// 续写 = 全新独立病历：只保留既往史，过敏史默认否认（不自动引用，手动确认后才引入）
    ...
    function addProgressEditor() {
        ...
        // 4.5 刷新只读区（首诊/其他文书锚点），避免新建编辑时其他文书消失
    ...
    function setReadonlyUI() {
        ...
        if (status) {
            // 区分会诊完毕只读与诊毕只读
            if (viewOnlyNow) {
                status.textContent = '会诊完毕 · 只读查看模式（不可编辑 / 不可开具诊断证明）';
            } else if (DATA && DATA.record && DATA.record.consultation_id > 0) {
    ...
    /**
     * 开单是否可删除（删除按钮显示与后端删除的共同规则）：
     * 必须同时满足【病历ID强关联 + 开单人本人 + 当前编辑状态】：
...
/* ==================== 病历正文开单展示——已拆至 emr_orders.js ==================== */
    ...
    /* ==================== 初步诊断（已拆至 emr_diag.js） ==================== */

    function clampPop(pop) { return Clinic.emr.diag.clampPop(pop); }
    function closeDiagPop() { return Clinic.emr.diag.closeDiagPop(); }
    function placeDiagPop(pop, ev) { return Clinic.emr.diag.placeDiagPop(pop, ev); }
    function diagEditable() { return Clinic.emr.diag.diagEditable(); }
    function myDiags() { return Clinic.emr.diag.myDiags(); }
    function saveDiags(newDiags, okMsg) { return Clinic.emr.diag.saveDiags(newDiags, okMsg); }
    function openDiagEditPop(ev, idx) { return Clinic.emr.diag.openDiagEditPop(ev, idx); }
    function openDiagPop(ev) { return Clinic.emr.diag.openDiagPop(ev); }
    function openDiagOpsPop(ev, idx) { return Clinic.emr.diag.openDiagOpsPop(ev, idx); }
    function saveDiagOrder(newKeys) { return Clinic.emr.diag.saveDiagOrder(newKeys); }
    function delDiag(ev, idx) { return Clinic.emr.diag.delDiag(ev, idx); }
    ...
    /* ==================== 科室间会诊（已拆至 emr_consult.js） ==================== */

    function openConsultCreate(ev) { return Clinic.emr.consult.openConsultCreate(ev); }
    function renderConsultList() { return Clinic.emr.consult.renderConsultList(); }
    function delConsult(code) { return Clinic.emr.consult.delConsult(code); }
    function openConsultDetail(id, withAccept) { return Clinic.emr.consult.openConsultDetail(id, withAccept); }
    function startConsult(consultId) { return Clinic.emr.consult.startConsult(consultId); }
    ...
    function viewCertificate() {
        return Clinic.emr.cert.viewCertificate();
    }

    function certificateModal(visitId, title, onIssued, warnOnIssued) {
        return Clinic.emr.cert.certificateModal(visitId, title, onIssued, warnOnIssued);
    }

    function openCertificate() {
        return Clinic.emr.cert.openCertificate();
    }
     ...
     *  当前函数也判定新建中（record_id=0）是否真的处于编辑中，
     *  即医生是否已点击「病历节点 ＋」开始建编（__pending_* 标志）。
...
function delOrderFlow(orderId, label) {
    ...
    Clinic.modal.confirm(isRx
        ? '确定毁方该处方？'
        : '确定删除该开单？', function () {
        // 携带当前编辑病历 id：后端校验开单 record_id + 开单人 双重匹配
        var curRec = (Clinic.emr && Clinic.emr._ctx && Clinic.emr._ctx.DATA && Clinic.emr._ctx.DATA.record) ? (Clinic.emr._ctx.DATA.record.record_id || 0) : 0;
        Clinic.ajax('/api/order', { action: 'delete', order_id: orderId, record_id: curRec }, {
            onSuccess: function (j) {

...
> 安全与并发缺陷修复（打印 XSS / 就诊序号 / 业务单号 / 双驱动兼容 / 退费竞态）+ 前端大型文件拆分。

### 新增
- 病历输入框右键菜单的「撤销」现可恢复误删内容（每字段独立历史栈，上限 50 步）——
  见 [7.25.0] 已实现功能在此版本经拆分重构保持行为不变

### 修复
- **打印病历存储型 XSS**：病历正文（主诉/现病史/既往史/过敏史/查体/诊断/意识状态/生命体征/
...
### 变更
- **前端大型文件拆分（低风险部分，行为不变）**：`emr.js` 由 3472 行降至 2603 行：

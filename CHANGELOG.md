...
> 三套布局微调：竖屏姓名下移且性别/年龄列对齐、过号徽标不挤压；方屏分隔线完整、年龄单行
> 省略；宽屏医生照片方形、左中右列宽优化。

### 修复

- **竖屏姓名视觉偏上**：等待条目顶部内边距略增，姓名整体下移数个像素。
- **竖屏性别/年龄未纵列对齐**：根因是各行独立 grid，`fit-content` 使姓名列各行宽度不同。
  修复：JS 计算已显示条目的最大姓名自然宽度并设为共享列宽（`--wait-name-w`），各行
  序号/姓名/性别/年龄全部纵向对齐，且姓名后无大片空白。
- **过号「过」字徽标位置**：清除残留的旧徽标样式（clamp 20px 覆盖了新样式导致徽标 22px
  挤压序号）。修复后徽标按 2.6cqmin（含边框 box-sizing）正好占满预留列宽。
- **方屏双排分隔线只在后几行显示**：原 `:nth-child(n+9)` 假定每列 8 行，动态行数下仅尾部
  显示。修复：JS 给第二列条目加 `wait-col2` 类，分隔线在第二列完整显示。
- **年龄「x岁x月」换行溢出**：年龄/性别列加 `white-space:nowrap` + 省略号，超长（如
  婴幼儿年龄）单行省略，不再撑高行导致列表溢出。

### 变更

- **宽屏（16:9 等）医生照片竖条**：宽屏医生照片改为正方形（aspect-ratio:1），不再拉成
  竖条；左中右列宽改为 `1.3fr / 1fr / 0.7fr`（医生加宽、中间保持、等待压缩），中间就诊/
  下一位随之右移。

...
![版本](https://img.shields.io/badge/版本-v8.34.0-blue) ![PHP](https://img.shields.io/badge/PHP-7.x-777BB4) ![数据库](https://img.shields.io/badge/数据库-SQLite%2FMySQL双驱动-003B57) ![部署](https://img.shields.io/badge/部署-Nginx-009639) ![代码](https://img.shields.io/badge/代码-全中文注释-orange)
...
### 🩺 医生工作站
- **所见即所得电子病历**：纸质病历版式；全部文本录入区为流式行内编辑器（长文按字符级自然折行，Tab 连续跳格，单行字段回车自动跳下一字段、叙述字段回车换行）；输入框自定义右键菜单（撤销/复制/剪切/粘贴/清空）；嘱托可调用模板（覆盖/续写）

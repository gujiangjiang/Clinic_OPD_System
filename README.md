...
![版本](https://img.shields.io/badge/版本-v6.4.0-blue) ![PHP](https://img.shields.io/badge/PHP-7.x-777BB4) ![数据库](https://img.shields.io/badge/数据库-SQLite%2FMySQL双驱动-003B57) ![部署](https://img.shields.io/badge/部署-Nginx-009639) ![代码](https://img.shields.io/badge/代码-全中文注释-orange)
...
| 🩺 医生工作站 | 多科室切换（标题「医生工作站-科室」可点击切换，仅多科室权限）、所见即所得电子病历（纸质病历版式，所见即所得）、顶栏工具箱（加号 / 切换科室 / 患者查询 / 模板管理）与叫号大屏绑定（绑定信息随登录会话持久 + 全局心跳保活，离开页面/刷新不自动解绑，仅退出登录或异常断开才解绑）、生命体征编辑（与护士站同步）、ICD‑10 诊断联动（搜索→选诊→部位/备注/疑似）、病历模板管理（个人/科室/全院，审核流）、无限续写病历节点（首诊+多段续写，链式拼接、只读/编辑自动切换）、续写必填校验、开检验/检查/处置/处方（需先完善病历，保存后立即生效）、静脉输液子处方、知情同意书（模板选择→填写→保存→打印，标题由模板自动推导、新建默认追加在下方、删除仅限本人）、转科、诊断证明、多医生接诊（1:N 续写，前序只读，段落签名互不篡改）、跨医生诊断引用查重、医生加号、就诊历史（左右两栏：左列表搜索+右只读纸面病历）、诊室叫号屏幕 |

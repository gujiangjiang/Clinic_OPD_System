...
![版本](https://img.shields.io/badge/版本-v8.1.1-blue) ![PHP](https://img.shields.io/badge/PHP-7.x-777BB4) ![数据库](https://img.shields.io/badge/数据库-SQLite%2FMySQL双驱动-003B57) ![部署](https://img.shields.io/badge/部署-Nginx-009639) ![代码](https://img.shields.io/badge/代码-全中文注释-orange)
...
| 🚨 危急值 | **检验科自动检测**（检验项目可配置危急值上下限，提交结果时自动比对——数值型低于下限/高于上限，**文本型如 HIV 阳性等于危急值文本即触发**，命中弹出通知流程，默认通知开单医生可另选，医生搜索分页无限滚动）；**检验报告单结果与单位之间自动显示趋势纵列**（对照闭合正常范围自动显示 ↓/↑，危急值显示红色「危」字，非数字/无范围/范围非闭合不显示）；**影像科手动上报**（写报告弹窗左下角「报危急值」，手动输入项目加入预览，发布时一并发送）；**医生左右分栏处理**（左侧患者信息 + 危急值 + 完整结果，右侧符合/不符合病情 + 处理措施），处理自动在病历插入只读「危急值记录」续写文书（**以续写形式呈现，不修改首诊，诊毕后亦可插入**）；医生 / 检验 / 影像侧边栏**危急值管理**（时间范围 + 状态筛选，处理时长可溯源）；管理员**查询中心 → 危急值查询**（全院记录按医生端列表呈现）；危急值记录与病历危急值文书全局只读、快照固化，不受检验科撤回修改影响 |

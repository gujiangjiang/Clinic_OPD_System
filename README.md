...
![版本](https://img.shields.io/badge/版本-v7.25.1-blue) ![PHP](https://img.shields.io/badge/PHP-7.x-777BB4) ![数据库](https://img.shields.io/badge/数据库-SQLite%2FMySQL双驱动-003B57) ![部署](https://img.shields.io/badge/部署-Nginx-009639) ![代码](https://img.shields.io/badge/代码-全中文注释-orange)
...
| 🩺 医生工作站 | 多科室切换（标题「医生工作站-科室」可点击切换，仅多科室权限）、所见即所得电子病历（纸质病历版式，所见即所得；输入框自定义右键菜单：复制/剪切/粘贴/清空，嘱托输入框预留模板项；下拉与编辑区其余部分一律屏蔽浏览器原生右键菜单）、顶栏工具箱（加号 / 切换科室 / 患者查询 / 模板管理）与叫号大屏绑定（绑定信息随登录会话持久 + 全局心跳保活，离开页面/刷新不自动解绑，仅退出登录或异常断开才解绑；绑定诊室后「叫号」按钮变为**叫号悬浮窗**（完整版/精简版可切换并记忆所选版本：当前就诊/下一位/重呼/过号/下一位，完整版含可滚动分段加载的完整候诊号源列表并实时刷新，下一位自动认领号源并跳转病历、过号患者大屏显示（过号）标记））、生命体征编辑（与护士站同步）、ICD‑10 诊断联动（搜索→选诊→部位/备注/疑似）、病历模板管理（个人/科室/全院，审核流）、无限续写病历节点（首诊+多段续写，链式拼接、只读/编辑自动切换）、续写必填校验、开检验/检查/处置/处方（需先完善病历，保存后立即生效）、静脉输液子处方、知情同意书（模板选择→填写→保存→打印，标题由模板自动推导、新建默认追加在下方、删除仅限本人）、转科、诊断证明、多医生接诊（1:N 续写，前序只读，段落签名互不篡改）、跨医生诊断引用查重、医生加号、就诊历史（左右两栏：左列表搜索+右只读纸面病历）、诊室叫号屏幕 |
| 💉 护士站 | **工作台布局**（候诊列表「待处置/完成/当日」）：点击候诊患者弹出护理记录单页（急诊病历式抬头；护理记录〔✕ 删除 + 模板模态框添加〕、只读病历摘要〔主诉/现病史/既往史/过敏史/查体/初步诊断 + 查看完整病历预览〕、生命体征趋势图 + 悬浮窗录入〔与医生站双向同步〕、待处理处置 / 待执行医嘱〔医嘱单号、处方号可点击预览〕）；护理记录模板（个人免审 / 科室全院审核） |
| 🧪 检验科 | **工作台布局**（候诊列表「检验中/完成/当日」，申请单号可展开项目）：点击候诊患者弹出检验报告单页（急诊病历式抬头 + 按申请单号组合，整张申请单统一登记）；化验值输入框在项目右侧、**失焦自动草稿暂存**（刷新不丢失），提交生成**横向 A5 检验报告单**（智能分列分页、报告单号自动缩放、检验备注）；申请撤回；检验项目/组合管理（只读，新增走审核） |
| 🩻 影像科 | **工作台布局**（候诊列表「检查中/完成/当日」，申请单号可展开项目）：点击候诊患者弹出影像诊断报告单页（急诊病历式抬头），整张申请单统一登记；「✍️ 去写报告」模态框书写影像所见 / 影像诊断（支持影像报告模板替换 / 追加），提交生成 **A4 检查报告单**（标题按检查分类动态化如 CT / DR、所见 2/3 + 诊断 1/3）；申请撤回；检查项目管理（只读，新增走审核） |
...
| 📋 模板系统 | 病历模板 / 知情同意书 / **护理记录模板** / **影像报告模板**：个人免审即用，科室 / 全院提交管理员审核（审核期间仅本人可用、被驳回自动降级为个人）；管理员可管理全部模板；报告书写模态框支持模板「替换 / 追加」 |
...
│   │                          #           validation / datetime / order / emreditor / emr_ctxmenu / emr / queuepanel /

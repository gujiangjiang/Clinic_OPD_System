...
![版本](https://img.shields.io/badge/版本-v7.14.0-blue) ![PHP](https://img.shields.io/badge/PHP-7.x-777BB4) ![数据库](https://img.shields.io/badge/数据库-SQLite%2FMySQL双驱动-003B57) ![部署](https://img.shields.io/badge/部署-Nginx-009639) ![代码](https://img.shields.io/badge/代码-全中文注释-orange)
...
> **医技科室工作台重构（v7.8.0+）**：护士站 / 检验科 / 影像科 / 药房工作台统一为与医生工作站
> 一致的「顶部患者信息横条 + 左侧候诊列表 + 主工作区」布局，点击候诊患者弹出类似电子病历的
> 患者工作台页，顶栏「叫号」弹出科室排队悬浮窗；公共骨架（`dept_workbench.php`）与公共组件
> （`deptwork.js` + `/api/deptwork`）高度解耦，四角色视图仅保留个性化渲染。
...
| 🩺 医生工作站 | 多科室切换（标题「医生工作站-科室」可点击切换，仅多科室权限）、所见即所得电子病历（纸质病历版式，所见即所得）、顶栏工具箱（加号 / 切换科室 / 患者查询 / 模板管理）与叫号大屏绑定（绑定信息随登录会话持久 + 全局心跳保活，离开页面/刷新不自动解绑，仅退出登录或异常断开才解绑；绑定诊室后「叫号」按钮变为**叫号悬浮窗**（完整版/精简版可切换并记忆所选版本：当前就诊/下一位/重呼/过号/下一位，完整版含可滚动分段加载的完整候诊号源列表并实时刷新，下一位自动认领号源并跳转病历、过号患者大屏显示（过号）标记））、生命体征编辑（与护士站同步）、ICD‑10 诊断联动（搜索→选诊→部位/备注/疑似）、病历模板管理（个人/科室/全院，审核流）、无限续写病历节点（首诊+多段续写，链式拼接、只读/编辑自动切换）、续写必填校验、开检验/检查/处置/处方（需先完善病历，保存后立即生效）、静脉输液子处方、知情同意书（模板选择→填写→保存→打印，标题由模板自动推导、新建默认追加在下方、删除仅限本人）、转科、诊断证明、多医生接诊（1:N 续写，前序只读，段落签名互不篡改）、跨医生诊断引用查重、医生加号、就诊历史（左右两栏：左列表搜索+右只读纸面病历）、诊室叫号屏幕 |
| 💉 护士站 | 采用与医生工作站一致的**工作台布局**（顶部患者信息横条 + 左侧候诊列表「待处置/完成/当日」+ 顶栏叫号排队悬浮窗）：点击候诊患者弹出护理记录单页（护理记录、只读病历摘要〔主诉/现病史/既往史/过敏史/查体/初步诊断〕、生命体征趋势图与手动录入〔与医生站双向同步〕、待处理处置执行、待执行医嘱）；患者搜索、处置执行、生命体征录入、待执行医嘱 |
| 🧪 检验科 | 采用与医生工作站一致的**工作台布局**（候诊列表「检验中/完成/当日」）：点击候诊患者弹出检验报告单页，逐项录入化验值（计量单位/正常范围/危急值提示，组合检验成员逐项录入），提交自动生成报告并打印；检验登记、结果录入、报告生成与打印、申请撤回；检验项目/组合管理（只读，新增走审核） |
| 🩻 影像科 | 采用与医生工作站一致的**工作台布局**（候诊列表「检查中/完成/当日」）：点击候诊患者弹出所见即所得影像诊断报告单页，可直接书写影像所见与影像诊断并提交生成报告；检查登记、影像所见与结论、报告打印、申请撤回；检查项目管理（只读，新增走审核） |
| 💊 药房 | 采用与医生工作站一致的**工作台布局**（候诊列表「待发药/完成/当日」）：点击候诊患者弹出只读打印样式处方单，可直接「通过发药/拒绝」（拒绝填写理由、库存自动恢复）；顶栏保留「📦 库存」入口（入库/出库/流水/低库存预警）、药品出入库；药品信息/设置管理（只读，新增走审核） |
| 🖨️ 开单与打印 | 开单项目按开单医生归档；删除/毁方仅限开单本人（后端硬校验）；打印病历为连续文书（页眉归首诊、续写段虚线承接、各段签名、页脚时间固定）；诊断证明开具即固化病历摘要快照，不随续写变化；知情同意书 A5 打印：正文按段落连续流式分页（自然换行、跨页自动续页、不硬切断词），签名区留在正文最底部最后一页（不预留空页、不落单），页脚仅一行一式两份提示语 |

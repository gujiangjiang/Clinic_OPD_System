...
| ⚙️ 管理员 | 首次安装、医院信息/LOGO/时区、科室/用户管理、检验/检查/药品/处置/诊断管理（ICD-10 完整标准编码库，四级分类树：章→节→类目→亚目，支持按编码/名称/拼音检索与分级浏览）、审核中心（一键通过/驳回重提/预览提交内容/站内消息通知）、组合管理（多对多关联表）、药品设置、检验/检查/药品分类管理、统一打印中心、医院运营分析（KPI 总览/收入趋势/科室医生统计/自定义维度/转归查询）、叫号大屏管理、HIS 预留接口 |

...
### 新增
- **连接测试统一组件**：后端 `app/core/ConnectionTester.php`（`db()` / `cache()`）统一数据库与缓存可用性探测，安装向导与系统设置共用；前端 `conntest.js`（`Clinic.connTest`）统一测试按钮与请求交互，消除重复实现。
- **连接状态可视化**：数据库中心「当前数据库连接」增加连接状态（🟢 正常 / 🔴 不可用，连接失败不再 500 而是返回状态并展示原因）；缓存与性能「缓存状态」增加缓存状态（实际驱动与配置驱动不一致时标记「不可用（已降级）」）。
- **内嵌测试按钮**：数据库密码栏、Redis/Memcached 连接参数末字段内嵌测试按钮（`input-affix`，与输入框融为一体，测试中显示转圈）；安装向导数据库与缓存步骤、系统设置迁移/切换/备份/缓存面板均支持；缓存测试新增 Memcached 支持。

### 变更
- **应用前必测**：数据库迁移、直接切换主库、缓存驱动保存、安装执行前，均先经 `ConnectionTester` 校验目标可用性，不可用则拒绝并提示。
- **修复**：数据库中心迁移/切换/备份三面板参数输入框 ID 冲突导致取值串读（各自独立前缀 migp_/swp_/bkp_）。
...
### 变更
- **管理员侧边栏分组与排序优化**：调整为 首页 / 业务管理（审核中心、叫号管理、科室管理、用户管理）/ 基础字典（检验管理、检查管理、药品目录、药品设置、处置项目、诊断字典）/ 模板与套餐（模板管理、套餐管理）/ 统计与查询（运营分析、查询中心、打印中心）/ 系统设置（接口管理、系统设置）；高频「审核中心」上移，单项分组（接口管理/系统）合并收尾。
- **文案微调**：药品信息→药品目录、诊断管理→诊断字典（侧边栏、页面标题、药房入口链接与相关提示同步）；检验管理/检查管理/模板管理维持原样。

---

## [8.35.0] - 2026-09-26

> 叫号「科室+诊室全称」统一拼接、悬浮窗离线体验优化与上线实时恢复、seed 诊室命名规范。

### 新增
- **科室+诊室全称共用方法**：新增 `naming.js`（`Clinic.deptRoomName`）统一拼接「科室名+诊室名」（诊室名已含科室名或为空时不重复），叫号语音播报、顶栏叫号按钮状态、悬浮窗标题三处共用，避免各处实现不一致。
- **大屏上线实时恢复**：大屏心跳检测「离线→在线」转变时 `push_room_event` 推送 `room:{id}`，叫号悬浮窗即时解除离线蒙板恢复可用；推送丢失或网络异常时，悬浮窗轮询兜底自愈。

### 变更
- **大屏语音呼叫全称**：由「到 1 诊室就诊」改为「到 外科门诊1诊室 就诊」（科室名+诊室名）；顶栏叫号按钮与悬浮窗标题同步显示全称；悬浮窗标题超长自动省略号截断。
- **悬浮窗离线蒙板优化**：mini 版省略图标、精简提示文案（空间小）；标准版蒙板底部保留「解绑」栏，离线时仍可手动解绑。
- **seed 叫号诊室命名规范**：临床/急诊诊室由「外科门诊1诊室」改为「1诊室」（科室名改由叫号时自动拼接，避免重复呼叫），检验「1号采血窗口」、药房「1号发药窗口」、护士站「输液室」同步去科室前缀；急诊诊室默认勾选「允许跨天叫号」；全部诊室默认勾选脱敏。

### 变更（接口）
- 医生/医技绑定诊室与可用诊室接口的 `bound` 增加 `dept_name`，供前端拼接全称与跨页恢复。
...
### 修复
- **模板预览只读字段配色不一致**：下拉禁用态在 WebKit/Blink 下由浏览器 `-webkit-text-fill-color` 灰字覆盖 `color`，导致「患者自诉」等选择项偏淡、与文本输入框主色不一致；显式指定 `-webkit-text-fill-color` 为主色，预览内文本与下拉统一为主色（标准色，不暗淡）。
- **模板预览空字段下划线消失**：只读预览空文本字段因占位提示被 `content:none` 抑制而无行盒，底部虚线随之消失（体格检查等）；模态框预览恢复占位提示，产生行盒使灰虚线下划线正常显示，与病历编辑页统一。
- **页面严格角色隔离**：原角色门对管理员放行，管理员直达 `/doctor/home` 会渲染医生工作站；现改为角色不在路由允许列表时（含管理员访问其他角色页面）自动重定向到 `Auth::home()` 本角色工作台，杜绝通过 URL 越权访问他人页面/数据。

---

## [8.34.3] - 2026-09-26

> 叫号大屏等待列表列宽/数量计算修复 + 预览弹窗自适应。

### 修复
- **大屏预览弹窗缩小后出现滚动条**：预览弹窗高度锁定为可视区高度，`.pv-stage` 由固定 `min(72vh,700px)` 改为 `flex:1` 占满剩余空间，画布随页面/窗口尺寸动态缩放，窗口变小时不再出现内部滚动条。
- **等待就诊列表右侧年龄被裁、姓名被压缩**：根因是列宽探针继承了上一轮测得的 `--wait-*-w` 固定列宽，每轮渲染各列再 +4px 缓冲、逐轮膨胀，最终把右侧年龄列挤出面板。改为探针内联 `grid-template-columns:max-content` 强制按内容自然宽度测量；姓名列设 `minmax(3ch, …)` 保证至少显示 3 个汉字（超出省略号），并收敛列间距、放宽宽屏等待列。
- **等待就诊数量裁剪少显示**：单列改用首行实际 `offsetHeight` 整除可用高度，替代 `scrollHeight/count` 均值（受末行/边框影响），避免首屏实际可显 10 个却只显示 7 个；并在窗口缩放/旋转后按最终尺寸重排重裁、首屏下一帧再裁一次。

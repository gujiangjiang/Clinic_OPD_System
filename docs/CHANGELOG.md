...
- **Tab 连续跳格**：全部填空字段配置 `tabindex="0"`，Tab 向后 / Shift+Tab 向前顺畅跳格；单行语义字段（时间/主要症状等）Enter 拦截换行并自动聚焦下一字段，大段落叙述字段（现病史详情/体格检查/处置/嘱托/续写内容等）允许 Enter 正常换行。
- **粘贴净化**：外部富文本粘贴统一过滤为纯文本——单行字段压平换行、多行字段保留段落换行，防止样式污染。
...
- 右键菜单（复制/剪切/粘贴/清空/模板）在重构后字段上的选区记忆与光标插入完全兼容。

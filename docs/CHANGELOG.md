...
### 新增
- **大屏纵向排列日期时间双行显示**：时钟拆分为日期、时间两段，纵向屏（宽<高）下改为上下两行右对齐、日期字号略小、时间略大，避免单行时钟挤占医院名称；横屏仍单行显示。`screen.php` 为 `screen.js`/`call.css` 增加版本号防缓存。
- **大屏底部提示语上下翻页动画**：多条提示轮播切换时以「上下翻页」动画切入（`translateY` + 透明度）；仅单条提示时不切换、保持静态，超长文本仍走跑马灯。

### 修复
- **叫号悬浮窗叫号后收缩、下一位按钮下移**：候诊号源区 `.doc-call-pool-list` 由 `max-height` 改为固定高度（约 5 行），号源增减时面板高度恒定，底部按钮不再跳动；医生叫号与医技叫号悬浮窗同步生效。
- **大屏等待就诊列表右侧溢出、年龄显示不全**：`.screen-wait-item` 姓名列改为 `minmax(0, var(--wait-name-w, max-content))`，空间不足时优先收缩姓名（省略号），保证右侧性别/年龄列不被面板裁切，纵向/横向/方屏双排均生效。

---

## [8.34.1] - 2026-09-26

> 修复叫号悬浮窗数据不再自动刷新的根因，并为其接入实时推送。

### 修复
- **叫号悬浮窗当前就诊 / 下一位 / 候诊列表不自动刷新**：SmartPoller 弹性轮询管理器在「纯 fetch 模式」（调用方自带请求逻辑、未传 `url`）下，`tick()`/`sync()` 因 `url` 为空直接 `return`，导致 `fetch` 回调永不执行——医生工作站叫号悬浮窗、医技四科室（护士/检验/影像/药房）排队悬浮窗、病历页候诊面板、消息铃铛的兜底轮询全部失效，仅在手动开关悬浮窗（触发一次主动刷新）时才更新。现修正为：仅当既无 `fetch` 又无 `url` 时才放弃调度，纯 fetch 模式正常执行。

### 新增
- **叫号悬浮窗接入诊室实时推送（`room:{id}`）**：医生工作站与医技四科室悬浮窗在打开时订阅所属诊室通道，收到叫号/过号/重呼事件即时刷新当前就诊、下一位与候诊列表（与叫号大屏同源），关闭或切换诊室时自动退订换订，配合弹性轮询实现「推流即时 + 轮询兜底」。

---

## [8.34.0] - 2026-09-25

> 病历文本录入区彻底重构为「流式行内编辑器」，长文按字符级自然折行。

### 变更
- **流式行内编辑器（Inline Flow Editable）**：病历书写区（主诉/主要症状/持续时间/次要症状/现病史具体内容/既往史详情/体格检查/辅助检查结果/门诊处置/嘱托等全部任意文本录入字段）由块级输入控件重构为行内可编辑 span（`contenteditable`），以纯行内字符流渲染——长句到达行末仅超出字符自然折到下一行首，后方文字与标点紧随其后，中间绝无多余空格、占位或整块下坠；不再被父级 Flex 盒模型强制整体换行。
- **Tab 连续跳格**：全部填空字段配置 `tabindex="0"`，Tab 向后 / Shift+Tab 向前顺畅跳格，聚焦控件显示主色软底高亮（与悬停同款，直观可见当前焦点）；单行语义字段（时间/主要症状/现病史详情/体格检查/辅助检查等）Enter 拦截换行并自动聚焦下一字段，仅嘱托（大段落叙述）允许 Enter 换行。
- **粘贴净化**：外部富文本粘贴统一格式化——单行字段压平为连续文字并自动去除空行、行内首尾空白与全角空格占位符，仅嘱托保留段落换行，防止样式污染与空行残留。
- **模板导入归一化**：科室/个人模板应用路径增加残留输入控件归一化兜底，导入后病历书写区 DOM 内绝无 `input`/`textarea`，一律为行内可编辑 span。
- **只读/预览自适应**：诊毕只读还原为病历正文文字（无输入框痕迹）；模板/套餐预览模态框内仍保留蓝色填充与虚线下划线形态。

### 新增
- **事件委托层**：容器级 `bindInlineFillEvents` 覆盖模板导入等动态生成字段的纯文本粘贴保护、单行回车跳格与删空清残留 `<br>`（保证占位符 `:empty` 生效）；`getFieldValue`/`setFieldValue` 兼容读写接口与 `normalizeInlineFields` 防残留归一化接口对外暴露。
- **EMR 资源时间戳防缓存**：`components-emr.css`、`emreditor.js`、`emr_template.js`、`emr_segments.js` 按文件修改时间戳做缓存失效，重构期浏览器不再命中旧版本缓存。

### 修复
- 病历文本字段横向下坠整块换行、框内撑大割裂前后文连贯性的历史问题。
- **空字段点击无法锁定输入**：浏览器对无内容的行内 contenteditable span 的默认 mousedown 处理不聚焦、且会把焦点保留在上一字段（无头浏览器实测：点击「外院辅助检查结果」后输入落入「辅助检查结果」）。空字段 mousedown 拦截默认处理并手动聚焦 + 光标到末尾，有内容字段保留原生点击定位；实测点击后 active=true、输入正确落入所点字段。
- **字段底虚线与选择框水平对齐校准**：无头浏览器实测行内元素边框盒按内容区高度绘制、选择框按 line-height 绘制，同参下字段底线偏高约 3.4px；padding-bottom 补偿后实测差值 0.00px，与选择框虚线完全同一水平线。
- **字段聚焦背景与选择框同高同顶**：无头浏览器实测字段盒高 23.4px、选择框盒高 25.4px，聚焦/悬停蓝色背景顶部差约 2px；padding-top 补偿后 topGap/bottomGap 均实测 0.00px，高亮背景与选择框完全一致。
- **时间字段限纯数字**：主诉两个时间、现病史时间共三个字段实时拦截非数字输入并 toast 提示（800ms 节流）；粘贴仅保留数字、IME 等渠道混入字符即时剔除，非时间字段不受影响。
- 右键菜单（复制/剪切/粘贴/清空/全选）在重构后字段上的选区记忆与光标插入完全兼容。
...
### 修复
- **病历模板正文双层边框**：模板编辑/预览模态框中 `.emr-doc` 纸张（12px 圆角边框）内又套了一层 `#templateEditor` 内框（`border + border-radius:8px`），与 v8.33.1 修复的病历编辑器属同类历史遗留——内框冗余叠加纸张边框，四角出现两圈圆角错位。已移除内框的 `border/border-radius/padding`，仅保留 `min-height:380px`，模板正文直接渲染在纸张上，与真实病历文档版式一致。审核中心/护理/影像模板共用同一模态框逻辑一并生效。
...
### 修复
- **病历编辑器双层边框/圆角叠加**：`#emrCard` 原套 `.card` 外层（`border-radius` 10px、双边框、双阴影）与内层病历纸 `.emr-doc`（12px 圆角 + 自身边框/阴影）叠加，四角出现两圈不同弧度的圆角错位。该外层卡片为历史遗留——`.emr-doc` 早已自带完整纸张样式，外层仅剩 `padding:0` 禁用了内边距的空壳。已移除 `#emrCard` 的 `card` 类与 `overflow:hidden`，保留 16px 底部间距，仅剩单层 12px 圆角纸张边框，纸张阴影恢复可见。

---

## [8.33.0] - 2026-09-25

> 会诊只读病历查看、个人中心安全邮箱、造数链路补全。

### 新增
- **会诊详情模态框内联预览只读病历**：会诊完毕（done）或就诊已诊毕（finished）的会诊患者，点击会诊条目即可在同一模态框预览只读病历（复用打印版式渲染，支持「← 返回会诊详情」），不再整页跳转；病历页右侧会诊列表 / 正文链接 / 候诊会诊tab 三处入口统一生效。
- **个人中心安全邮箱输入框**：与管理员用户管理一致（文案、格式校验规则相同），随个人资料修改申请提交审核，审核通过写入 `users.email`。

### 修复
- **造数不再生成同科室会诊**：`VisitSeeder` 会诊目标原从全部科室随机取，可能选中来源同科室（如急诊科→急诊科）且会向医技/药房发会诊；现统一仅从其他临床/急诊科室选取并排除本科室，会诊医师限定为目标科室绑定医生。
- **造数病历字段结构化**：首诊 informant、续写 informant/arrival_way 不再硬编码「患者自诉/自行来院」，一律取自结构化 `emr_data` 投影，与真实保存链路一致。
- **造数处方发药/库存链路补全**：`order_items` 补写 `unit_type/pack_size`、`dispensed` 订单补发药人（`done_by`）、处方开单即按 `数量×pack_size` 原子扣减 `drugs.qty` 并写 `inventory_trans`（order_out）。

### 变更
- **会诊完毕只读放行**：新增 `visit_consult_done_authorized` 后端权限点，会诊结束后目标科室医生仅可只读打印/查看病历（不可再处理、修改），`print.php` record 动作追加该只读放行。
- **诊毕会诊不再拦截**：候诊会诊tab 中已诊毕患者点击直接打开会诊详情模态框查看只读病历（此前仅提示无法会诊）。
...
### 修复
- **非会诊目标科室医生看到「🤝 确认会诊」横幅**：A 外科发起 → 急诊科会诊后，同在外科的 B 医生点开该患者，续写占位误显示为「确认会诊 · 会诊病历创建后即可书写会诊记录…」。原 `renderEmrCard` 分叉只按 `status ∈ {pending, doing}` 取本就诊任一会诊、不比对目标科室。现改为仅在当前医生处于会诊模式（后端 `consult_mode` 权威 + 前端 `__consult_mode` 兜底，即存在发给本科室的进行中会诊）时才渲染会诊横幅，并按 `consult_code` 精确匹配对应会诊单；否则一律显示「📝 病历续写」。

### 变更
- **恢复被误删的 `data/db/icd10.db` 并加装守护**：该字典库曾在某次 `git add -A` 提交中因工作区文件缺失被误暂存删除（.gitignore 一直有 `!data/db/icd10.db` 反忽略，并非忽略所致），已重写对应历史提交恢复完整 33304 条诊断库并强制推送同步远程。`npm run lint` 新增守护校验：`data/db/icd10.db` 缺失或未被 git 跟踪即判失败；AGENTS.md 补充该文件的例外铁律。

---

## [8.32.0] - 2026-09-25

> 安装向导：config.db 归属校验、关联库机构预填、弹窗队列。

### 修复
- **仅存主库而无 config.db 仍判定已安装**：现要求 `config.db` 存在、可用且含数据库驱动设置，再校验主库安装标记；无 config.db 即进入安装向导，且 bootstrap 不再自动新建 config.db。
- **多个校验弹窗叠在一起**：新增轻量对话框队列，同一时刻仅显示一个，关闭后再弹下一个，全部解决后才进入下一步。

### 新增
- **关联现有库机构信息预填**：新增 `load_db_settings` 接口，关联现有库时读取主库的医院名称/机构代码/第二名称/联系电话/联系地址/时区并预填 Step4（`views/install.php`）。
- **机构信息变更确认**：关联模式下若医院名称/机构代码/第二名称被修改，下一步前二次确认可能的历史单据/报表口径影响；未修改则直接下一步。

---

## [8.31.0] - 2026-09-25

> 安装完成标记归属主数据库 + 安装页确认/ICD-10 交互修复。

### 修复
- **安装完成标记错误存于 config.db**：删除主数据库后系统仍判定「已安装」，直接进入登录页并自动生成空主库（无管理员无法登录）。现改为安装标记写入【主数据库】`settings.install.done`，config.db 仅存连接配置；删除主库即视为未安装并重新进入安装向导。
- **主数据库自动创建**：安装状态探测完全非破坏性（SQLite 文件缺失/远程库无表直接判定未安装），杜绝因一次探测而生成空主库。
- **安装确认页 `dbParams is not a function`**：`renderConfirm` 内局部变量 `var dbParams` 遮蔽同名函数，改名修复。

### 新增
- **ICD-10 诊断库缺失交互**：文件不存在时弹窗提示放入可用诊断库或创建空库（仅表结构）；字段不匹配则报错 toast 并阻止下一步。
- **ICD-10 名称随请求校验**：`check_db` 携带用户填写的 `icd10_name`。
...
### 修复
- **安装页点击下一步提示「未知操作」且仍生成 clinic_main.db**：`Clinic.get` 在已有查询串的 URL 后再拼 `?`，导致 `action` 被解析为 `check_db?driver=...`；改为将 `action` 放入请求参数对象统一编码。
- **SQLite 数据库名称描述冗长**：移除「（如输入 123 与 123.db 均创建 123.db）」示例。
- **ICD-10 诊断库未校验完整性**：新增结构校验——文件存在时必须为有效 SQLite 且 `icd10` 表字段齐全，缺字段报错并阻止下一步。

### 变更
- **非本系统库二次确认**：目标库非空但未检测到本系统安装数据时弹窗确认。
- **驱动扩展缺失提示**：所选数据库驱动扩展未安装时内联红字提示。
- **全新安装清库提示**：确认页在全新安装且目标库已有数据时红字提示将清空。

---

## [8.30.0] - 2026-09-25

> 安装向导专项优化 + 用户邮箱 / 医院联系信息字段补齐。

### 新增
- **用户安全邮箱字段**：`users` 表新增 `email` 列（schema v40 + 迁移 + 运行时列自愈）；用户管理表单/保存支持安全邮箱（格式校验），安装向导管理员邮箱写入同一字段，保证安装与后台一致。
- **医院联系电话 / 联系地址**：系统设置-医院信息新增两输入框并持久化到 `contact_phone` / `contact_addr`，与安装向导共用同一 settings 键。
- **ICD-10 诊断库名称自定义**：安装数据库步骤可填写诊断库名称（默认 `icd10.db`，可省略后缀），路径写入 `config.db`（`db.icd10.path`），不写主库。
- **重复安装检测弹窗**：数据库校验后若检测到已有安装完成数据，弹窗选择「关联现有数据库」或「全新安装」；全新安装清空目标库后重建。

### 变更
- **安装向导拆分为 6 步**：数据库与缓存设置拆成独立页面；Step1 移除「已有数据」展示并新增重新检测。
- **SQLite 安装仅填写数据库名称**：统一存放 `data/db/`，自动补全 `.db` 后缀（如 `123` 与 `123.db` 均创建 `123.db`）。
- **数据库必填与连接强校验**：MySQL/PostgreSQL 主机/端口/库名/用户名/密码必填，点击「下一步」强校验连接后才可继续；测试连接按钮内嵌密码栏右侧，SQLite 不显示测试连接。
- **安装导航按钮对齐**：上一步靠左、下一步靠右。

### 修复
- **安装向导完成数据库选择前不再自动创建 `clinic_main.db`**：系统安装状态改以 `config.db` 的 `install.done` 标记为准（兼容旧库非破坏性探测）；`public/index.php` 仅在已安装时初始化主库；`bootstrap` 站点时区与 `setting()` 读取增加未安装短路。
...
### 修复
- **医保机构编码/HIS 系统代码只读文字仍为黑色**：只读输入框规则补齐 `color: var(--text-muted)`（此前仅灰底、文字未变灰，与禁用态不统一）；排除 `.input-date` 保持日期框独立样式。
- **HIS 调用示例未配置密钥时点击误报复制成功**：`copyHisCurl` 改为按当前密钥判断（此前检查旧占位文案 `<密钥>`，占位文案已改导致检查失效），未配置时与接口地址一致 toast 提示先生成密钥。
- **安装页报 `Clinic is not defined`**：安装页初始化包裹 `DOMContentLoaded`（页面内容脚本先于 layout 末尾的 ajax.js 输出，立即执行时 Clinic 未定义）。

### 变更
- **config.db 统一迁移至 data/db/ 目录**（原 data/config.db）：与主库/字典库/Session 数据目录一致；`ensure()` 兼容旧位置自动迁移；.gitignore/AGENTS/HELP 路径同步。
- **数据库中心迁移图标改为向右箭头**（➡️）。
- **备份/同步卡片标题右侧显示最近同步/备份时间徽章**（靠右对齐，按状态着色）。
- **安装页移除硬编码「🗄️ 数据库：SQLITE」徽章**（数据库由用户手动选择，徽章多余）。

### 文档
- **同步版本号至 v8.29.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。
...
### 修复
- **非 HIS 接口字段渲染丢失**：v8.26 重构 HIS 接口左右栏时，非 his 分组（支付/医保/PACS/HL7/FHIR）的字段表单渲染被误删，恢复 else 分支字段循环。
- **HIS 左右栏样式未生效**：db-center 左右栏样式从 settings.php 页面级移入公共 layout.css，数据库中心/系统设置子项/接口管理共用。
- **HIS 接口地址与调用示例 I-beam 光标闪烁**：移除 user-select:all（点击全选导致文本竖线悬停闪烁），改 user-select:text + cursor:pointer。

### 新增
- **医保机构编码/HIS 系统代码自动引用医疗机构代码**：`yibao_org_code`/`his_system_code` 字段标记 source=org_code，渲染为只读并自动填充系统医疗机构代码（org_code），无需人工输入。
- **数据表浏览器刷新按钮移位**：移到标题右侧徽章旁（移除底部独立按钮）；数据表查看模态框标题栏 CSV 右侧新增刷新按钮。
- **缓存与性能详情页缓存驱动徽章**：与数据库中心详情驱动类型一致，badge-primary 呈现。
- **HIS 调用示例未开启占位**：密钥未配置时 curl 示例显示「请先填写接口密钥并保存，调用示例将自动生成」（与地址占位一致）。

### 变更
- **网站时区改用完整时区列表**：与安装向导一致，DateTimeZone 全量按区域分组（不再只列常见时区）。
- **品牌外观改标题为医院标志**：系统设置页签/边栏/卡片标题统一。
- **数据库中心侧边栏【备份】改为【备份/同步】**。

### 文档
- **同步版本号至 v8.27.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。

---

## [8.26.0] - 2026-09-24

> 数据库中心/接口管理交互优化与侧边栏消息徽章专项。

### 修复
- **审核已处理项目不存在时双 toast 修复**：模板/套餐预览抽为通用函数 `previewEntityPage`，接口失败不再手动 toast（ajax.js 已自动提示接口原因），仅接口成功但数据缺失时提示一次。
- **缓存驱动切换显示当前驱动**：进入缓存 Tab 时自动选中 config.db 当前实际驱动，不再恒显 File。

### 新增
- **侧边栏站内消息未读数字徽章**：notify.js 同步更新侧边栏徽章（99+ 封顶）；展开时靠右对齐、缩小（sidebar-mini）时固定在图标右上角角标。
- **HIS 接口左右分栏**：接口管理 HIS 接口改为 db-center 左右栏（详情/配置/接口/测试/说明）——详情面板展示启用状态徽章（密钥为空=未启用）、系统代码、同步模式、接口地址。
- **日志模态框滚动加载**：日志查看固定大小（modal-lg + 420px 高），infiniteList 滚动到底自动加载更多；标题去掉冗余后缀、空态用 .empty 组件。

### 变更
- **医院机构信息/缓存与性能/安全与加密 Tab 改左右分栏**：医院机构信息（医院信息/品牌外观/作息时间/网站时区）、缓存与性能（详情/驱动/刷新）、安全与加密（登录安全/加密混淆），复用 db-center 两栏布局。
- **密码修改模态框**：忘记密码按钮改红色并移至底部按钮区（与取消/确认同排、靠左）。
- **数据表查看模态框**：修复滚动加载（首屏渲染完整表格），固定大小，标题去掉「（滚动加载）」备注；空态改 .empty 居中组件。
- **HELP.md 移入 docs/ 目录**（文档统一归档），README 链接同步。

### 文档
- **同步版本号至 v8.26.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。
- **AGENTS.md 新增 UI 文案克制铁律**：禁止冗余备注后缀、空态统一用 .empty 组件。

---

## [8.25.0] - 2026-09-24

> 数据库中心交互优化与审核预览拦截修复专项。

### 修复
- **审核已处理项目不存在时预览仅 toast 不跳转**：模板/套餐预览先预检实体存在性，后台误删导致项目不存在 → 直接 toast 提示（该审核项目已被删除，无法预览），绝不跳转页面。
- **数据表查看滚动加载不显示数据**：infiniteList 首屏清空容器导致表格结构丢失；改为首屏由 render 返回完整 table（含 thead/tbody），后续页仅返回行插入 tbody（first 标志区分）。

### 新增
- **备份/双向共用驱动选择组件**：备份与双向同步共用顶部驱动选择与数据库配置（bkDriver + bkParamsBox），下方【备份】【同步】按钮切换特定内容（备份=定时自动备份+立即备份；同步=镜像延迟+状态+开关），统一【保存设置】按钮（备份模式自动关双写、同步模式自动开启）。
- **切换主库支持同驱动**（如 MySQL → 另一 MySQL 库）：切换面板显示当前主库类型徽章，后端移除同驱动拒绝。
- **备份/同步最近时间显示**：db_status 返回 last_backup_at/last_dual_at；双写镜像成功 30 秒节流更新 dual.last_at，备份面板显示最近一次备份/同步时间。
- **徽章化**：数据表浏览器标题右侧显示当前库类型徽章；当前数据库连接页驱动类型改为徽章呈现。
- **密码修改模态框按钮区优化**：忘记密码按钮移至底部按钮区（outline），与取消/确认修改同排同大小、靠左对齐。

### 变更
- 个人信息页 GitHub 风格布局、侧边栏菜单合并（上版本已含，本版本继续完善密码模态框按钮区）。

### 文档
- **同步版本号至 v8.25.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。
...
### 新增
- **备份/双向二选一子 Tab**：备份面板改为「备份 / 双向同步」两个子 Tab（二选一）——备份子面板含定时自动备份；双向子面板含镜像延迟与状态；统一【保存设置】按钮（备份模式自动关闭双写、双向模式自动开启）。
- **操作日志功能**：`MigrationRunner::log/logs/clearLogs` 记录备份成功/失败、定时备份、双写开关、镜像写入失败等详情（`data/logs/db.log`）；备份面板新增【查看日志】模态框（滚动列表 + 清空 + 导出 .log 文件，文件名带日期时间戳）。
- **详情页活动任务**：数据库中心详情页显示当前活动的迁移/切换（进行中或待确认）、双向实时同步（已开启）、定时备份（配置与最近结果）；未启用不显示。
- **个人信息页 GitHub 风格重构**：左侧资料卡（头像/姓名/工号/用户名/角色）+ 左下方设置区（界面主题/打印偏好/账号安全）；右侧基本资料与学历/学位/介绍编辑；密码修改改为模态框（原密码/新密码/确认 + 忘记密码申请，校验与后端逻辑不变）。

### 变更
- **侧边栏菜单合并**：【个人信息】【修改密码】合并为【个人信息】（密码修改在个人信息页内模态框）。
- **审核模板/套餐预览改 SPA 局部刷新导航**：不再新开窗口，管理页加载后自动弹出原始只读弹窗；实体不存在时统一走审计快照分支、无快照单次提示（修复双 toast 重复）。
- **数据表查看模态框改滚动动态加载**（infiniteList，去除分页按钮）；数据表浏览器移除重复的导出 CSV 按钮、列表延伸至页面底部（与科室管理/个人管理等底部边距统一）。
- **安全与加密 Tab 布局统一**为占满宽度卡片（登录安全与防爆破、URL 混淆密钥两个独立卡片）。

### 文档
- **同步版本号至 v8.24.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。

---

## [8.23.0] - 2026-09-24

> 数据库中心重构、双向实时同步与文档拆分专项。

### 新增
- **数据库中心左右两栏布局**：左侧边栏（详情/浏览/迁移/切换/备份）+ 右侧内容区，模块导航清晰。
- **双向实时同步（RAID1 式双写）**：`DatabaseManager` 新增双写镜像层，DB 写操作（INSERT/UPDATE/DELETE）成功后实时镜像到备份库；仅同驱动可靠（跨驱动提示改用迁移/定时备份），备份库未初始化时静默跳过、失败自动降级，绝不降低主库体验。与备份功能分离（备份=手动/定时全量；双向=写入实时镜像）。
- **定时自动备份**：备份面板选择每天时刻（00:00-23:00），页面调度检查到点且当日未备份 → nohup 后台 CLI 执行（`tools/cli/db_backup_run.php`），不阻塞页面，running 5 分钟锁防并发、完成后写 last_date 去重。
- **数据表浏览兼容 PostgreSQL**：表清单/字段属性用 information_schema 查询（SQLite/MySQL/PG 三驱动均可浏览）。

### 变更
- **审核中心预览彻底去重**：删除 review.php 内复制的模板/套餐弹窗渲染代码，模板/套餐预览统一跳管理页原始弹窗（previewTpl/previewPkg + readonly + modalReadonly 禁止复制/右键），URL 携带 audit 参数——实体被删除时由管理页按审计快照渲染同一份原始弹窗；item_* 保持 forms.php 同源原始表单（audit_preview + makeReadonly）。
- **README 拆分**：使用帮助全部移入独立 `HELP.md`（搭建/初始化造数/生产部署/Session 存储/数据库切换/数据库中心/迁移/备份/定时/双写/安全/开发约定），README 精简为纯项目介绍并在顶部建立直达链接。

### 文档
- **同步版本号至 v8.23.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。

---

## [8.22.0] - 2026-09-24

> 数据库中心能力专项：后台迁移、全站锁定、主库切换、多库备份与驱动体系完善。

### 新增
- **后台数据库迁移**：`db_migrate` 改为 MigrationRunner 启动 nohup 后台 CLI 任务（`tools/cli/db_migrate_run.php` 独立进程），刷新页面不中断；逐表更新进度到 config.db，前端轮询展示进度条。支持 SQLite / MySQL / PostgreSQL 任意双向迁移。
- **全站迁移锁定页**：迁移进行中（running）或完成后待确认（done）期间，`public/index.php` 拦截全部页面与接口（除 migration 状态接口）至 `migrating_lock.php`——半透明遮罩 + 进度条（表/行进度/当前表）+ 重新登录按钮；管理员（localStorage 令牌）显示取消按钮，取消后完成当前表即停止、主库指针未改自动回退原库。
- **迁移成功切换询问**：迁移完成后锁定页询问是否将主库切换为目标数据库（switchMain 更新 config.db 指针 / 暂不切换保留原库），切换/迁移启动时强制清除全部用户会话（所有人重新登录）。
- **直接切换主库**（db_switch）：目标库须已有完整数据（users 表非空校验），校验后强制清会话并切换 config.db 指针。
- **多数据库备份**（backup_save/backup_run）：配置备份库（sqlite/mysql/pgsql）并同步当前主库全部数据到备份库（47 表实测），不动主库指针、可反复备份。

### 变更
- **强制创建 config.db**：bootstrap 启动即 `ConfigStore::ensure()`（缺失/损坏自动重建并回填默认配置），系统设置等配置写入统一以 config.db 为唯一载体。
- **系统设置清理**：移除「外部接口已迁移」提示与【外部集成与接口】Tab（外部接口已独立）、隐藏右上角数据库徽章。
- **迁移目标驱动屏蔽当前驱动**（同格式迁移无意义）；迁移工具/备份库/缓存驱动下拉均按驱动注册表动态渲染。

### 修复
- **审核中心预览完全复用添加/编辑原始模态框**：item_* 已处理预览统一用 `modal.load` 打开 `audit_preview`（与编辑弹窗同款尺寸/布局/字段，仅只读）；套餐快照预览改为与原始 pkg-form 两栏布局（左侧信息+右侧内容）；audit_preview 无快照回退实时数据渲染原始表单。
- **数据表浏览器**：关闭后再次点击无法弹出、翻页后样式丢失/背景透明/排版错乱——改为每次打开新建弹窗（不缓存已关闭引用）。

### 文档
- **同步版本号至 v8.22.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。
...
### 修复
- **电子病历现病史具体内容长文本横向溢出**：此前 `white-space:nowrap` 使字段宽度随内容无限增长，内容超行宽后字段整体换到下一行便不再换行、横向溢出病历编辑区；改为自适应多行——宽度随内容增长（最短 260px），内容超过可用行宽时在超出处自动断行到下一行继续书写（自动分节），下一行再超再换，`max-width:100%` 保证绝不横向溢出，整体仍是一个字段，并以背景底线模拟病历横线（多行每行都有下划线边界）。
- **审核中心预览与添加/编辑模态框样式不一致**：项目类（检验/检查/药品/处置）快照预览改调用 `form_item`/`form_drug`/`form_disposal` 渲染【与添加/编辑完全一致的原始表单】，仅值来自快照 + 前端 `makeReadonly` 只读化（此前为简化字段列表）；表单函数新增可选快照参数（$snap），项目被删除后仍可按提交时数据回填渲染；处置表单从 admin_disp.php 收敛到 forms.php 消除重复实现。模板/套餐预览统一复用原始模态框——实体仍存在跳管理页原始弹窗只读预览，实体已删除用快照渲染（历史快照缺 content/items 时回退实体当前数据）。

### 新增
- **工具箱患者查询日期范围默认近一个月**：医生工具箱/医技工作台患者查询弹窗建档日期范围默认填充最近 1 个月（`Clinic.datePicker.lastRange(29)`），可手动修改，跨度上限 1 年由 datePicker maxSpan 钳制。

### 文档
- **同步版本号至 v8.20.1**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。

---

## [8.20.0] - 2026-09-24

> 基础设施架构重构专项：config.db 配置隔离、安装向导、系统设置多 Tab、数据库迁移引擎、缓存管理。

### 新增
- **config.db 基础设施配置库（Phase 1）**：新增 `ConfigStore` 核心类，独立存储主库驱动/连接凭证、缓存驱动、App Key、维护模式；打开前校验 16 字节 SQLite Magic Header，损坏/非法文件自动备份为 `config.db.corrupt.[timestamp]` 并优雅降级（绝不 500）；删除 config.db 仅重置配置、不破坏业务数据。数据库驱动与 Session 缓存驱动优先读取 config.db（回退 bootstrap 默认常量，向后兼容）。
- **5 步安装向导（Phase 2）**：环境巡检（PHP 版本/扩展/目录权限/已有数据检测）→ 数据库与缓存（SQLite 指定路径/MySQL 参数 + 测试连接 + 全新安装/关联现有库 + Redis 测试）→ 医疗机构（机构/时区/联系方式）→ 管理员（含邮箱；关联模式自动跳过）→ 确认汇总执行。关联现有库仅重写配置并校验主库结构，不破坏已有数据。
- **系统设置多 Tab 化（Phase 3）**：医院机构信息/数据库中心/缓存与性能/安全与加密/外部集成与接口 5 个 Tab；数据库中心含连接状态（驱动/延迟/表数量/库大小）、数据表浏览器（字段属性 + 分页行数据 + CSV 导出）、迁移工具入口；缓存与性能含缓存状态、驱动切换、模块化刷新。
- **SQLite ↔ MySQL 双向迁移引擎（Phase 4）**：`DatabaseMigrator` 分批 Chunk 500 行同步、外键约束临时关闭/恢复、自增序列校准（sqlite_sequence/AUTO_INCREMENT）、防原地迁移保护、迁移期间维护模式、完成后自动更新 config.db 主库指针。
- **缓存/会话驱动联动（Phase 5）**：Session 驱动跟随 config.db 缓存驱动（显式环境变量可覆盖），redis 连接参数优先读配置库；缓存驱动切换接口（校验扩展可用性）。

### 变更
- bootstrap 加载顺序调整：ConfigStore 先于 Session 加载（Session 依赖配置库驱动）。
- 系统设置数据库徽章保留；外部接口迁移提示改为独立 Tab 跳转接口管理页。

### 文档
- **同步版本号至 v8.20.0**（大重构提升次版本号；README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。

---

## [8.19.3] - 2026-09-24

> 日期范围、seed 虚拟病历与病历编辑器缺陷修复专项。

### 修复
- **`lastRange` 报 Assignment to constant variable**：`s/e` 误用 `const` 声明却在 yesterday/month/year 分支重新赋值，运营分析初始化即触发崩溃——改为 `let`。
- **日期顺序实时校验逻辑颠倒**：`clampOrder` 用 `val>=peerVal` 一刀切判断"合法"，导致开始框选早于结束的日期被误判倒置并强制同步对端；现按框角色区分——结束框合法条件 `val>=peer`（结束≥开始）、开始框合法条件 `val<=peer`（开始≤结束），越界时才对端同步并提示。
- **seed 虚拟病历体格检查显示 content:**：`emr_pe_text` 按「键：值」渲染体格检查，但 VisitSeeder 误用 `content` 键存体检描述；改按结构化键存放，体检描述归入「其它体格检查」（records 镜像表同步）。
- **seed 虚拟病历现病史非结构化**：现病史改为完全按结构化字段生成——供史者(患者自诉)+时间+单位+内容，content 以「前」衔接时间单位、末尾去掉句号（渲染时固定拼接「，来院途径」）；起病方式按主症状是否含伤/摔/切/撞/扭/压字眼自动选择；piTails 文案池去掉句号与「来院/就诊」字样防止重复。

### 新增
- **seed 虚拟病历续写/会诊场景增强**：续写医生池 50% 概率加入首诊医生，形成同医生/跨医生两类续写；finished_recent 配额改概率制（约 2/3 续写、1/2 会诊、20% 诊断证明），便于测试续写病历与会诊流程。
- **电子病历现病史具体内容整体换行**：字段新增 `ef-field-nowrap`（white-space:nowrap），内容不换行、宽度随内容增长（最短 260px），过长时整个字段换到下一行；仅影响编辑器展示，打印不受影响。

### 变更
- **运营分析默认联动**：初始化默认日期为本月时，同步将快捷按钮「本月」置为高亮（此前高亮「今日」与日期范围不一致）。
- **审核中心一键全部通过移至右上角**：从筛选栏右侧组移至 `.page-head` 右上角（与科室管理新增按钮一致），不再随待审核/已处理页签切换显示/隐藏引起布局跳动。

### 文档
- **同步版本号至 v8.19.3**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。
...
### 新增
- **`Clinic.datePicker.lastRange` 支持多跨度语义**：数字=最近 N 天（含今天）、`'today'/'yesterday'/'month'/'year'`，并统一替换全站默认时间跨度计算——危急值查询（近 3 天）、运营分析（快捷范围/初始本月）、打印中心/影像引用/审核中心（近 7 天）。删除 analytics 本地 `anaDate/anaPad` 与 critical 本地 fmt、querycenter PHP 侧日期计算，消除重复实现。
- **日期范围顺序实时校验**：`datePicker` 新增 `clampOrder`，选择日期后若结束早于开始，自动同步对端并 toast 提示（开始框 `maxToday:false` / 结束框 `maxToday:true` 约定），先于跨度钳制执行，保证范围始终合法。
- **审核中心已处理记录预览快照**：检验/检查/药品/处置/模板/套餐提交审核时，将提交时的完整字段快照写入 `audits.data`；已处理（通过/驳回/使用）记录预览优先用快照在当前页只读渲染，发起者删除项目后仍可预览原始提交内容供追溯。历史无快照记录回退原表单/模板/套餐接口。
- **审核中心筛选栏查询按钮**：日期范围组靠左并新增【查询】按钮（此前选日期后无法生效），页签/分组/一键全部通过移到右侧，组件间距统一为 `flex gap-8`。

### 修复
- **日期重置按钮语义统一**：打印中心/影像引用/审核中心重置由「清空回全部」改为「恢复默认最近一周并填充显示」（与危急值查询重置恢复默认范围一致）；打印中心页面初始化复用 `pcReset()` 消除重复计算。
- **发起者删除项目时撤销待审核请求**：检验/检查/药品/处置删除接口同步删除该项目 `pending` 状态的审核记录（项目已不存在无需再审核）；已处理审核记录保留。

### 变更
- **审核列表行新增 `data-status`**：前端据记录状态（待审核/已处理）选择快照或实时数据预览来源。

### 文档
- **同步版本号至 v8.19.2**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json` + AGENTS.md 基准版本）。
...
### 修复
- **查询中心危急值/影像引用统一为两卡结构**（无头浏览器截图验证）：危急值 `#qcCritical .card { margin-bottom:0 }` 覆盖了卡片间 16px 间距导致筛选卡与列表卡靠在一起，改为仅筛选卡保留 `margin-bottom:16px`、列表卡仍贴底对齐；影像引用整个 tab 原为一张卡（`.qc-ref-card` padding:0）搜索栏和列表共用边框融为一体，拆分为标准两卡结构（筛选卡 `.card list-filter` + 列表卡 `.qc-ref-list-card`，pacs.css 同步重命名去掉旧式 calc 定高改 flex 占满剩余高度）；卡片间 16px 间距与打印中心/科室管理一致。
- **影像引用查询排序改为登记时间倒序**：原按 `ir.id DESC`（写入顺序），补录的历史数据（写入顺序≠登记时间顺序）显示乱序，改为 `ir.created_at DESC, ir.id DESC`（登记时间倒序，最新登记在最上面）。

### 移除
- **临时测试脚本**：无头浏览器测试用 `test_session.php` 与 `?tab=` 测试用深链接（影像引用列表的 🔍 调阅按钮不受影响）。
...
### 新增
- **统一打印中心登记时间日期范围筛选**：检索工具条新增 开始/结束日期（与危急值/运营分析页一致）+ 重置清空；后端 `print_visits` 新增 `from`/`to` 参数（`date(r.registered_at)` 筛选），减少全表扫描量。
- **审核中心申请时间日期范围筛选**：工具条新增 开始/结束日期 + 重置按钮；后端 `audit_list` 新增 `from`/`to` 参数（`date(created_at)` 申请时间筛选），平铺分页与分组全量两种模式均生效。
- **医生/医技工作站患者查询建档日期范围筛选**：患者查询弹窗（`Clinic.patientSearch`，医生工具箱/医技工作台共用）新增建档日期范围（可选）；后端 `/api/patient?action=search` 新增 `from`/`to` 参数（`date(created_at)` 建档时间筛选），`PatientRepository::search` 支持日期范围。
- **日期范围跨度钳制（差异化限制，防数据库压力）**：新增统一 `date_span_clamp()`（`helpers.d/input.php`），各功能开始/结束跨度不能超过该功能域上限，超限时以结束日期为锚前推：
  | 功能 | 上限 | 设计理由 |
  |---|---|---|
  | 危急值查询 | 31 天（1 个月） | 即时告警数据，超月无临床回溯价值 |
  | 统一打印中心 | 92 天（3 个月） | 就诊记录量最大，补打集中近三月 |
  | 影像引用查询 | 183 天（6 个月） | 影像台账 PACS 调阅溯源周期较长 |
  | 审核中心 | 365 天（1 年） | 审核事项量小，审计回溯最长 |
  | 患者查询（建档） | 366 天（1 年） | 患者建档时间回溯 |
  | 运营分析 | 366 天（1 年） | 趋势图可读性（原有口径） |
- **datePicker 跨度联动校验**（v1.0.0 → v1.1.0）：新增 `peer`（对端输入框）与 `maxSpan`（跨度上限天）选项——选定值与对端值跨度超限时自动调整到上限边界并 toast 提示，与后端钳制同口径、前后端双重防护；全部日期范围 UI（危急值/影像引用/运营分析/打印中心/审核中心/患者查询）均已接入。

### 变更
- **运营分析日期范围统一走 `date_span_clamp('ana')`**：`ana_range()` 的 366 天防御性上限改为调用统一钳制函数（单一数据源），行为不变。
...
### 修复
- **叫号管理未选科室空态提示居中**：空态提示（请先选择科室）由列表区顶部改为垂直水平居中显示。

### 文档
- **同步版本号至 v8.17.51**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **打印中心 / 查询中心危急值外框底部与科室管理等列表统一**：打印中心左右栏（`.pc-left`/`.pc-right`）与危急值卡片覆盖全局 `.card` 的 `margin-bottom:16px`（此前导致卡片下边缘比标准列表高 16px），现在左右栏与危急值列表卡撑满视口剩余高度，下边缘与科室/用户/检验等列表卡片底部对齐。

### 文档
- **同步版本号至 v8.17.50**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **处方套餐只读预览补全控件**：剂量按钮显示含单位文本（如「250ml」「8万U」）；新增护士勾选框（disabled 且按 `nurse_required` 勾选）；子医嘱保留成组医嘱行（药品名称 + 规格 + 剂量 + 数量控件，全部 disabled），隐藏 -/+✕；修复子医嘱保存时 `item_name`/`item_id` 为空（对象字段为 `name`/`id`，保存扁平化须回退），并回填历史套餐子医嘱名称。

### 文档
- **同步版本号至 v8.17.49**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **打印中心底部留白与其它列表统一**：左栏记录最后一项与卡片底边距统一为 18px（`.pc-item:last-child` margin 控制，`pc-list` 底部 padding 归零），不再比其它列表高出几个像素。

### 文档
- **同步版本号至 v8.17.48**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **查询中心危急值空态去表格感**：空态单元格去掉表格下边框线、去掉内联样式（改由 CSS 控制），内容居中、padding 撑开，不再有「固定表格内」的观感；危急值 tab 外框固定布局（筛选卡固定、列表卡内部滚动）确认生效。

### 文档
- **同步版本号至 v8.17.47**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **新增 SmartPoller 弹性轮询管理器**（`smart_poller.js`，全站加载）：推流健康时低频对齐（默认 60s）、断开时自动升级应急高频（默认 8s），与 push.js 状态广播联动；Page Visibility 集成（后台挂起零空耗、切回立即静默同步）；并发抑制（上一次未返回跳过调度）。

### 文档
- **同步版本号至 v8.17.42**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **场景带参调用 + 指定医生工号严格校验**：`bin/seed.php` 支持 `--scene="doctor=2001"`（解析为指定医生接诊专项）与 `--scene="dept=lab"`（医技精细模式）；`doctor2001_seed.php` 增加工号参数与角色校验——工号不存在或关联角色非医生时输出 `[Preflight Error]` 并安全退出，拒绝写入脏数据。

### 文档
- **同步版本号至 v8.17.36**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **测试造数体系增量重构（第一步）**：
  - 新增 `PreflightChecker`（数据库先验探测）：诊断库（ICD-10）/检查/检验/药品库存/处置项目缺失时输出 `[Preflight Error]` 并终止，杜绝写入脏数据；
  - 新增 `ClinicInfoSeeder`（`--module=clinic`）：写入医院名称/必填机构代码/第二名称/机构简介；
  - 新增 `ScreenSeeder`（`--module=screen`）：按数据库科室分类动态生成叫号大屏（临床诊室/检验采血窗/影像按检查分类生成检查室/药房发药窗/护士站输液室），严禁写死；
  - `bin/seed.php` 模块分发支持逗号/空格多选。

### 文档
- **同步版本号至 v8.17.35**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **系统设置补全机构信息**：医院信息卡新增【医疗机构代码】（必填，附说明「医保结算、监管报送与接口对接唯一标识」）与【机构简介】（选填多行文本域）；保存 API 后端校验机构代码非空并持久化 `org_code`/`hospital_intro`。

### 文档
- **同步版本号至 v8.17.34**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **首次安装增加「机构代码」必填项**：机构代码（医疗机构代码）为医保结算/监管报送/接口对接唯一标识，安装表单在医院名称同级展示并必填；安装接口后端校验非空并持久化 `org_code`。系统设置存储机制（`settings` 键值表）天然支持 `hospital_name`/`org_code`/`hospital_sub_name`/`hospital_intro` 等机构字段。

### 文档
- **同步版本号至 v8.17.33**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **运营分析科室统计/自定义统计列表显示不全**：图表卡内容较高时列表卡被 flex 压缩到极小（自定义统计列表仅 80px 左右）。现列表卡保底高度 `min-height: 320px`（内部滚动），pane 高度改为由内容撑开 + 最小占满剩余，页面内容多时 content 可滚动查看，列表不再被裁剪。

### 文档
- **同步版本号至 v8.17.32**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **查询中心「更多子项」占位卡同步外框固定布局**：占位卡占满剩余高度、内容 flex 居中，与危急值/影像引用 tab 底部对齐一致。

### 文档
- **同步版本号至 v8.17.31**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **查询中心【暂无危急值记录】空态优化**：空态行居中显示（`text-align:center`），并去除鼠标悬停的背景变色（空态无点击效果）。

### 文档
- **同步版本号至 v8.17.30**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **规格每包装数量为 1 时禁止拆零零售**：药品表单中【允许拆零零售】选项在规格 ×1 时隐藏并禁用（数量为 1 无法拆零），规格改大后恢复显示；后端 `drug_save` 对 ×1 药品强制 `allow_split=0`（无论前端提交值），杜绝拆零。

### 文档
- **同步版本号至 v8.17.29**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **药品设置页界面优化对齐药品信息页**：新增右上角【＋ 新增XX】按钮（跟随子 tab 动态变化，如切到包装单位显示「＋ 新增包装单位」）；列表上方新增快速搜索栏（服务端 `drugsetting_list` 支持 kw 过滤）；子 tab 移入搜索栏右侧；页面统一固定高度布局。

### 文档
- **同步版本号至 v8.17.28**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **病历模板只读预览体格检查等空字段下划线消失**：`.ef-field.readonly` 将下划线设为透明导致空字段纯空白；现模态框内只读字段保留灰色虚线下划线（`--border`），且 hover 不触发蓝色高亮；文字颜色仍保持蓝色。

### 文档
- **同步版本号至 v8.17.27**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **套餐失效项【更换】按钮消失**：套餐内项目因原始数据变更失效（`valid=0`）时，此前的渲染条件把失效项的【更换】按钮也一并隐藏——这与添加更换按钮的初衷相悖（失效项正是最需要更换为可用项目的场景）。现失效项保留【更换】按钮（非处方条目 + 处方条目 `drugReplaceBtn` 均放开），失效项的剂量/频次/途径控件仍禁用，可点击【更换】搜索相近可用项目替换。

### 文档
- **同步版本号至 v8.17.26**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **处方套餐只读预览改回输入框/下拉样式（disabled）**：剂量按钮、频次/途径下拉保持与编辑一致的样式但全部 `disabled`（此前改为静态文本，样式割裂）；子医嘱静态展示；隐藏数量 -+/护士开关/更换/✕/搜索添加框。

### 文档
- **同步版本号至 v8.17.25**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **统一各列表底部留白高度**：所有固定高度列表（科室/用户/检验/检查/药品/处置/模板/套餐/审核/药品设置等 list-card 列表）统一最后一行距列表外框下边框 18px；查询中心影像引用（`.qc-ref-list`）、打印中心左栏（`.pc-list`）底部留白统一为 18px，避免切换页面时列表外框底部忽高忽低。

### 文档
- **同步版本号至 v8.17.23**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **运营分析各 tab 列表统一固定高度布局 + 转归查询分页**：科室统计/医生统计/自定义统计/转归查询 pane 固定视口高度，列表卡占满剩余并内部滚动（表头 sticky、底部对齐一致）；转归查询由全量 200 条改为服务端分页（page/size/kw/type 过滤），滚动加载避免一次性返回过大；其余聚合列表（科室/医生/自定义）数据量小保持聚合渲染。

### 文档
- **同步版本号至 v8.17.22**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **叫号管理 / 诊断管理列表统一固定高度布局**：叫号管理列表套用 `.list-layout`（科室选择卡固定、大屏列表在 `.list-scroll` 内滚动、表头 sticky 吸顶）；诊断管理左右栏（ICD10 分类树 + 详情）由旧式 `70vh` 定高改为占满剩余高度、内部滚动，底部对齐与其它列表一致。

### 文档
- **同步版本号至 v8.17.21**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **打印中心列表统一固定高度布局**：左右两栏套用 `.list-layout`（页头/检索条固定），左右栏占满剩余高度、内部滚动，底部对齐与其它列表一致（覆盖旧式 `calc(100vh-300px)` 定高）。

### 文档
- **同步版本号至 v8.17.20**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **查询中心各 tab 列表统一固定高度布局**：危急值查询 / 影像引用查询 tab 内容区统一占满剩余高度、内部滚动（底部对齐一致，切换不忽高忽低）；危急值筛选卡固定、列表卡 `.table-wrap` 内部滚动 + 表头 sticky；影像引用卡片改占满剩余高度（覆盖旧式 `calc(100vh-320px)` 定高）。

### 文档
- **同步版本号至 v8.17.19**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **药品信息列表接入固定高度布局**：药品信息页（已分页无限滚动）套用统一 `.list-layout`（外框固定、顶部固定、仅列表内部滚动、表头 sticky 吸顶、底部留白），与科室/用户/检验/检查/处置列表布局一致；列表容器补 `.table-wrap` 内部滚动。

### 文档
- **同步版本号至 v8.17.14**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **病历模板只读预览文字颜色由蓝变黑**：模板预览（模态框内）走 emreditor 只读态，`.ef-field` 加 `.readonly` 后 CSS 将其颜色改为灰黑，导致模板中本应蓝色的可编辑字段全部变黑。现限定「模态框内只读预览」保留字段蓝色（`--primary`），下拉字段（`.ef-select:disabled`）同样保留蓝色；病历页诊毕只读的灰字样式不受影响。

### 文档
- **同步版本号至 v8.17.13**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **套餐只读预览残留可操作元素**：预览套餐时底部【更换】【✕】按钮、数量/护士开关仍显示，且处方套餐的频次/途径下拉在字典就绪后重渲染恢复为可点击。现 `pkgBuildForm` 记录只读标记 `PKG_READONLY`，`pkgRenderItems` 只读时一律不渲染操作按钮（更换/✕/数量/护士开关/子医嘱控件），频次/途径/剂量改为静态文本展示，子医嘱以只读列表展示；同时隐藏套餐搜索添加框。

### 文档
- **同步版本号至 v8.17.12**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **偶发「网络请求失败，请检查网络连接」打扰用户**：根因是 `ajax.js` 对任何请求失败无条件弹 toast，而后台轮询（站内消息 15s、诊室心跳、候诊队列 30s、叫号面板 10s 等）偶发网络波动/服务器瞬断时也会触发弹窗。现 `Clinic.ajax` 新增 `silent` 选项：后台轮询/心跳请求失败时仅 `console.warn` 记录（含失败 URL，便于排查），不再打断用户；用户主动操作（表单提交、查询等）的失败提示保留。

### 变更
- **后台轮询全部静默化**：notify（未读消息）、room_heartbeat（诊室心跳）、call（科室叫号）、deptwork（候诊队列 + 叫号面板）、doctor_tools（叫号面板）、queuepanel（候诊列表）等轮询请求统一加 `silent:true`。

### 文档
- **同步版本号至 v8.17.11**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **无限滚动列表表头（thead）随分页加载重复堆叠**：服务端每页都返回 thead，此前追加逻辑在每页加载时都会把 thead 重新插到 table 开头，滚动多页后回到顶部时所有表头堆叠占满顶部空间；现追加前检测 table 是否已有 thead，仅首屏插入一次。

### 文档
- **同步版本号至 v8.17.10**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **审核中心/管理页预览已删除模板或套餐时闪退**：此前 `Clinic.modal.load` 收到「不存在」错误响应后 900ms 自动关闭模态框，导致「显示不存在然后消失」；现 `previewTpl`/`previewPkg`（及护士/影像模板预览）改为 `Clinic.get` 手动加载——模板/套餐不存在时 toast 明确提示且不弹模态框，存在时正常打开只读预览；管理员审核预览跳转前先校验存在性。

### 文档
- **同步版本号至 v8.17.9**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **模板/套餐只读预览统一复用编辑模态框**：非本人创建的模板/套餐（他人条目）预览不再使用独立的文本预览模态框，改为直接调用「添加/编辑模板/套餐」同一个模态框（`buildTplForm`/`pkgBuildForm`），打开后整框强制只读（emrEditor `readonly` + `modalReadonly` 禁用全部控件/拦截交互/禁止复制），前后端一致；预览按钮去掉 👁 图标、去掉「他人模板/他人套餐」字样，仅保留「预览」文字。
- **管理员审核预览统一**：审核中心的模板/套餐预览改为跳转到对应管理页（`?preview=ID` 自动打开只读预览模态框），复用同一套编辑模态框与只读限制；管理员可查看含待审核在内的全部内容（后端 get 权限已覆盖）。
- **删除 ui.js 自建 `previewTemplate`/`previewPackage`**（样式差、代码冗余），统一由模板/套餐管理页的 `previewTpl`/`previewPkg`（复用编辑模态框）替代；保留通用只读化 `Clinic.modalReadonly`。

### 文档
- **同步版本号至 v8.17.8**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **套餐添加药品切换单位金额不正确（根因）**：套餐应用 `pkgToOrderItem` 生成的开单条目缺失 `spec_pack_qty`（每包装数量），`applyUnit` 拆零单价误按 pack_size=1 计算（套餐添加庆大切支显示 ¥18 而非 ¥1.8）；已补全 `spec_pack_qty`/`spec_dose_unit`/`single_use_qty`，套餐添加与手动搜索添加的药品结构化字段完全一致，切换单位/数量/金额行为 100% 相同（套餐仅作为输入底层药品 ID 的便捷方式，不产生独立计算逻辑）。无头浏览器实测套餐添加庆大金额 ¥1.80、右侧总金额随单位联动。
- **套餐失效校验修正**：按规则「药品价格改变不影响套餐有效性，仅药品名/厂商/规格变动判失效」——补齐规格字段（`spec_dose_unit` 剂量单位、`spec_pack_qty` 每包装数量、`spec_pack_unit` 最小单位），移除 `allow_split`（拆零开关，套餐应用自适应当前拆零状态）；频次/途径为医生的用法（套餐内医生可自定义），不参与失效比对；价格始终不参与比对（价格变动不判失效）。
- **开处方切换销售单位按单次剂量重算数量，行内与右侧总金额实时联动**：切换单位时数量按「覆盖单次剂量」重算（庆大 1 支 ↔ 1 盒），行内金额简化显示「¥金额」（1 支 ¥1.80 / 1 盒 ¥18.00），右侧边栏【开单总金额】随单位切换实时变化（1.80 ↔ 18.00）。
- **根因补充（SW 资源陈旧）**：仅 amend 代码而未递增版本号时，PWA Service Worker「缓存优先 + 后台刷新」持续返回旧 JS；本次递增版本号（`?v=` 变化强制资源失效）并将 SW 缓存名升级（`clinic-opd-v2`，activate 自动清理旧缓存），彻底根治资源陈旧。

### 文档
- **同步版本号至 v8.17.7**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 修复
- **开处方搜索下拉不出现药品列表（回归，根因定位）**：v8.17.3 引入 `Clinic.ellipsis` 时误删 `rxItemHtml` 的 `return '<div class="rx-drop-item"...'` 语句，导致下拉条目渲染返回 `undefined`、列表恒为空。已恢复 return 并通过无头浏览器实测：聚焦搜索框下拉正常加载 15 条药品。
- **处方数量自动计算（无头浏览器实测）**：健胃消食片 0.8g×32片 开 2.4g → 自动数量 **1 盒**（此前 3 盒）；切换单位/设剂量后数量均正确。
- **开单总费用与切换单位单价实时联动（无头浏览器实测）**：庆大 8万U×10支 默认支 → 单价 ¥1.8 / 总 ¥1.8；切盒 → 单价 ¥18 / 总 ¥18；切回支 → ¥1.8；数量 +1 支 → 总 ¥3.6，实时刷新。
- **数量步进器布局**：已确认为「− 数量 ＋ 单位」（单位在加号后）。
- **模板/套餐只读预览中文渲染**：模板预览字段映射为中文（主诉/现病史/既往史/医嘱建议等），不再显示 `chief_complaint` 等原始英文键。

### 文档
- **同步版本号至 v8.17.4**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 新增
- **种子数据全面升级（tools/seed_test_data.php）**：
  - **全量 104 种药品库存初始化**：按包装单位随机 50~100 盒/瓶，底层以最小单位物理数量入库（盒数 × pack_size）；警戒库存按包装单位随机 5~10 盒/瓶同步换算最小单位绝对阈值（warn_qty）。
  - **护士执行智能规则**：按给药途径/剂型/名称特征自动设定 `is_nurse`——静脉滴注/静脉输液/静脉注射/肌注/皮下/皮内/直肠/雾化吸入 → 1（护士站执行），口服/外用自用/滴眼/舌下 → 0，全部 104 种药品均有明确 0/1。
  - **通用名与剂型补全**：按名称特征派生 `generic_name`（品牌药显式映射，如 感康→复方氨酚烷胺片）与 `form`（注射液/胶囊/片剂/口服液/滴眼剂/吸入剂等）；包装单位按规格推导（×1瓶→瓶，其余→盒，修复全部硬编码「盒」问题）。
  - **液体药品单次用量修正**：`single_use_qty` 按规格单位折算（100ml/瓶 口服液单次 10ml → 0.1 瓶），修复剂量展示与开方自动数量错误（此前显示 1000ml）。
  - **9 组全院公共套餐模板（管理员身份发布，scope=hospital）**：感冒口服/门诊输液/血常规感染筛查/肝肾功能生化/胸部DR/腹部彩超/心电图/清创缝合/雾化吸入；处方套餐明细完整固化 `unit_type`（pack/min）+ `sale_unit` + `price`/`pack_price` + 结构化单次剂量，医生一键导入即触发 v8.17 动态库存与剂量校验。
  - **幂等重生成**：重新运行前先清空旧业务链数据（患者/就诊/病历/开单/缴费/报告/影像引用等 14 张表），字典类数据按唯一键幂等 upsert；修复 `registered_at` 跨午夜导致同日同科室 `visit_seq` 唯一索引冲突；控制台输出清晰导入统计、退出码 0、无任何 PHP Notice/Warning。

### 修复
- **药房药品/设置只读（种子生成问题根因）**：`/admin/drugs`、`/admin/drugsettings` 本已对药房开放路由，但后端 `drug_list`/`drugsetting_list` 编辑按钮仅限管理员、且 `drug_form` 不在药房开放动作列表——新增 `drug_can_manage()`（管理员+药房）能力判定：药房可编辑药品信息/库存/警戒库存与药品设置（提交走审核流），删除按钮仍仅管理员。
- **药品模态框库存单位切换二次打开失效**：`bindSplitBox`/`bindQtyUnit` 此前用模块级标志位防重绑，模态框关闭再打开后新 DOM 未重新绑定监听导致点击无反应——改为元素级 `data-bound` 标记（每次打开重建 DOM 即重新绑定），`__qtyDirty` 每次打开重置。
- **警戒库存单位显示不准确**：种子硬编码所有药品 `package_unit='盒'`，现按规格推导（×1瓶→瓶），表单「警戒库存（瓶）」、库存单位按钮、药品/库存列表展示随实际包装单位联动。

### 文档
- **同步版本号至 v8.17.2**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 新增
- **警戒库存配置（3.4）**：药品添加/编辑表单新增【警戒库存】输入，单位强制为包装单位（盒/瓶）；后端存库自动换算为「最小单位」绝对警戒阈值（输入盒数 × pack_size，`drugs.warn_qty` v39 迁移）。低库存报表/报警判定统一以最小单位绝对值对比（未配置时回退「10 包装」启发式），杜绝大包装药品（如 100 粒/瓶）因单位混淆导致断货前夕才预警。管理员药品列表新增「警戒」列，药房库存列表展示「警戒 ≤ X 盒」。
- **库存单位动态可视化联动（3.5）**：
  - 开处方/套餐搜索候选项：不可拆零药品库存展示「X 盒」（包装单位）；允许拆零药品默认展示「X 支（Y盒Z支）」（最小单位 + 整包装折算）。
  - 已选药品/套餐明细：库存显示与【单位下拉框】双向绑定——切最小单位刷新为「X 支（Y盒Z支）」，切包装单位刷新为「X 盒（余Z支）」；输入/提交按当前单位折算回最小物理数量与真实库存比对，超库存即时阻断。
- **药品表单库存录入单位切换（3.6.1）**：库存输入框右侧单位按钮（默认包装单位，点击可切换最小单位，来源于已填数据实时动态显示）；保存按当前单位换算为最小单位绝对值落库（未编辑保留真实余量）。药房出入库弹窗同样支持单位切换——入库默认包装单位、出库默认按是否拆零（允许拆零→最小单位 / 否则包装单位），后端按单位折算最小单位入库/出库。

### 修复
- **拆零参数面板单位显示重复（3.6.2）**：包装售价/拆零单价金额与单位分元素展示，修复「¥16.00 / 盒 / 盒」「¥0.6667 / 粒 / 粒」单位重复；拆零单价保留 4 位小数（0.6667）与服务端初始渲染一致。

### 文档
- **同步版本号至 v8.17.1**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 新增
- **拆零销售机制（注射剂按支卖）**：药品表新增 `allow_split` 允许拆零开关（默认关闭）；开启后开方可选【最小单位】（支/粒/片）销售，系统自动按「包装单价 ÷ 每包装数量」核算拆零单价；关闭时仅按整包装（盒/瓶）销售，前端下拉仅展示包装单位且不可切换，后端接口直传最小单位一律拦截「该药品不支持拆零销售，请按整包装（盒/瓶）开立！」。
- **规格参数结构化落地**：`spec_dose`=单最小单位规格量（如 0.3）、`spec_dose_unit`=规格单位（g/mg/万U/ml）、`spec_pack_qty`=每包装最小单位数（pack_size）、`spec_pack_unit`=最小单位（支/粒/片）、`package_unit`=包装单位（盒/瓶/板）；单包装总规格量 = pack_size × 单剂量值（0.3g×24粒 = 7.2g/盒）。`tools/refill_drug_spec.php` 注释同步字段映射。
- **管理员药品表单【允许拆零零售】开关**：开启后必填 包装单位/最小单位/每包装数量(>1)/单剂量值（前端即时校验 + 后端 `drug_save` 硬校验），表单实时换算展示「包装售价 / 拆零单价」；管理员/药房药品列表对拆零药品显示「拆零」徽标。
- **库存统一为最小单位口径**：v38 迁移将存量 `drugs.qty` 折算为最小单位（整盒数量 × pack_size）；整盒售出扣减 数量×pack_size、拆零按实际支/粒数（系数 1）扣减，药房审方驳回 / 收费退费按同一口径恢复（历史处方明细 `pack_size` 回填当前规格快照）。药房库存列表同时展示「最小单位 + 折算整包装」数量。

### 变更
- **开处方数量自动计算重构**：医生输入单次剂量后——
  - 按【包装单位】销售：1 盒容量（PackCap=pack_size×单剂量值）足以覆盖单次剂量时自动推荐 **1 盒**（0.3g×24粒开 0.6g → 1 盒，不再错误变 2 盒）；剂量超单盒容量才按 ⌈剂量÷PackCap⌉ 上调盒数。
  - 按【最小单位】销售（允许拆零）：数量 = ⌈单次剂量 ÷ 单剂量值⌉（8万U/支开 10万U → 2 支）。
  - 数量框后方新增【销售单位下拉】（必选，简洁无搜索/清除）：允许拆零药品可选 支/盒（默认最小单位），切换即时刷新 单价/销售单位/自动重算数量；不支持拆零药品下拉仅包装单位且禁用。
- **单次剂量覆盖性拦截（前后端双重）**：开药总量 = 数量 × 单位容量必须 ≥ 单次剂量，否则前端标红提示、后端返回「开立数量无法满足单次剂量要求（当前 1 盒仅 0.2g，单次需 0.3g），至少需要 2 盒！」式精确拦截（防开空/开漏）。后端 `order_submit` 以服务端药品数据权威核价（拆零单价=包装价÷pack_size，保留 4 位小数防除不尽）。
- **处方套餐（模板）联动**：套餐保存固化 `unit_type`（pack/min）+ `pack_price`（包装单价）+ 数量 + 销售单位文本；套餐导入开处方时按开立单位触发自动数量换算与剂量合法性校验，不覆盖成错误盒数；药品信息变更（含拆零开关/包装单位）判为套餐失效。
- **单据打印单位一致**：处方笺/取药凭条/缴费凭条/退费审批页的数量统一带开立单位（×2 支 / ×1 盒），单价为开立销售单位单价，杜绝「单价 40 元、数量 2 支、总价 8 元」单位错位。
- **金额舍入统一**：拆零单价除不尽场景（100 元/盒 30 粒 = 3.3333 元/粒）在各处核算按「行小计 round(单价×数量, 2)」金融四舍五入，订单总额 = 各明细四舍五入之和，收银对账一致。

### 修复
- **药房发药单位错发风险**：发药清单/处方笺明确「2 盒 / 2 支」单位展示，防止把 2 支发成 2 盒造成药损。
- **历史库存退费/驳回恢复口径错乱**：旧处方（迁移前）按整盒扣减，恢复按回填 pack_size 折算，避免退费少退库存。

### 文档
- **同步版本号至 v8.17.0**（README 徽章 + `bootstrap.php APP_VERSION` + `package.json`）。
...
### 变更
- **处方更换按钮并入子医嘱行靠右对齐**：添加/编辑处方套餐、开处方已选列表中，更换按钮不再单独占一行，而是与「＋子医嘱」按钮同一行、`margin-left:auto` 靠右对齐，节省纵向空间。

### 文档
- **同步版本号至 v8.16.21**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 修复
- **开处方搜索栏缺少分类子tab**：`RX_TAB_GET` 为模块级变量，开处方重开后 guard 跳过导致新输入框无 tab；`initRxList` 开头重置。
- **切换子tab时列表消失**：点击 tab 使输入框失焦触发 blur 关闭下拉；tab 栏加 `mousedown preventDefault` 保持输入框聚焦，列表随 tab 动态切换并保持打开（无头 Chrome 实测）。
- **模板/套餐名称重复**：后端 save 同类型下禁止同名（此前可创建完全同名模板/套餐）。

### 变更
- **组合徽章换行优化**：套餐编辑器项目名称+「组合」徽章用 inline-flex nowrap 组合，不再被挤到名称下方。

### 文档
- **同步版本号至 v8.16.20**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 修复
- **保存套餐后组合检验丢失（真正根因）**：`pkgSave` 扁平化提交主药时未发送 `is_group/members/member_ids`（后端白名单虽有这些字段但前端从未上传），导致保存后组合身份丢失（`is_group=0`）、组合徽标/成员标签消失。修复后保存流程保留组合字段。
- 无头 Chrome 全流程实测：添加组合（徽标+5成员）→ 保存（套餐已保存）→ 编辑（徽标+5成员完整保留）。

### 文档
- **同步版本号至 v8.16.19**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 修复
- **组合检验显示缺失（按 ID 权威解析）**：套餐内组合检验显示改为以底层项目 ID 为准解析成员，不依赖快照字段：
  - 后端 `package catalog`/`get` 返回 `lab_map`（组合→成员ID→名称）；
  - 套餐编辑器按组合 ID 从 `lab_map` 解析成员标签，保存后编辑回填不再缺失；
  - 套餐应用弹窗组合显示「组合」徽标 + 成员标签（按 `GROUP_MEMBERS`/`ID_NAMES` 解析）；
  - 加入开检验已选列表后组合样式完整（沿用 `GROUP_MEMBERS`）。
- **频次/途径下拉移除 X 清除按钮**：必选字段不再显示清除按钮（保留搜索）。

### 文档
- **同步版本号至 v8.16.18**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 修复
- **套餐编辑器频次/途径不显示下拉（真正根因）**：共享 `drugControls` 读取的是 order.js 闭包内的 `RX_FREQS/RX_ROUTES`，而套餐编辑器把字典写入了自己的全局变量（死变量）；新增 `Clinic.order.setRxDicts` 将字典写入 order.js 闭包，套餐编辑器加载目录首页后调用，频次/途径恢复下拉。
- **处方套餐药品不显示规格**：套餐编辑器头部规格改为所有类型显示（不再仅非药品显示）。

### 变更
- **套餐应用库存不足提示**：使用套餐时，处方药品数量超出库存仍正常加入已选列表，并 toast 提示「xx库存不足，已加入但请在提交前调整数量」；提交处方时已有的强制数量校验保持不变。套餐编辑器内数量不受库存限制（上限 99），搜索列表仍排除库存为 0 的药品。
- **更换药品后数量默认 1**：更换后数量不再沿用旧值，默认 1；结构化剂量按单份用量自动调整数量（1 份不够则向上取整）。

### 文档
- **同步版本号至 v8.16.17**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 修复
- **更换项目提示串台**：通用项目选择器面板复用，条目点击委托闭包 `opts` 停留在首次调用，切换检验/处方后提示出现旧条目名+新项目名（如「促甲状腺激素(TSH)→感康片」）；改为读取面板最新回调 `panel.__onPick`。
- **更换后列表只显示金额、名称丢失**：套餐编辑器（`pkg` 上下文）期望 `item_id/item_name` 结构，原更换用 `pkgToOrderItem` 生成 `id/name` 结构；新增 `pkgFromPick` 按上下文生成正确结构。
- **套餐编辑器添加药品后频次/途径不显示下拉**：`pkgInitCatList` 加载 `link_dicts`（频次/途径选项）后未重渲染已选列表，补 `pkgRenderItems()` 使下拉即时可用。

### 文档
- **同步版本号至 v8.16.16**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 新增
- **通用项目选择器（子医嘱 / 更换 共用）**：新增 `openItemPicker`（body fixed 悬浮窗 + 搜索 + 无限滚动），条目渲染按类型区分（处方药品/检验/检查/处置）；子医嘱下拉重构为调用该通用选择器。
- **已选项目快速更换**：已选列表每个项目新增【更换】按钮 → 打开通用选择器（数据跟随当前开单/套餐类型）→ 确认「确定要将xxx更换为xxx？」后原位替换。
  - 开处方已选列表 / 处方套餐编辑器：处方更换按钮靠右显示在头部下方；检验/检查/处置更换按钮在删除按钮左侧。
  - 套餐编辑器更换走 `/api/package` catalog（兼容管理员）；同单去重拦截（新项已存在则阻止）。
- **编辑套餐失效项禁用**：编辑套餐弹窗中，失效项目（改名/删除/未审核/缺货/信息变更）名称删除线+灰字+「已失效」徽标，下方剂量/频次/途径/护士/数量/子医嘱全部禁用；后端 `save` 拦截（含任一失效主项则拒绝保存，提示先删除或更换）。

### 修复
- **套餐应用弹窗全选/确认按钮报错**：`pkgApplyToggleAll`/`pkgApplyConfirm` 未导出导致 `is not a function`；已导出并统一按钮样式（padding 8px 16px）。
- **套餐失效校验误判**：校验比较字段 `company_short`（套餐存储）对应 `drugs.vendor_short` 加列别名；频次/途径为医生自定义项不作为药品身份变更判据；`single_dose` 为配置剂量显示（如 0.3g）与药品默认用量文本（如 1粒）概念不同不参与比对。

### 变更
- **管理员可编辑他人个人模板/套餐**：管理员新建仍限全院/科室；编辑他人已存在的个人模板/套餐可保存，并站内信告知原作者「xxx修改了您的模板/套餐，请及时查看」；若适用范围由个人变更为科室/全院，消息中明确提示。

### 文档
- **同步版本号至 v8.16.15**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 新增
- **处方/套餐下拉改为 body 覆盖层可滚动**：处方药品下拉（`rxDrop`）与套餐编辑器搜索下拉（`pkgCatDrop`）从模态框内绝对定位改为 `document.body` 上的 `fixed` 覆盖层，避开模态框 `transform` 造成的定位/滚动干扰，滚轮可正常滚动且滚动到底自动续加载。
- **模板选择器接入统一无限滚动**：首次新增病历的模板列表、知情同意书模板列表改用 `Clinic.infiniteList` 分页加载（后端 `template list` 支持可选 `page/size/scope`），搜索/范围徽章切换自动重置分页，滚动到底续加载，避免模板过多一次全量加载。
- **诊断搜索接入统一无限滚动**：ICD-10 诊断搜索结果列表改用 `Clinic.infiniteList`（后端 `icd10 search` 支持 `page/size/has_more`），移除手写 offset 分页逻辑；`infiniteList` 新增 `check()` 钩子（内容不满一屏自动续载）。
- **医生首页快捷入口新增套餐管理**。

### 修复
- **套餐调用报「套餐不存在」**：`package get` 原仅读 GET 参数，`openPkgApply` 走 POST 导致 `id` 恒为 0，改 `req()` 兼容 GET/POST。
- **编辑套餐列表为空**：`pkgBuildForm` 未渲染已保存项目；后端 `get` 返回的扁平 items 需按 `sub_of` 归组为 主药+子医嘱。
- **套餐项目失效校验**：后端 `get(for_apply)` 逐项对比当前目录（改名/删除/未审核/缺货/信息变更判失效）；前端失效项删除线+灰字+复选框禁用，全选/确认自动跳过；后端 `submit` 已有存在性+库存硬拦截。
- **开处方频次/途径下拉不显示**：`initRxList` 未加载目录首页 `link_dicts`（频次/途径选项），补上并重渲染已选列表。
- **新建套餐搜索下拉不显示**：body 覆盖层内联 `display:none` 覆盖 class 的 `display:block`，改为直接设 `style.display`。
- **套餐编辑弹窗高度随内容变化**：新增 `.modal.pkg-form-modal` 固定高度，内容区内部滚动（参考开单模态框模式）。

### 变更
- **处方套餐条目控件对齐开处方模态框**：新增【护士】勾选，子医嘱按钮移到途径右侧（对齐开处方模态框布局）。
- **模板悬浮窗排序**：后端 `sort=picker` 按 全院→科室→个人 分组、组内按创建时间升序（最新创建在最下方），待审核模板按个人分组；前端选择器传入 `sort=picker`。

### 文档
- **同步版本号至 v8.16.13**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 新增
- **快速开单套餐（快速开单套餐模块）**：医生可把常用的检验/检查/处置/处方项目预置成套餐，一键加入开单。
  - 数据表：新增 `packages`（套餐主表）与 `package_depts`（科室关联表）；接口 `/api/package` 提供 `list/get/save/delete/catalog` 五个动作。
  - 套餐管理页（`/admin/packages`、`/doctor/packages`）：四类套餐下拉切换、范围子 tab（全部/个人/全院/科室）、列表分页滚动加载；新建/编辑弹窗含适用范围/科室树、搜索栏分页动态加载项目目录（检验组合展开为单个检验项目；处方支持剂量/频次/途径/子医嘱编辑）。
  - 审核联动：个人套餐免审即用；科室/全院套餐提交审核中心；审核通过发布为对应范围、驳回自动降级为个人（个人依然可用）并通知提交者。
  - 开单弹窗：检验/检查/处置/处方搜索栏旁新增「🥡 套餐」按钮，弹出套餐列表（分页滚动加载）+ 项目勾选弹窗（处方组合医嘱按主药+子医嘱整体勾选树形展示、全选切换）；确认后按套餐结构一键加入已选列表，重复项目自动跳过并提示，需皮试药品逐项弹出皮试确认。

### 变更
- 套餐目录接口首页响应附带频次/途径联动字典，供处方套餐编辑下拉使用。

### 文档
- **同步版本号至 v8.16.12**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 新增
- **开单目录与药品下拉滚动分页加载**：医生开单弹窗的检验/检查/处置左侧项目目录改由通用无限滚动封装（`Clinic.infiniteList`）按页加载（每页 20 条），首屏仅加载 1 页，滚动到底部自动续加载；处方顶部药品下拉与子医嘱内联下拉同样改为分页加载（每页 15 条）。
- **目录接口服务端分页检索**：`/api/order?action=catalog` 新增 `page/size/kw/f` 参数，搜索关键字与检验筛选（单个/组合）下沉到服务端过滤；组合/成员互斥映射（`lab_map`）与联动字典仅随首页响应携带。
- **已选列表组合成员标签化展示**：组合检验项目所含成员改为灰色标签换行排列，不再挤占头部价格/操作区导致内容变形、重叠、溢出。

### 修复
- **开检验默认勾选【单个】tab 但列表仍混显组合项目**：目录列表默认按 `f=single` 服务端过滤，单个/组合两页签互斥单选，切换后自动重置分页重新检索。

### 文档
- **同步版本号至 v8.16.11**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 修复
- **续写病历右侧+按钮实时显示**：创建续写病历时，右侧诊断、检验、检查、处置、处方等+按钮现在能够实时显示，无需先保存病历后才显示。
- **保存后消除重复的「续写编辑中（未保存）」节点**：修复保存续写病历后，右侧病历节点错误显示「续写编辑中（未保存）」的问题。
...
### 优化
- **病历徽标色彩区分**：四种徽标使用不同颜色以便一目区分——`首诊病历` 使用新色 `badge-indigo`（靛蓝）；`病历续写` 保持 `badge-primary`（青色）；`会诊记录` 保持 `badge-warning`（琥珀色）；`危急值记录` 保持 `badge-danger`（红色）。新增 `--indigo` / `--indigo-soft` 主题变量及 `.badge-indigo` 样式类，适配明亮 / 夜间模式。
...
### 优化
- **病历灰底横条改为表格网格布局**：`prev-record-head` 由 flex 改为 CSS grid 表格布局（`grid-template-columns: auto 30% 45% auto`），徽标 / 科室 / 医生 / 记录时间 四列固定比例排列，不同医生姓名长短、科室名长短均能纵向对齐；列内内容添加溢出省略（ellipsis）防止换行。
...
### 优化
- **病历编辑器灰底横条文案精简**：去除 `记录医生：`、`记录时间：` 描述文字，横幅仅显示科室名称、医生姓名、职称、（工号）、记录时间，更加简洁清晰。
...
### 修复
- **病历编辑器灰底横条科室名称显示修复**：修复 API `record` 字段缺少 `dept_name` 导致当前可编辑病历横条不显示科室名称（`app/api/parts/record_read.php` `$recordData` 补充 `dept_name` 字段——取自 `patient_records.dept_id` 映射，无现有文书时回退就诊当前科室名）。该问题影响所有可编辑病历（首诊、续写、会诊），特别是最后一位医生的病历。
...
### 优化
- **病历编辑器灰底横条美化**：徽标 `首诊` 升级为 `首诊病历`；四种徽标（首诊病历 / 病历续写 / 会诊记录 / 危急值记录）统一靠左对齐，记录时间靠右对齐；横条左侧新增显示当时书写病历的科室名称（首诊、续写、会诊、危急值均显示当时书写科室）；医生信息显示顺序为姓名、职称、（工号 xxxx）。
- **未保存 / 切换病历科室名称显示修复**：补齐 `switchToRecord`、`restorePendingEditor`、`freezeUnsavedBeforeSwitch`、`cancelPendingRecord`、`addProgressEditor` 及 `emr_segments.js` 待渲染记录对象中的 `dept_name` 字段，避免最后一次病历续写、未保存编辑、会诊编辑等场景科室名称缺失。

### 文档
- **同步版本号至 v8.16.5**（README 徽章 + `bootstrap.php APP_VERSION`）。
...
### 修复
- **点击他人病历节点不再被未保存内容拦截**：`scrollToRecord` 他人文书分支（branch 2）重构——查看他人病历是只读操作，不再要求当前文书完善/保存，避免"无可编辑病历时误报『请完善当前病历』"（此前 B 医生打开仅有 A 医生首诊的患者、点击该首诊节点会被错误拦截且无法定位）。
- **未保存续写编辑器切换后自动变为只读且内容不丢失**：B 医生正书写未保存续写（`record_id=0`）时点击他人病历节点，内容冻结暂存到 `DATA.__pending_emr`，切换后以只读段按时间序展示（`emr_segments.js` 追加渲染），不再滞留可编辑编辑器；点击左侧「续写/首诊编辑中」节点或「病历节点 ＋」可原样恢复编辑（`restorePendingEditor`）。本人已保存文书有未保存修改时仍提示先保存（防误切换丢失修改）。
- **查看他人病历隐藏顶栏写操作按钮**：`renderEmrCard` 的 `otherDoctor` 分支隐藏保存/诊毕/转科按钮并给出"仅可查看（只读）"提示，避免在他人只读病历上误触保存；病历节点「＋」与知情同意书「＋」保留（创建本人病历入口）。
- **取消编辑同时清空冻结暂存**：`cancelPendingRecord` 删除未完成病历时同步清空 `__pending_emr`，防止恢复幽灵编辑器。
...
- **引用诊断标识仅在主动引用时显示**：`【引用】` 标签改由诊断数据的 `quoted` 字段驱动，仅从当前浏览文书读取——浏览他人病历时不再错误显示【引用】，仅当医生主动引用（添加）他人诊断到自己的病历中时才显示。后端 `record_save_diags.php` 同步保存 `quoted` 字段。
- **阅读他人病历时删除按钮不应显示**：`myDoctorId()` 改为从 `DATA.currentDoctorId`（固定登录医生）读取，不受 `switchToRecord` 切换文书影响；`renderLeftNav` 补充 `curIsMine` 判定，仅本人文书才显示删除按钮和 `inCurrent`；`pushDiag` 新增 `fromCurrentRecord` 参数，`quoted` 字段仅从当前浏览文书诊断读取。
- **续写病历提示语纠正**：续写占位提示「点击右侧『病历节点 ＋』开始书写续写病历」（原为「左侧」，与实际 UI 右侧边栏不符），与会诊提示语保持一致。
- **README 去除重复标题**：移除重复的 `## ✨ 功能特性` 标题。
...
### 新增
- **接口管理新增【清空密钥】按钮**：HIS 接口密钥输入框旁新增一键清空按钮（`clearHisKey`），清空后保存生效，并实时刷新接口地址 / curl 示例占位提示。

### 文档
- **README 功能特性重排**：由大表格改为模块化分组区块（挂号收费处/医生工作站/护士站/检验科/影像科/药房/医技打印/危急值/PWA/模板系统/开单与打印/叫号大屏/登录安全/管理员），核心功能要点化呈现，一目了然。
- **README 数据库描述更新**：SQLite/MySQL 双驱动描述统一为 SQLite/MySQL/PostgreSQL 三驱动（技术栈表、技术要点、schema 注释、切换章节补齐 PGSQL 常量与方言转换说明）。
- **README URL 混淆描述更新**：明确全站 AJAX 刷新后访问链接不再出现 visit_id 明文混淆串，混淆/防撞库机制仍后台全链路生效（接口解码校验、did() 解密、明文 ID 拒绝）。
- **landing.php 描述更新**：落地页数据库双驱动等过时描述改为三驱动。
...
### 新增
- **测试数据生成器全面升级**：`tools/seed_test_data.php` 重写为完整测试数据生成器（`~/.local/bin/frankenphp php-cli tools/seed_test_data.php`）：
  - **平台基础数据**：14 个账号（密码统一 123456，用户名=中文姓名全拼音，如张伟→zhangwei，按 emp_no upsert 保留密码/角色/科室/职称）；10 个科室固定 id 1-10（临床 1-6 + 医技 7-10）。
  - **检验目录 110 项 + 16 个组合项目**（肝功能十项/血常规二十项/肿瘤标志物五项等）；51 个数值型项目配置危急值上下限（如 WBC 2.0~30.0、钾 2.5~6.5），组合成员同步。
  - **检查项目 102 项**（CT 增强/CTA、MRI 各部位、DR、超声、内镜、病理等）；**处置项目 57 项**（含青霉素/头孢皮试、护士执行类 46 项）。
  - **常用药品 104 项**：规格结构化拆分（0.35g×24粒 → spec_dose=0.35/spec_dose_unit=g/spec_pack_qty=24/spec_pack_unit=粒/single_use_qty），支持中文单位（万U/贴/粒等）。
  - **模板体系**：病历模板 2（【开检查】【开药】）、知情同意书 6（手术/病重/清创/石膏固定须知/气管插管/门诊一般告知书，完整正文）、护理记录 4、嘱托 5、影像报告 4；全部 `is_system=0` 归属管理员可编辑。
- **平台患者数据**：近 15 天（今日起前推 14 天）每天随机分布，各状态配额 ≥25（待缴费 25 / 待就诊 30 / 就诊中 30 / 诊毕 88 含历史），另为新建患者生成 60-150 天前的历史就诊（58 位患者具备 2+ 次就诊便于历史调阅）。
- **完整就诊链**：严格遵循 挂号→缴费→结构化病历→开单（检验/检查/处置/处方）→执行（结果/报告/发药/皮试 5 条）→多人续写 30 条→会诊 10 条→诊毕转归 全链路；病历遵循 `emr_default_data` 结构化规范（体格检查存「其它体格检查」节、现病史时间不重复且不加句号），诊断取自 ICD-10 标准库真实编码。

### 变更
- **药品规格统一动态拼接**：新增公共函数 `drug_spec_text()`（app/core/helpers.d/string.php），药品编辑回显/药房列表/管理员列表/处方开单快照全部统一走拼接展示，修改 spec_dose/spec_pack_qty 后展示自动联动，杜绝逐处手写冗余。
- **医院第二名称**：默认由「门诊一体化信息系统」改为「淮海大学第一附属医院」。
- **检验项目汉化**：`粪便C.difficile毒素` → `粪便艰难梭菌毒素检测`。
- **修正 ICD-10 查询字段**：`code/name` → `diagnosis_code/diagnosis_name`（匹配新版标准库）。
...
### 变更
- **安装页数据库徽章独立居中**：徽章从标题行内移出，改为标题下方独立居中显示（text-align:center），不再挤占「🏥 门诊一体化系统」标题。
...
### 变更
- **安装页显示数据库类型徽章**：安装卡片标题旁新增与系统设置页一致的数据库驱动徽章（🗄️ 数据库：SQLITE / MYSQL / PGSQL）。
- **网站时区下拉改用内置可搜索样式**：安装页时区下拉改为 `data-csd-search` 可搜索下拉（无 X 按钮），与全站自定义下拉组件一致；authPage 独立页补加载 dropdown.js 以支持该组件。
- 清理误提交的临时数据库文件（data/db/clinic_main.db1）。
...
### 变更
- **首次安装页优化**：隐藏默认医院图标与品牌区（尚未配置医院信息，图标无意义），安装卡片居中呈现。
- **管理员用户名可自定义**：安装页新增「管理员用户名」输入框（默认填充 admin 可修改），后端校验英文字母开头/长度/唯一性，并写入用户表（不再写死 admin）。
- **移除安装页输入法提示文案**：删除「⚠️ 请输入英文字母/数字/符号…」提示，密码长度等校验错误统一通过 toast 提醒（含实际输入长度，便于发现输入法/自动填充导致的不完整）。
...
### 新增
- **系统设置页显示当前数据库类型**：页面标题右侧靠右对齐显示数据库驱动徽章（🗄️ 数据库：SQLITE / MYSQL / PGSQL），管理员可直观查看当前使用的数据库类型。
...
### 变更
- **科室/用户编辑模态框移除「状态」字段**：状态改由左下角启用/禁用按钮呈现后，表单内不再显示「状态」label，仅保留隐藏字段，界面更整洁。
- **系统管理员角色锁定**：编辑管理员账号时省略角色下拉（显示只读文本「系统管理员（角色不可修改）」），后端强制拦截——目标为管理员时角色强制 admin 且职称/关联科室/候诊天数/状态一律清空，普通账号禁止直接改选为管理员（防越权提升）。
...
### 变更
- **启用/禁用按钮样式统一**：切换按钮去掉 `btn-sm`（不再偏扁），与右侧取消/保存按钮同尺寸同风格。
- **状态徽章配色标准化**：可用=绿色、禁用=红色、待审核=黄色，其余状态=灰色（`item_status_badge` 四色规则）；检验/检查/处置/药品列表状态列统一接入，区分「待审核」与「已禁用」；科室列表（启用/停用）、用户列表（正常启用/安全锁定/已停用）状态本就精确，保持。
- **科室/用户管理同步改版**：新增/编辑科室、用户模态框的状态下拉改为左下角按钮切换（与项目表单一致：新增显示启用/禁用、编辑显示已启用/已禁用、点击切换 toast 提示「保存后生效」）；用户表单中管理员角色强制启用且按钮禁用（不可自停用，后端 `status=1` 权威兜底），`onRoleChange` 联动按钮禁用态，`unlockUser` 一键解锁同步走 `f_enabled`。
...
### 变更
- **启用/禁用按钮布局与文案优化**：模态框底部按钮区改为两端布局——【启用/禁用】切换按钮靠左独立显示，与右侧的取消/保存按钮分开；新增项目模态框显示【启用/禁用】（避免歧义），编辑项目模态框显示【已启用/已禁用】；点击切换时 toast 提示「该项目已禁用/已启用，保存后生效」，明确需保存后才真正生效。

---
## [8.14.0] - 2026-09-15

### 新增
- **单据打印快照（法律合规）**：新增 `print_snapshots` 表（v37 迁移）与快照读写函数（`snapshot_patient`/`snapshot_get`/`snapshot_apply_patient`），在挂号、缴费、开单、出具报告、开具诊断证明、病历诊毕 时固化患者资料与上下文快照——打印时优先使用快照，杜绝事后修改患者资料/检验检查字典导致历史单据显示变化。
  - 申请单/处方笺：开单时患者姓名/性别/出生日期 + 临床诊断快照（打印不再取"最新病历"诊断）；
  - 挂号凭条/缴费凭条：挂号/缴费时刻患者快照；
  - 报告单：患者资料 + 检验项目名/单位/参考范围/危急值/组内成员快照（字典改名/改范围不影响历史报告，打印与医生站报告详情模态框均优先快照）；
  - 诊断证明：开具时刻患者资料 + 科室名快照；
  - 电子病历：**诊毕前打印显示最新（医生可修正患者资料）；诊毕时固化快照，诊毕后打印冻结为诊毕时刻的患者资料与生命体征**。
...
### 变更
- **项目启用状态改按钮切换**：四类项目（检验/检查/处置/处方）表单的启用开关由复选框改为模态框底部按钮（绿色已启用/红色已禁用，点击切换）；禁用后医生开单列表不再显示该项目，已开单流程（缴费/执行/报告/发药）不受影响；删除提示引导先禁用项目。
...
### 变更
- **项目启用状态改按钮切换**：四类项目（检验/检查/处置/处方）表单的启用开关由复选框改为模态框底部按钮（绿色已启用/红色已禁用，点击切换状态）；禁用后医生工作站开单列表不再显示该项目，已开单流程（缴费/执行/报告/发药）不受影响；删除提示引导"请先禁用该项目，等相关流程完成后再尝试删除"。
...
### 变更
- **医技大屏未绑定显示整体轮廓**：去掉「等待医技人员绑定」提示，未绑定时照常渲染当前患者/排队队列整体框架，内部显示「当前暂无患者/当前暂无排队患者」占位——大屏始终面向患者展示（与医生诊室大屏空态一致）。
- **删除检验/检查/处方/处置项目的流程占用检查**：原「只要有开单记录就禁删」改为仅当存在**未完成流程**的开单明细时禁止删除（待缴费 open / 已缴费未执行 paid / 已登记 registered / 执行中 executing / 发药中 dispensing）；已完成（done/dispensed）或终态（refunded/cancelled/rejected）的历史开单不影响删除（过期作废可删）。提示精准化：如「该项目当前有 2 位未缴费、3 位待检验，请先完成相关流程后再删除」（按项目类型显示 待检验/待检查/待处置/待发药）。通用检查函数 `item_delete_check()` 供 检验/检查（item_delete）、处置（disposal_delete）、处方（drug_delete）三处复用。
...
### 修复
- **删除分类后孤儿测试项目无法删除**：分类删除的级联逻辑升级——该分类下**从未被开单（order_items 无引用）**的项目视为测试/占位项目，连同其测试结果记录（results）一并删除，不再残留「该项目已有结果记录」而无法清理；**已被开单**的真实历史项目保留并转未分类（不破坏就诊历史）。
...
### 修复
- **分类增改删保存后项目列表实时刷新无效（缓存问题）**：`APP_VERSION` 静态版本号长期停在 8.7.2，JS/CSS 均按 `?v=APP_VERSION` 缓存，浏览器一直加载旧版 admin_items.js（无 onChanged 逻辑）——已把 bootstrap APP_VERSION 与 README 徽章同步为 8.11.3，强制刷新全站静态资源缓存后 onChanged 即时生效。
...
### 变更
- **HIS 接口密钥禁止手动输入**：输入框改为 disabled 样式（灰置、不可点击输入），仅可点【🔑 生成密钥】随机生成后保存。
- **HIS 地址/curl 示例点击即复制**：移除右侧复制按钮，地址框与 curl 示例框改为禁用样式（灰底 + 手型光标 + 单击全选），点击即自动复制到剪贴板，hover 高亮边框。
...
### 变更
- **HIS 接口地址改为普通字段行**：去掉虚线卡片与🌐标题/说明描述，改为与上方同步模式/接口密钥同级的字段（label「HIS 接口地址」+ 灰底地址 code 框 + 复制按钮），更紧凑统一。
...
### 变更
- **HIS 接口地址样式统一**：地址框底色由白色改为灰底（`var(--bg-soft)`），与下方「调用示例（GET…）」的 curl 示例框一致，不再突兀。
- **非临床科室叫号大屏未绑定不显示患者**：医技大屏（lab/imaging/pharmacy/nurse）在未绑定（或绑定者心跳失效）时返回 `bound:false` 且不显示任何患者队列，显示「等待医技人员绑定大屏后开始叫号」提示——与医生诊室大屏规则一致，避免「无医技人员绑定却出现患者队列」的错乱。
- **检验/检查分类管理弹窗改版**：由徽章流改为小巧列表（每行分类名 + 重命名/删除按钮，容器内滚动）；重命名弹窗回填原名，保存后**同步更新该分类下全部检验/检查项目**的分类字段（后端 `cat_rename` 级联，按 lab/exam 分表）；删除需二次确认，删除后该分类下项目自动转为未分类（空值占位，不列入分类列表）；新增分类名查重。
...
### 新增
- **通用无限滚动工具 `Clinic.infiniteScroll`（infinite.js）**：把「滚动接近列表底部 → 自动加载下一页」收敛为统一方法，自动识别滚动容器（弹窗内固定高度列表 / 页面主内容区 .content / 整页 window），内容不满一屏时自动连续加载，支持 `stop()` / `check()` 手动判定。

### 修复
- **影像引用查询列表滚动加载失效**：此前用 IntersectionObserver 观察视口相交，未识别 .content 滚动容器导致滚动到底不加载——改为通用工具监听实际滚动容器；加载完成回调后主动 check() 保证内容不满一屏时也连续加载到底。

### 变更
- **医生搜索弹窗（critical.js）与号源池（doctor_tools.js）滚动加载统一改为通用方法**：逻辑与原有实现一致，消除三处重复的滚动判定代码。
...
### 变更
- **影像引用查询列表改为滚动动态加载**：用 IntersectionObserver 观察列表底部哨兵元素，向下滚动接近底部自动加载下一页（20 条/页递增），加载锁防重复请求，全部加载后显示「已加载全部引用」，底部状态提示随加载进度更新；列表卡片底部留白收紧。
...
### 修复
- **查询中心子 Tab 选中样式未切换**：qcTab 切换面板时未同步切换按钮选中态，危急值查询始终显示为选中——现按 data-qc-tab 属性切换 btn-primary/btn-outline。
- **影像引用类别显示直接以管理员分类为准**：此前 modality 显示做了关键词猜测简写（CT/MR/DR/US/OT），现改为直接显示登记时快照的管理员分类名（订单分类/检查项目分类），不做任何猜测映射——分类后续删除或改名不影响历史引用。
...
### 变更
- **影像引用查询合并入查询中心子 Tab**：功能迁移至「查询中心 → 影像引用查询」子 Tab（不再占左侧边栏菜单空间，移除侧边栏两处菜单项）；旧链接 /admin/imagerefs 保留并跳转至查询中心。
- **检查类别以管理员设置的分类为准（快照固定）**：影像引用登记时 modality 直接快照管理员设置的分类名（订单分类优先、检查项目分类次之、项目名兜底），页面显示归一化简写（CT/MR/DR/US/OT）；分类后续删除/修改不影响历史引用显示。
...
### 修复
- **影像引用查询只显示影像检查引用**：refs_list 查询加入 `o.order_type='imaging'` 过滤——此前影像引用台账混入检验（lab）订单的引用（26/46 条），导致页面出现空腹血糖、C反应蛋白等检验项目，与影像引用定位不符。
...
### 变更
- **接口说明独立成框**：「📖 接口说明（外部系统调用）」从 HIS 配置卡片内移出，改为下方独立的卡片（.itg-pane-docs），随 Tab 联动显隐。
- **左列新增分区标题**：HIS 左右两栏均带标题（左侧「⚙️ HIS 接口配置」/ 右侧「🧪 接口连通性测试」），分区更清晰。
- **左右两列以左列高度为基准**：新增 syncHisCols() 将右列 maxHeight 同步为左列高度（加载/Tab切换/保存/窗口缩放时对齐），右列连通性测试内容超高时内部滚动条呈现，不再往下挤压页面。
...
### 变更
- **HIS 接口地址改为保存后更新**：移除密钥输入的实时联动，接口地址与 curl 示例仅在点击【保存本组配置】后刷新（页面加载按已保存密钥渲染一次），地址卡文案同步更新。
- **接口说明横跨两栏**：HIS 分组下方的「📖 接口说明（外部系统调用）」由左侧移出，改为左右两列下方全宽展示（含五个 action 说明表与可复制 curl 示例）。
- **同步模式补充选择提示**：字段下方新增说明——手动同步为当前推荐（外部按需调用）、定时同步为按计划自动拉取推送（预留）、实时同步为事件驱动双向交互（预留）。
- **调用示例框可正常选中文本**：移除 `.itg-his-url` / `.itg-his-curl code` 的 `user-select: all`（此前单击即全选、误触输入会覆盖删除），恢复为普通文本选择。
...
### 变更
- **HIS 接口分组页面重构（左右两列）**：左侧 2/3 配置区（系统代码 / 接口密钥+一键生成 / 同步模式 / 自动生成地址卡 / 接口说明表 / 保存按钮），右侧 1/3 接口连通性测试面板；两列等高对齐，超出部分各自滚动。
- **HIS 接口地址实时化**：地址卡与 curl 调用示例跟随接口密钥输入**实时刷新**（无需保存即预览）；密钥为空时地址卡显示提示占位而非网址；复制按钮仅在有真实密钥时可用。
- **接口连通性测试增强**：测试结果区无数据时显示占位符（🧪 图标 + 提示语）；点击【保存本组配置】后自动刷新地址显示并清空测试结果；密钥已修改未保存时点击测试会先提示保存（HIS_SAVED_KEY 对比）。
- **页面内置接口说明表**：HIS 分组内直接展示 ping / patient_get / visit_list / visit_status / order_list 五个 action 的参数与用途说明，并附随密钥实时更新的可复制 curl 示例。
- **系统代码语义说明**：字段提示注明其由 HIS 侧分配、当前为预留字段（认证仅依赖密钥），并随 `ping` 自检返回回显（`system_code`），便于联调确认。
- README【🔌 HIS 预留接口】部分同步重写（自动生成地址 / 实时显示 / 测试 / 系统代码 / ping action）。
...
### 变更
- **HIS API 地址改为自动生成展示（无需填写）**：移除 his_api_url 输入框，改为页面底部自动生成的地址卡片（当前访问地址 + /api/his），展示完整调用地址并**附带接口密钥**、一键复制；未配置密钥时以占位提示。
- **HIS 接口连通性测试**：HIS 分组内置【▶ 开始测试】，实际请求本系统 /api/his 接口的 `ping` 自检动作，分别验证「X-HIS-Key 请求头」与「api_key GET 参数」两种认证方式并展示返回 JSON（成功/失败徽标）；后端 /api/his 恢复 GET 参数 api_key 认证兼容（推荐请求头方式，GET 方式密钥会进入访问日志，风险由管理员评估），新增 `ping` action。
- **HIS 分组页面美化**：系统代码 / 接口密钥（一键生成）/ 同步模式合理排序，接口地址卡片（虚线框）与测试区独立分区，布局更清晰。
...
### 新增
- **HIS 接口密钥一键生成**：接口管理 HIS 分组接口密钥输入框右侧恢复【🔑 生成密钥】按钮，一键生成 128 位随机十六进制密钥（点击保存后生效）。

### 变更
- **HIS API 地址语义澄清与自动填充**：该字段即本系统对外提供服务的接口地址（当前访问地址 + `/api/his`），留空时按当前访问地址自动生成并附说明（本机地址，非 HIS 的 IP，可手动修改）；README 同步补充说明。
...
### 变更
- **引用结果末尾不显示标点**：引用的结果始终追加在病历辅助检查字段末尾（最后一位），默认不带句号——避免与后续【请填写外院辅助检查结果】输入产生标点干扰；已有内容以句号结尾先去句号再以「，」衔接。
...
### 变更
- **检验引用点击结果行即选中**：引用选择弹窗中点击项目/结果/单位等行内容默认切换该行复选框（checkbox 上点击由 stopPropagation 单独处理避免双触发），更顺手易用。
- **引用后自动关闭全部弹窗**：检查/检验引用结果后调用 modal.closeAll() 一次性关闭 检查详情/检验引用/检验详情 全部层叠模态框（modal.js 新增 closeAll）。
- **引用文案标点衔接优化**：追加时若病历辅助检查已有内容以句号结尾先去句号再以「，」衔接，避免「。，」叠字；新文案始终以「。」结尾，与下一字段【请填写外院辅助检查结果】自然衔接。
...
### 新增
- **报告结果一键引用到病历**：化验/检查报告模态框在已出报告时、打印申请单左侧新增【引用结果】快捷按钮——
  检验报告先弹多选框（全选/逐项勾选，展示项目/结果/单位/参考范围/危急值）再引用，影像报告直接引用诊断结论；
  引用文案追加到病历【请填写辅助检查结果】字段，不影响既有内容（「，」分隔），影像诊断结论去换行单行展示。

### 变更
- **管理员【外部接口集成】更名为【接口管理】**：导航分组、页面标题、设置页迁移提示与影像科调阅提示等文案统一，
  接口分组子 Tab 由下划线式改为 btn btn-sm 按钮样式（激活 btn-primary / 未激活 btn-outline），与全局按钮体系统一。
- **病历辅助检查移除（已出报告）后缀**：已出报告状态由右侧边栏圆点标示，病历正文原样展示开单项目，不再画蛇添足。
...
### 变更
- **独立视窗阅片更名为阅片视窗**：按钮文案、视窗页标题（阅片工作站）、页面/脚本注释统一去掉「独立」字样（9 个文件），称谓更聚焦。
- **检查申请单徽章靠右显示**：经典模式申请单头部【检查中/已完成】徽章与登记/阅片按钮一并包裹进右侧操作组（margin-left:auto 靠右对齐），修复徽章与独立视窗按钮各自均分空白悬在中间的问题。
- **医技通用患者横条精简**：护士/检验/影像/药房共用横条（deptwork.renderHeader）移除【就诊中/诊毕】状态徽章与「急诊科 第003号」科室序号段（均与医技功能无关），保留姓名/性别年龄/费用类别/急诊门诊徽章/患者ID/流水号/挂号时间。

### 修复
- **书写报告模态框报危急值徽标 0 时隐藏**：`renderImgCritQueue()` 原先在 modal-foot 赋值前调用，`imgmCritBtnCount` 元素尚不存在，徽标停留在初始 0 不更新——改为 footer 就绪后再回填计数，0 时置空由 CSS `:empty` 隐藏，添加后显示数字角标。
...
### 新增
- **报危急值弹窗「＋」实时添加**：危急值项目输入框右侧新增【＋】按钮（回车亦可），
  点击实时加入暂存队列并就地刷新中部已添加列表（弹窗不关可连续添加），主窗口
  按钮徽标同步更新；弹窗三段布局重排为「输入(顶) → 已添加列表(中，就地实时刷新)
  → 接收医生(底)」；删除已添加项先弹确认框（避免误操作），确认后就地更新列表。
- **独立视窗阅片隐私安全（地址栏零泄漏）**：视窗地址栏不再携带 `visit` 参数——
  选中患者/序列改由 `localStorage` 会话握手（`clinic_viewer_pending_ctx`，sid
  绑定校验后读取清除）+ `BroadcastChannel viewer:context` 广播传递；视窗端数据
  加载仍走登录 Session 鉴权 + 科室归属校验（换用户/伪造链接均被拒）；主窗口关闭
  患者时广播空上下文，视窗复位空白引导页。
- **阅片自动内嵌（免手动点击）**：移除一体化工具栏与独立视窗底部的【内嵌阅片器】
  按钮及提示语；一体化模式选中序列（或初次加载）自动挂载 Web 阅片器 iframe，
  随序列切换即时更新；经典模式点【去写报告】广播上下文后独立视窗自动挂载；
  未配置 PACS 阅片器地址时视窗内居中提示（不弹 toast 打断操作）。
- **危急值按钮化收敛**：一体化底部与经典模态框底部按钮栏统一为【🚨 报危急值 N】
  （右上角圆形红色徽标实时计数），不再单独占用撰写区/左栏空间（小概率事件回归
  按钮入口）。

### 变更
- **危急值暂存队列统一持久化**：队列由页面内存变量改存 `sessionStorage`（按检查
  项目 id 键控），刷新页面/模式切换不丢失；报告发布时按当前项目取出逐条发送后
  清除；一体化与经典共用同一套渲染源（`openImgCritSend` / `renderImgCritQueue` /
  `imgCritRemove`）。
- **无效提示语清理**：删除「DICOMweb 接入后展示」「工具栏占位（Web 阅片器接入后
  生效：窗宽窗位 / 缩放平移 / 多序列 / MPR / 测量标注）」「专业阅片视窗」等冗余
  占位文案，工具栏保留工具按钮、视窗改简洁引导。
- **经典写报告模态框三栏分隔统一**：左栏补右侧分隔线（border-right + padding），
  与右栏对称；模态框底部新增【💾 保存草稿】（与一体化同一数据源/存储/校验）与
  【🚨 报危急值】按钮。

### 修复
- **经典模态框「建议随访复查」快捷词点击无反应**：`imgWritePane` 渲染快捷词时
  唯一一行把元素前缀写死为 `pacs`（经典模态框为 `imgm`），点击后查找不存在的
  `pacsConclusion` 元素无反应——改为与其余快捷词一致的 `idPrefix` 变量拼接。
- **APP_VERSION 常量不同步导致的资源缓存失效**：`bootstrap.php` 中 `APP_VERSION`
  此前硬编码 8.4.4 未随版本号提交同步（所有资源 URL 恒为 `?v=8.4.4`，浏览器与
  Service Worker 长期缓存旧 JS/CSS）——同步为当前版本并逐版本递进，强制缓存失效。
...
### 修复
- **无法保存草稿（一体化模式误报「已提交报告以库内容为准」）**：根因为演示库
  历史脏数据——order_items.status 存在非法值 `'3'`（100 行，伴随 sub_of 被
  文本 `'paid'/'done'` 污染 90 行、created_at 为空），前端状态机把非法状态
  一律判为「非已登记」命中已提交分支。三层修复：
  1. 数据出口归一（根治）：`deptwork_orders` 明细状态经
     `deptwork_norm_item_status` 防御——非法状态回退 `paid`（已缴费待登记），
     处方 `reviewed` 语义保留，任何来源脏数据不再打穿前端状态机；
  2. 演示库清洗：`status '3'→paid`、sub_of 文本污染→0、created_at 空值回填
     同单时间；
  3. 前端文案分态：`imgDraftSave` / `imgPublish` 按状态精确提示（done=已提交
     需先撤回 / paid=尚未登记需先登记 / 未知状态=显示状态名），不再用笼统的
     「已提交」文案掩盖真实状态，便于后续问题定位。
...
### 新增
- **撰写报告登记门禁（一体化模式）**：已缴费未登记（paid）项目在报告撰写区
  呈现锁定态——表单只读 + 「pacs-reg-gate」模糊遮罩卡片（居中提示「患者尚未
  登记」+ 说明文案 + 【📝 登记患者】按钮），遮罩整块可点：confirm 确认后走
  既有 register_order 整单登记，成功后 fetchPatient 局部刷新自动解锁；
  项目选择优先级扩展 registered → done → paid，顶/底操作栏同步登记入口。
- **撰写报告登记门禁（后端硬拦截）**：save_result 对 paid 状态显式拒绝并提示
  「该检查项目尚未登记，请先登记后再书写报告」（防止绕过前端直接提交）。
- **影像引用查询台账（第13项）**：新增 /admin/imagerefs（admin/imaging 角色）
  ——只存引用架构视图：按门诊流水号 / 患者编号 / 申请单号检索（三单匹配键），
  列表展示 Study UID / 检查类型 / 区域存储 / 张数 / 登记人；配置 Web 阅片器
  URL 模板后提供【🔍 调阅】直链；管理员【外部接口集成】组与影像科【管理】组
  挂菜单；后端 /api/imaging?action=refs_list 分页接口（角色门禁）。
- **历史报告影像调阅直链（第13项）**：history_reports 每条返回 study_uid /
  viewer_url（影像引用优先、报告号占位回退），历史详情新增【🔍 调阅影像】
  按钮（新窗口打开 Web 阅片器）。
- **报告草稿服务端同步（第13项）**：/api/imaging 新增 draft_save / draft_load /
  draft_clear（settings 键值对 img_draft_{uid}_{itemId}，仅本人读写 + 就诊
  归属校验）；前端草稿双写（服务端为主 + localStorage 兜底），撰写区重建后
  异步回填（本地草稿优先），跨设备/跨浏览器保留。
...
### 新增
- **PACS 三单匹配（优化1）**：以门诊流水号 `flow_no` 为唯一索引，把「影像引用、
  检查申请单、临床诊断快照」硬绑定——schema v34 新增 `imaging_refs` 影像引用表，
  `ImagingRepository::putRef` 写入前强制校验「引用 ↔ 申请单 ↔ 明细 ↔ 就诊」四方
  归属一致性（`assertTripleMatch`），不一致抛异常硬拦截并回滚整个报告出具事务，
  杜绝张冠李戴。
- **平台只存影像引用（优化2）**：`imaging_refs` 仅存 Study/Series UID 引用与元数据
  （modality / 张数 / region / meta），影像二进制交给 region 指向的区域影像存储
  （Orthanc + 对象存储等）与 Web 阅片器负责——系统耦合低、权限与审计统一、便于
  后期扩展；`save_result` 报告出具事务内同步登记引用（study_uid 报告号占位，
  PACS 网关接入后替换真实 DICOM UID）。
- **内嵌 Web 阅片器（优化3）**：一体化读片视窗与独立视窗均支持无插件挂载 Web
  阅片器（`viewer_url` 优先读影像引用，返回 region / series_count）——窗宽窗位、
  缩放平移、多序列、MPR、测量标注由阅片器自身提供，热数据调阅延迟由区域影像
  存储保障（≤ 5 秒目标）。
- **独立视窗患者/序列实时同步（优化12）**：主窗口（一体化与经典模式）加载患者 /
  选择序列时通过 BroadcastChannel 广播 `viewer:context`，独立阅片窗口实时刷新
  ——切换患者重新拉取患者数据、切换序列自动重挂阅片器。
- **/api/auth `action=me` 会话预检接口**：独立视窗打开时与 60s 心跳调用，
  401 即锁定（服务端为准）。

### 修复
- **独立视窗登录状态下误锁（优化11）**：根因为 authsync 登出里程碑写入
  localStorage 后永不清理，主窗口登出后新开/重开独立视窗被静态里程碑误判失效。
  修复：里程碑与广播携带触发时点 sid/uid，订阅方仅同会话事件才预检确认锁定；
  viewer 打开即调 `me` 心跳预检（401 才锁）；登录成功 `broadcastLogin` 清除旧里程碑。
- **一体化模式模板下拉为空（优化4）**：`loadPacsTpls` 此前从未被调用——挂载后
  接入加载，与经典模态框共用 `/api/template` imaging_report 数据源；选择模板
  弹出预览模态框（取消 / 续写 / 覆盖），应用后跳到撰写页签。
- **一体化与双屏数据不一致（优化5）**：已提交报告（done）在一体化撰写区回显库内
  正式内容并置只读（提示报告号），不再呈现「未书写」。

### 变更
- **一体化报告预览与草稿（优化6）**：草稿 localStorage 按项目 id 暂存、刷新自动
  回填、提交成功清除；已提交报告底部操作栏切换为【🖨️ 打印报告】+【✏️ 申请修改】
  （走既有撤回管理员审核流）。
- **一体化左栏布局精简（优化7）**：检查信息卡移至左下角，移除与患者横条重复的
  门诊号 / 患者ID；新增申请单号（点击弹出申请单预览模态框）；序列列表按检查类别
  （DR/CT/US/MR）小分隔标题分组；选择序列后左下角检查信息（申请单号 / 申请类型 /
  开单科室 / 检查项目 / 检查状态）动态更新，撰写区与底部操作栏随状态联动重建。
- **经典写报告模态框三栏重构（优化10）**：布局对齐一体化右栏——左临床信息 /
  中报告撰写 / 右历史报告；`imgClinPane` / `imgWritePane` / `mountImgHistory` /
  `loadPacsTpls` / `pacsTplPick` 支持 idPrefix 前缀双实例（pacs / imgm），
  内容与交互单点维护，两处体验完全一致；删除模态框旧模板函数体系。
- **历史报告文案与空态（优化9）**：移除啰嗦的检索方式说明；空态改为居中提示卡片
  （图标 + 主文案 + 副文案，虚线边框卡片）。
- **外部接口集成子 Tab 样式（优化8）**：补全 itg-tabs/itg-tab 分组导航样式
  （底部主色指示条 + hover/active，适配明暗主题）。
...
### 新增
- **外部接口集成中心（管理员 → 🔌 外部接口集成，/admin/integration）**：
  原【系统设置 → HIS 接口配置】抽离迁移至此，按 Tab 选项卡统一维护六组外部
  接口配置——**HIS 接口**（API 地址 / 系统代码 / 密钥 / 同步模式）、**支付接口**
  （微信支付 / 支付宝 / 聚合收单占位）、**医保接口**（国家 / 地方医保前置机地址、
  机构编码、操作员账号）、**DICOM / PACS 接口**（PACS 服务器 IP / 端口、
  AETitle、WADO-RS / DICOMweb 服务地址、Web 阅片器 URL 模板——支持
  `{study_uid}` 变量替换）、**HL7 v2.x 接口**（MLLP / HTTP 接收地址、
  发送方 / 接收方标识符）、**FHIR 接口**（R4 Endpoint、Token 认证参数）。
  字段字典统一由 `integration_field_groups()` 维护（视图渲染与保存白名单共用
  同一数据源），存储复用 settings 键值对规范，`action=integration_save`
  分组保存互不干扰（含 PACS 端口格式校验）。
- **影像科工作台双模式阅片重构（PACS/RIS 工作站）**：
  - 顶部新增【🖥️ 经典双屏分屏 / 🩻 一体化阅片】视图切换器 + 【🖥️ 独立视窗
    阅片】按钮，localStorage（`clinic_img_view_mode`）记忆用户最后选择；
  - **模式 A 一体化阅片模式**（参考专业 RIS 阅片工作台）：三栏展开并隐藏系统
    右侧大纲栏——左栏检查信息（患者 / 门诊号 / 检查项目 / 检查类型 DR·CT·US·MR
    自动推导）+ 序列缩略图占位（512×512 尺寸与张数标注）；中栏专业深色读片
    视窗（HOIF / Cornerstone 风格、不随主题切换）+ 工具栏占位（窗宽窗位 /
    缩放平移 / 测量标注 / 旋转翻转 / 重置）+ DICOMweb Viewer 挂载容器；右栏
    三 Tab【临床信息】（门诊号 / 姓名 / 性别 / 年龄 / 主诉 / 临床初步诊断）、
    【报告撰写】（模板下拉、常用语快捷插入、影像学表现 / 影像学诊断双区、
    标记危急值、保存草稿 / 提交审核 / 退回修改底部操作栏）、【历史报告】；
  - **模式 B 经典双屏分屏模式**：保留现有布局与模态框写报告，患者行新增
    「独立视窗阅片」按钮；
  - **独立视窗阅片（/viewer.php）**：`window.open` 弹出无工具栏窗口用于多
    显示器全屏拖拽阅片，与主窗口共享登录 Session（登录门 + 影像科角色门）。
- **跨窗口 Session 安全锁定（核心规范）**：`BroadcastChannel`（通道
  `clinic_auth_sync`）+ `localStorage` 里程碑兜底（`authsync.js` 组件）——
  主窗口退出登录（确认框通过后）或任意 AJAX 请求检测到 401 会话失效时广播
  `auth:logout`，独立阅片窗口**立即激活高斯模糊遮罩**，正中提示「登录会话
  已失效，阅片工作站已锁定，请重新登录主系统」并禁止一切影像操作。
- **患者历史报告调阅（影像科）**：一体化右栏 Tab 与经典模式写报告模态框均
  新增【历史报告调阅】——严格按患者唯一标识 `patient_id`（`patients.patient_no`）
  异步检索（禁止姓名检索，防同名同姓混淆），首屏最近 5 次分段加载 + 「加载
  更多」；列表项含检查日期 / 影像号（报告号）/ 检查项目 / 报告医生 / 审核
  医生 / 报告状态；点击展开详情全部**只读**（检查所见 / 影像表现 / 影像诊断），
  仅【影像表现】【影像诊断】旁提供一键【复制到当前报告】（`pacshistory.js`
  组件，事件委托支持多容器挂载）。
- **后端接口**：`/api/imaging` 新增 `action=history_reports`（按 patient_id
  分页查询历史影像报告，已撤回报告纳入调阅并标记状态）与 `action=viewer_url`
  （读取阅片器 URL 模板并完成 `{study_uid}` 替换，PACS 未接入前以报告号 /
  申请单号作为占位 UID）。

### 变更
- 【系统设置】移除 HIS 接口卡片，保留迁移指引链接；`action=settings` 不再
  处理 `his_api_key`（改由 `integration_save` 维护，历史数据无缝继承）。
...
### 修复
- **大屏绑定残留自愈（关闭浏览器未解绑不再永久占用）**：绑定人心跳超过
  300 秒无更新（关闭浏览器/异常断开且未走解绑流程）即视为已离开，新增
  `QueueRepository::sweepStaleBindings()` 批量释放——在 管理端诊室列表、
  医生/医技可用诊室查询、绑定入口 惰性触发（与 screen.php 心跳侧过期自动
  解绑同语义）。此前该清理仅在大屏在线轮询时触发，绑定人关浏览器且大屏
  离线时残留绑定需管理员手动踢除；现在任何触碰诊室数据的操作都会自愈，
  被占用的诊室可直接重新绑定。叫号推送状态（当前呼叫患者）保留不清除，
  大屏继续展示最后呼叫患者直至重新绑定叫号。

### 构建
- **GitHub Actions 运行时升级（消除 Node 20 弃用警告）**：checkout v4→v7、
  upload-artifact v4→v7、download-artifact v4→v8（均迁移至 Node 24 运行时）。
...
### 修复
- **CI 首轮发现的 PHP 7.2 语法不兼容（2 处）**：
  1. `tools/php-lint.php`：`array_merge()` 调用尾随逗号（PHP 7.3+ 才允许）→ 移除；
  2. `tools/seed_test_data.php`：箭头函数 `fn() =>`（PHP 7.4+ 语法）→ 改为
     `function () use ()` 匿名函数。
  全库同步扫描箭头函数 / `??=` / `match` / `?->` / 数组展开等 7.3+ 专属语法，
  无其他不兼容点。
- **叫号管理「选择科室」模态框在线数滞后**：科室下方「x/x 在线」为页面加载时
  固化的统计数据，页面停留期间大屏上下线后不更新（刷新页面才恢复）——
  现改为**每次打开模态框前实时拉取** `room_stats` 刷新在线统计（获取失败仍
  打开模态框沿用已有数据）。
...
### 变更
- **默认图标改为透明底圆形红十字**（PWA 桌面图标 + 浏览器标签页图标共用）：
  - 透明背景 + 白色圆形（浅灰描边）+ 居中红色医疗十字，圆外区域全透明；
  - 2x（1024）绘制后重采样到 512，圆形/十字边缘平滑无锯齿；
  - 浅灰外环保证白色圆在浅色标签栏中也有轮廓；
...
### 修复
- **缴费页签去除挂号费缴费凭条**：挂号凭条（就诊页签）即挂号缴费凭条，
  缴费页签仅展示开单缴费凭条，不再重复。
- **报告补打纸张恢复原格式**：此前误传 a5（竖版病历纸）参数导致先按竖版分页
  破坏版式——改为不传纸张参数，由 print.js 依据报告内容自动识别
  （检验报告=横版 A5 分列分页，检查报告=A4 纵向）。
- **报告页签项目名解析**：修复 results.type（lab/imaging）与项目表前缀
  （lab_items/exam_items）键不一致导致检查报告项目名回退「检查项目」的问题。

### 变更
- **打印中心右栏四子页签**：选中就诊后，右栏按「就诊 / 开单 / 缴费 / 报告」
  四个子页签切换（页签含单据数量），不同项目之间虚线分隔：
  - **就诊**：挂号凭条（挂号费退费/取消后红色删除线保留溯源、不可补打）、
    电子病历、诊断证明；
  - **开单**：全部开单记录按 检验申请单 / 检查申请单 / 处置单 / 处方单 分组
    （已退费开单标注「已退费」，仍可查看原始申请单）；
  - **缴费**：全部缴费凭条（新增展示，此前缺失）——整批退费的凭条红色删除线
    保留溯源且不可补打，部分退费显示「部分退费」标注；
  - **报告**：检验报告 / 检查报告 分组（新增展示，此前缺失），已撤回报告
    红色删除线保留溯源、可查看原始报告。
...
### 变更
- **统一打印中心改版（左右两栏）**：
  - 左栏就诊记录列表 + 右栏可打印单据详情，选中即看、点单据即打印预览；
  - 检索新增**患者姓名**（姓名 / 患者ID / 门诊流水号 / 身份证号 任一匹配），
    支持清空关键字回车直接浏览全部；
  - 列表按**就诊时间倒序**（最新在最上），**滚动分段加载**（每页 20 条，
    滚动到底自动追加，附「加载更多」按钮兜底），不再一次全量加载；
  - 进入页面自动加载最新列表并选中第一条，右栏直接呈现该就诊可打印单据；
  - 后端新增 `/api/admin?action=print_visits` 分页检索接口。
...
### 变更
- **医技叫号大屏排队队列纵向对齐**：性别 / 年龄 / 就诊科室 三列由内容自适应改为
  **共享列宽**（与医生端等待就诊同思路——隐藏探针行测各列最长内容自然宽度，
  设为 `--dept-*-w` 变量供所有行共用），各行序号/姓名/性别/年龄/就诊科室
  全部纵向对齐，观感更整齐。
...
> 医技四科室（护士站/检验/影像/药房）叫号升级：参考医生工作站「绑定诊室 → 叫号」模式，并重设计医技叫号大屏三种尺寸。

### 新增
- **医技工作台叫号（绑定诊室 + 叫号面板）**：
  - 点击「叫号」未绑定诊室时先弹出**选择大屏诊室**悬浮窗（按角色类型过滤
    nurse/lab/imaging/pharmacy 诊室，在线可绑定，一人一块屏），绑定后才打开叫号面板；
  - 叫号面板：当前处理中 / 下一位 / 候诊队列 + 「📢 叫号下一位」「🔁 重呼」按钮 +
    底部**解绑**；**mini 精简版**（localStorage 记忆）+ 拖动位置记忆（sessionStorage）；
  - 绑定信息复用 `Clinic.roomHeartbeat`（sessionStorage 跨页面持久 + 30s 心跳保活，
    心跳接口按角色自动路由 doctor→/api/doctor、医技→/api/deptwork）；
  - 后端新增 `/api/deptwork`：`get_available_rooms / bind_room / unbind_room /
    room_heartbeat / call_next / call_repeat`。
- **医技叫号大屏三种尺寸重设计**（lab/imaging/pharmacy/nurse）：
  - 竖屏：上半「当前患者」大卡（标签+姓名换行+序号/开单科室）→ 下半排队队列；
  - 方屏：当前患者文字横排「当前患者 XXX」、上半更小；
  - 宽屏：左侧当前患者（标签+姓名换行）、右侧一半排队队列；
  - 队列展示 **姓名 / 性别 / 年龄 / 开单科室**；无患者「当前暂无患者」、
    无队列「当前暂无排队患者」占位居中，字号 cqmin 随屏等比缩放。
- **医技大屏数据升级**：由注册态看板改为**订单级待办队列**
  （`tech_dept_queue` 按 room_type 区分检验/检查/审方发药/护理处置），
  叫号面板推送的患者作为「当前」并移出队列；被叫号患者不在队列前 N 位时
  独立回查保证大屏始终显示当前叫号患者。
- **医技叫号测试数据**：`tools/seed_dept_call_test.php` 为四个医技队列各加
  N 位待办测试患者（默认 20，幂等按「尚无该类型待办单」挑选患者）。

### 修复
- **医技大屏一直显示「正在加载大屏数据…」**：医技大屏页仅加载 `screen.js`
  （无 ajax.js），`renderDeptMode` 误用 `Clinic.escHtml` 导致渲染抛错——
  改为就地定义 HTML 转义函数。
- **医技叫号绑定窗口改为与医生工作站一致的下拉**：点「叫号」未绑定时，在叫号
  按钮下方弹出诊室下拉（🟢 在线空闲 / 🟡 占用 / 🔴 离线 / 🔵 已绑定，
  显示在不在线），点击在线诊室绑定后才自动打开叫号悬浮窗；按钮文案跟随绑定
  显示「叫号：诊室名」，解绑同样在下拉中操作。

---

## [8.2.1] - 2026-09-10

> 浏览器标签页图标统一 + 修复危急值管理日期默认值未填充。

### 变更
- **浏览器标签页图标（favicon）**：登录页与系统框架页统一指向 `/pwa-icon.png`
  ——有医院 LOGO 时输出 LOGO，未设置时显示默认主色医疗十字图标，与 PWA 图标一致
  （LOGO 直链封禁不变，图标走独立端点，URL 不暴露真实文件路径）。

### 修复
- **危急值管理日期框默认值未填充**：`initListPage` 在容器渲染前调用
  `setDefaultRange()`，此时日期输入框尚未存在导致默认日期写入失败、始终显示
  「开始日期 / 结束日期」占位文字——调整为容器渲染后再填充默认近 3 天日期，
  打开页面即看到实际日期值。
...
> PWA 桌面应用支持 + 危急值管理默认近 3 天时间范围。

### 新增
- **PWA 桌面应用**：浏览器可将系统安装为桌面应用（应用名 = 医院名称，独立窗口运行）：
  - `/manifest.webmanifest` 动态生成 Web App Manifest（名称 / 短名称取医院名称，
    无则默认「门诊一体化系统」；图标 192/512 双规格）；
  - `/pwa-icon.png` 动态图标：优先输出医院 LOGO（settings.logo，realpath 防目录穿越，
    封禁直链不变），未设置 LOGO 时用 GD 自动生成 512×512 主色圆角 + 白色医疗十字默认图标；
  - `public/assets/js/sw.js` Service Worker：仅静态资源（css/js/字体/图片）缓存优先 +
    后台刷新（stale-while-revalidate，随 `?v=` 版本参数自动失效），页面与接口实时直连网络；
  - 登录页与系统框架页 `<head>` 注入 manifest / theme-color / apple-touch-icon，
    页面加载时自动注册 Service Worker（登录页同样生效，便于首次即安装）。

### 变更
- **危急值管理页默认时间范围**：医生 / 检验 / 影像 / 管理员危急值列表默认查询
  「近 3 天」（今天起往前 2 天，含今天），可手动修改；「重置」恢复默认 3 天范围；
  默认日期取服务端站点时区（前端浏览器时区与站点时区不一致时仍准确）。

---

## [8.1.2] - 2026-09-09

> 修复影像科危急值发布流程与预览、统一危急值管理页日期选择器。

### 修复
- **影像科危急值「加入预览」后实际未发送**：`critical.js` 发送弹窗的 `SEND_CTX`
  未保存 `onAdd` 回调，导致点击「＋ 加入危急值预览」时提示成功但队列始终为空，
  报告发布后不会发送任何危急值——已修复回调保存，发布时逐条发送。
- **影像科左下角无危急值等待发送预览**：写报告弹窗左下角新增
  「危急值等待发送（N）」计数与已添加列表（可删除）；再次点击「报危急值」
  时弹窗内展示已添加的危急值（项目 + 接收医生），避免重复添加 / 误以为丢失。

### 变更
- **危急值管理页日期选择器统一**：医生 / 检验 / 影像 / 管理员危急值列表页的
  开始 / 结束日期框改为系统自定义日期选择器（`Clinic.datePicker`，点击弹出），
  高度与查询 / 重置按钮保持一致。
...
> 危急值三项优化：文本型危急值（HIV 阳性）支持、检验报告单趋势标记纵列（↓/↑/危）、危急值记录严格以续写形式插入且绝不修改首诊（诊毕后可插入）。

### 新增
- **文本型危急值**：检验项目危急值上下限可填文本（如 HIV 填「阳性」、正常值填「阴性」），
  提交结果时录入值等于危急值文本（不区分大小写）即触发危急值通知——数值比对与
  文本比对统一收敛为 `crit_row_hit`。
- **检验报告单趋势纵列**：结果与单位之间新增细小无表头纵列——数值型结果对照闭合
  正常范围（如 4-8）自动显示 ↓（低于下限）/ ↑（高于上限），危急值显示红色「危」；
  非数字 / 未配置范围 / 范围非闭合（如「阴性」）不显示箭头。

### 修复
- **危急值处理不再「修改首诊」**：处理危急值始终以 `is_critical=1` 的续写文书独立
  插入，绝不覆盖首诊；修复处理后前端误重载病历导致的「首诊未保存内容丢失」——
  仅在「正打开该患者病历页且无未保存修改」时才重载；意识状态 / 初复诊改按
  「当前文书自身镜像」精确回读（危急值记录的系统空镜像不再污染首诊显示），
  打印病历同理。
- **诊毕后可插入危急值记录**：诊毕（归档）病历不可修改，但危急值记录仍可正常插入
  （新增续写节点，插入后同样全局只读），已实测验证。
- 打印病历续写段标题支持「危急值记录」标识（原仅 续写病历 / 会诊记录）。

---

## [8.1.0] - 2026-09-09

> 危急值全流程体系上线——检验科自动检测 / 影像科手动上报 → 站内消息通知开单医生 → 医生左右分栏处理并自动写入病历 → 各部门危急值管理与全院查询溯源。危急值记录与病历「危急值记录」文书全局只读、快照固化，检验科撤回修改数据不影响已归档危急值。

### 新增
- **危急值数据模型**：新增 `critical_values` 表（v32/v33 迁移），报告快照独立固化
  `snapshot_json`（完整检验结果 / 影像所见与诊断），危急值明细存 `items_json`，
  后期检验科撤回 / 修改数据不影响已处理危急值的展示。
- **检验科危急值自动检测**：提交检验结果生成报告时自动比对项目危急值上下限
  （容忍 `> / < / ≤ / ≥` 前缀的数值写法），命中即弹出「检测到危急值」通知流程，
  默认通知开单医生，可点击医生姓名弹出搜索框另选（分页每页 20 人 + 无限滚动，
  搜索按姓名 / 工号 / 用户名，工号前置避免重名）。
- **影像科危急值手动上报**：写报告模态框左下角新增「🚨 报危急值」按钮，手动输入
  危急值项目（如脑疝、眼球破裂），加入左下角预览队列（可删除），报告发布时一并发送。
- **医生危急值处理**：站内消息 / 危急值管理点开 → 左右分栏处理弹窗（左侧患者信息 +
  危急值红色加粗 + 该报告完整检验结果；右侧符合病情 / 不符合病情 + 处理措施）；
  提交后自动在病历插入「危急值记录」续写文书（医生 / 时间自动写入，`is_critical=1`
  全局只读，不占用医生编辑位），右侧病历节点以「（危）」标记展示。
- **危急值管理页**：医生端显示接收到的全部危急值（待处理 / 已处理），检验科 / 影像科
  显示本部门已发送记录（发送医生 / 接收医生 / 是否处理 / 处理时间 / 处理时长），
  均支持日期范围与处理状态筛选；点击进入只读详情查看完整处理过程。
- **管理员查询中心**：运营分析与打印中心之间新增「🔍 查询中心」，首个子项
  「危急值查询」按医生端列表呈现全院危急值记录并支持详情查看。
- **危急值站内消息**：新消息类型「critical」，铃铛面板与消息中心显示红色危急值徽标，
  点击直达处理 / 详情弹窗。

### 变更
- 检验科工作台列表补充危急值阈值提示的展示与检测逻辑联动；组合项目逐项比对。
- `deptwork` 患者工作台数据回传开单医生 id，支撑发送弹窗默认接收人。

### 安全
- **SPA 直链访问优化**：侧边栏 / 消息链接均经 AJAX 局部刷新，角色门服务端 403
  拦截（此前已验证非影像科账号直链 `/imaging/home` 返回 403）；前端对 403
  局部导航给出明确「无权限」提示并返回工作台，不再整页刷新到死 403 页。
- 危急值记录与「危急值记录」病历文书无任何删除 / 修改接口，处理仅推进状态字段，
  完成即为只读（病历侧 record_save / record_delete / get_editable_record 均熔断）。
...
### 文档
- **README 与落地页同步 v8.x 能力**：
  1. 功能表新增【🔐 登录安全】行（验证码三模式 / 防爆破自动锁定 / 管理员
     三态徽章与一键解锁 / IP 频控 / 防重放 / 防枚举）；管理员行补验证码
     模式与锁定阈值配置说明；医生工作站使用流程补科室间会诊（多科室并发、
     自动打印预览）与诊毕快照封存说明；安全说明与角色说明同步补齐；
  2. 目录结构修正：主库表数 39→43、core 类补齐 6 个新类（LoginSecurity /
     barcode / IdObfuscator / EmrContextResolver / DataExportImport /
     emr_rules）、js/components 补齐 17 个新组件；
  3. 落地页特点清单补登录安全行，医生工作站卡片补会诊/知情同意书/
     诊断证明能力，theme.js 引用补版本参数（缓存失效跟随版本）。
- **APP_VERSION 常量更新**：bootstrap.php 中 `APP_VERSION` 长期停留在
  7.28.16，导致静态资源缓存参数（`?v=`）从未随版本失效——更新至 8.0.3
  与 README 徽章对齐，后续版本递增时同步维护。

---

## [8.0.3] - 2026-09-09

> 验证码弹出时序修复——登录失败立即弹出、密码框聚焦即嗅探、首次失败即入惩罚期。

### 修复
- **登录失败后验证码不立即显示（需刷新页面）**：`onError` 旧逻辑仅在验证码
  已显示时换图，首次失败不展开。修复为无论何种失败（密码/验证码/锁定）
  无条件展开验证码并强制换新图（off 模式除外，永不展示）。
- **服务端惩罚期阈值与体验不一致**：`ipInPenalty` 判定连续失败 ≥2 次才
  进入惩罚期，导致失败 1 次后 `check_captcha` 仍返回不需要、刷新页面
  才出现验证码。改为**首次失败即入惩罚期**（30 分钟无失败自动解除）。
- **输完用户名切到密码框不触发嗅探**：仅监听了用户名 blur/输入，新增
  密码框 focus 嗅探——光标切到密码框立即调用 `check_captcha` 弹出验证码
  （覆盖 Tab 切换/浏览器预填用户名等不经 blur 的路径）。
- 登录页输出服务端权威 `CAPTCHA_MODE` 全局变量：off 永不展示不嗅探、
  force 初始常显、auto 按双轨驱动，前端展示逻辑与全局配置严格对齐。

---

## [8.0.2] - 2026-09-09

> 验证码 blob 化呈现、登录失败强制换图、输入框高度统一、用户编辑表单按角色裁剪。

### 变更
- **验证码图 blob 化呈现**：前端 fetch（same-origin，no-store）拉取验证码
  内容后经 `URL.createObjectURL` 以 blob 呈现——图片不再以可直链复制的
  URL 暴露（外部无法跨会话复用同一链接批量拉取验证码做撞库预处理）；
  换图前 revoke 旧 blob 防内存泄漏。图片仍与 Session 绑定，安全性不变。
- **登录失败强制刷新验证码**：验证码行已显示时，无论失败原因（密码错误/
  验证码错误/账号锁定）均重新 fetch 换新图——服务端比对该图后已销毁，
  旧图本就不可复用，此举消除「失败后旧图残留」的体验歧义。
- **验证码行与用户名/密码行严格等高**：`.login-captcha-box` 容器固定
  38px（与 `.input` 标准 38px 总高一致），边框由容器提供不叠加，输入框
  height:100%——三行输入控件高度完全统一。

### 修复
- **用户编辑表单按角色裁剪（后端权威 + 前端联动）**：
  1. 【候诊列表可显示天数】仅医生角色显示——其他角色的候诊可见范围由
     开单医生的天数权限决定，自身无此配置；后端 user_save 对非医生强制
     回落默认 3 天；
  2. 管理员编辑页不再显示【状态】选择且强制启用——管理员不可被停用
     （含自锁保护），后端对 role=admin 强制 status=1，防伪造请求绕过；
  3. 前端 onRoleChange 同步联动候诊天数/状态的显隐。

---

## [8.0.1] - 2026-09-09

> 登录 500 紧急修复（运行时关键列自愈）与登录验证码独立行布局。

### 修复
- **任何用户（含 admin）无法登录——登录接口 500**：存量库 schema 版本
  虽到 v31，但 users 表缺失 `lock_reason`/`locked_at`/`lock_ip` 三列——
  版本迁移链若在历史版本失败即 `break` 停留在旧版本号（如旧环境缺
  JSON1 扩展卡在 json_extract 迁移），v31 的 ALTER 从未执行；而新登录
  代码的成功路径/锁定路径 UPDATE 引用缺失列 → PDOException → 500 →
  前端 JSON 解析失败报「网络请求失败」。修复：`DatabaseManager::getMain`
  新增 `ensureRuntimeColumns` **运行时关键列自愈**（与版本迁移解耦）——
  登录安全关键列逐列幂等补齐，首次访问即生效，无需人工干预；本地实测
  「验证码 + 正确密码」登录成功、计数清零。
- **登录验证码遮盖密码框**：验证码从密码同行改为**独立一行**（display
  切换 + fade 入场动画）；输入框与图片同处一个圆角边框容器——图片嵌入
  输入框内部右段（object-fit 裁切 + 细线分隔 + focus 边框联动），总宽
  与用户名/密码输入框严格等宽，浑然一体。
...
> 多模式登录验证码、防撞库安全锁定与账号生命周期管理体系（数据库迁移 v31 / 大版本功能新增）。

### 新增
- **图形验证码（零依赖 GD）**：`core/LoginSecurity.php` 生成 4 位字母数字
  验证码 PNG（剔除易混淆字符、干扰线+噪点、逐字符随机偏移），Session
  存储 5 分钟有效期；**防重放关键约束——比对即销毁**（无论密码校验是否
  通过，同一验证码仅可消耗一次），杜绝同一验证码复用发起并发爆破。
- **验证码需求预检接口** `check_captcha`：off 直接不需要 / force 直接需要 /
  auto 按「IP+会话连续失败惩罚期（30 分钟窗口）或目标用户 `login_fail_count>0`」
  判定；每会话每分钟 30 次限流防高频穷举有效工号。
- **防爆破自动安全锁定**：连续密码错误达阈值（可配 3-10，默认 5）→ 原子
  递增计数后条件锁定（`status=0, lock_reason='password_error_locked',
  locked_at, lock_ip`，`WHERE status=1` 防并发重复告警），并向全体管理员
  发送高优先级安全告警站内信（含工号/姓名/时间/来源 IP/直达解锁页链接
  `/admin/users?edit_user_id=N`）。
- **登录精准话术拦截**：`status=0` 账号不比对密码、不累加计数——
  `password_error_locked` → 「该账号因密码连续输入错误过多已被系统安全
  锁定…」（接口 403）；其他 → 「该账号已被管理员停用，无法登录」（403）。
  用户不存在仅记 IP/Session 失败频次，返回通用「用户名或密码错误」。
- **登录页验证码弹性等宽布局与双轨驱动**：密码行 flex 容器，激活验证码后
  密码框收缩（flex 1→1.9）+ 验证码输入框 + 验证码图，三段拼接总宽与
  用户名输入框严格 1:1 对齐（transition 平滑过渡）；轨道1 本地 Storage
  失败标记优先展示（隐私模式静默降级）；轨道2 用户名 blur/防抖异步预检
  无条件展开；图片点击刷新、验证码框回车提交。
- **管理端用户管理三态徽章**：正常启用（绿）/ 安全锁定 (密码超限)（红）/
  已停用（灰），区分系统自动锁定与管理员主动停用。
- **编辑弹窗锁定警告框与一键解锁**：`password_error_locked` 用户置顶黄色
  警告框（锁定时间/连续错误次数/来源 IP）+【解除锁定并启用】快捷按钮。

### 变更
- **数据库迁移 v31**（`users` 表）：新增 `lock_reason` / `locked_at` /
  `lock_ip` 锁定归因三件套；ALTER ADD COLUMN 幂等（列已存在自动跳过），
  SQLite/MySQL 双驱动兼容，存量用户数据零影响。
- **系统设置**：新增 `login_captcha_mode`（off/auto/force，默认 auto）与
  `login_fail_lock_count`（3-10，默认 5）全局配置，后台【安全设置】表单
  可视化保存回显；`options_data.php` 注入模式枚举字典。
- **管理员解锁闭环**：启用用户保存时同步重置 `lock_reason=NULL/locked_at=
  NULL/lock_ip=''/login_fail_count=0`，从安全锁定恢复时写入 audits 审计
  日志（`user_unlock` 类型）供追溯；解锁后即可无障碍正常登录。
- **登录成功处理**：失败计数清零、锁定归因置空、IP 失败频控与验证码
  状态清除、历史 15 分钟临时锁（`login_locked_until`）废弃失效。

### 安全
- 验证码防重放销毁、登录失败 IP+会话频控、check_captcha 防枚举限流、
  阈值锁定并发幂等（告警信单发）、锁定态不比对密码不计数（锁定账号非
  有效爆破目标）。
...
> 会诊申请单 A5 版式修复——正文区 min-height 超出版心导致整单变长与空白第二页。

### 修复
- **会诊申请单未按 A5 单页呈现（整体变长 + 空白第二页）**：
  `print-consult-flex` 的 `min-height:140mm` 按未扣页眉页脚的版心估算，
  而 A5 分页器实际可用版心 ≈ 187mm − 完整页眉（约 48mm）− 页脚沉底组
  （约 38mm）≈ 99mm——140mm 必然超出，触发 `.a5-overflow` 兜底：正文页
  自然增高（整单变长），提示语/签名/页脚组被整体推到第二页（视觉上的
  空白第二页）。该缺陷自 2/3 病历摘要 + 1/3 会诊信息排版优化引入，
  会诊发起后自动弹出预览后首次暴露。修复：
  1. `min-height` 140mm → 85mm 安全值（任何页眉高度下均可单页容纳），
     flex 内部 2:1 弹性分栏仍会拉伸撑满该区域，屏显与打印一致；
  2. 提示语上方透明占位 div 借 `print-note` 类归入分页器「页脚沉底组」
     （原 className 为空会被识别为正文节点，极端页眉高度下可能被单独
     推到第二页）。
...
> emr.js 拆分全面审计（清尾）、门诊处置打印版式细节修复、会诊自动打印申请单。

### 修复
- **门诊处置打印行间空行与处置项缩进**：上一版逐行节点内 pf-body（inline）
  嵌 block 处方 div，块中断行内上下文产生匿名空行盒（每个处方行后跟一空行）；
  非首行的 visibility:hidden 标签占位使处置项整行右移标签宽度。修复：行节点
  全部改为纯 inline 内容（去掉 block 包裹与隐藏标签）——「门诊处置：」标签
  独立占行（与原整节版式一致），处方行/处置项行全部从标签起点左对齐起笔，
  无空行、无缩进；行间距由 .pf-row 类控制（不依赖 :last-child，分页器
  克隆重组 DOM 后该伪类语义不可靠）。
- **会诊申请单底部提示语上方实线去除**：正文区结尾的 print-line 改为同尺寸
  透明占位（复刻 print-line 的内外边距与 padding）——仅去掉 1px 线条，
  布局排版零位移（患者信息区下方的横线保持不变）。

### 变更
- **会诊发起成功后自动弹出会诊申请单打印预览**（A5），与诊断证明开具后
  自动打印同模式。

### 说明
- **emr.js 拆分全面审计**（ctx 属性 vs _ctx 匹配、隐式全局赋值、未声明
  标识符、桥接递归、inline onclick 导出核对、共享状态同步路径）：除诊断
  删除 DIAG_ROWS（已修）外唯一残留为 `consultRawId` 死桥接——目标函数未从
  emr_consult.js 导出且被本地实现声明提升覆盖（死代码），已删除，防未来
  「is not a function」回归。其余各项均无问题。

---

## [7.31.0] - 2026-09-09

> 右栏交互修复（诊断删除失效拆分后遗症 / 会诊+消失允许多科室并发 / 证明与会诊实时刷新）与门诊处置跨页拆分。

### 修复
- **右侧边栏诊断删除/排序按钮无效（emr.js 拆分后遗症）**：`renderLeftNav`
  中 `DIAG_ROWS = diagOrder` 赋值到 emr.js 作用域的隐式全局变量，而
  `delDiag` / `openDiagOpsPop`（删除/设为主诊断/上移/下移）读取的是
  `emr_diag.js` 模块私有 `var DIAG_ROWS`（恒为空数组）→ `row` 恒 undefined，
  点击全部静默无反应。修复：`emr_diag.js` 暴露 `setDiagRows(rows)`，
  渲染后经 setter 同步行缓存。
- **诊断证明开具/删除后右侧列表不实时**：开具后 `renderLeftNav` 用的仍是
  旧 DATA（`has_certificate` 未更新）。「＋」开具时被物理 remove，删除后
  无法恢复。修复：后端 `certificate` 响应附带完整证明行（can_delete 与
  record_read 同口径），前端开具/删除后即时同步本地 DATA 并重渲染，
  删除后重建分区「＋」开具入口（跨科室只读不重建）。
- **发起会诊后右侧列表不实时**：发起成功后本地插入 DATA.consults（后端
  create 响应附带完整会诊行）并立即重渲染右侧列表与病历门诊处置
  「请X科会诊」，异步列表拉取保留为服务端权威校准。其他模块刷新闭环
  排查：开单/知情同意书/诊断/病历节点均已实时，无同类问题。
- **发起会诊后「＋」消失且刷新后不再出现（无法多科室并发会诊）**：
  「＋」显示条件误用「就诊存在任意 pending/doing 会诊」。正确规则
  （与后端 create 拦截同口径）：仅「处于会诊模式」（本医生科室被要求
  会诊，后端 consult_mode 权威锁定）隐藏「＋」；本人向多科室发起的
  并发会诊不锁定发起方，「＋」保留；同科室重复会诊由选择弹窗与后端
  拦截（完毕后可再发）；无可编辑病历隐藏「＋」。
- **打印：门诊处置项目过多整节后置到第二页（第一页大片空白）**：A5
  分页器按「整节点」分配，仅 `print-split` 纯文本节点可拆分。修复：
  ① 门诊处置按「行」输出独立节点（处方行逐行、处置项+自定义一行，
  后续行以隐藏标签占位对齐缩进），处置/处方过多时自然跨页续行；
  ② 其余纯文本小节（主诉/现病史/嘱托等）追加 `print-split`；
  ③ 分页器新增 `splitFlowNode`：含 .pf-body 的结构化小节保留
  「标签+内容」结构按字符二分，续段隐藏标签占位对齐缩进；纯文本节点
  仍走原拆分路径，分页器核心逻辑对其他文书零影响。

### 说明
- 条形码单号文字显示参数：`barcode128_svg($text, $height, $scale, $withText)`
  第 4 参数此前已存在——默认 `false` 不显示文字，传 `true` 才在条码下方
  内嵌单号文字（textLength 与条码等宽对齐）。当前仅文档类条码
  （`pt_barcode` / 电子病历页头）启用；挂号凭条保持无内嵌文字
  （下方独立「门诊号:」文字行，样式不同），无需改动。

---

## [7.30.0] - 2026-09-09

> 科室选择 Tab 智能记忆（登录按最后使用科室 / 切换转科按当前科室类型 / 空 Tab 通用隐藏）、处方笺双实线修复、条形码单号文字几何精确对齐。

### 新增
- **科室选择弹窗 Tab 智能记忆**（`deptpicker.js`）：
  1. 首次登录医生工作站：科室选择弹窗默认 Tab 按「最后使用科室」记录显示——
     最后用急诊 → 默认急诊 Tab；最后用外科门诊 → 默认门诊 Tab
     （`localStorage` 持久化、绑定账号，选择科室时自动写入）；
  2. 切换科室 / 叫号大屏：仍按当前科室定位本科室所在类型 Tab；
  3. 转科 / 发起会诊：默认进入患者当前科室所在类型 Tab
     （调用方传入 `currentType`——当前科室已被服务端排除，列表中找不到）；
     若该类型下已无其他科室（如急诊仅一个急诊科且被排除），该 Tab 已被隐藏，
     自动回落其他 Tab；
  4. 通用空 Tab 隐藏：某类型下没有科室（如医院未设置急诊科室）则该 Tab
     整个不渲染，不再显示「暂无XX科室」空态。

### 修复
- **门诊处方笺「调配 / 复核、发药」下方双实线**：`print_order.php` 处方分支在
  调配/复核发药后手动输出一条实线，而紧随其后的 `pt_doc_foot()` 自带一条横线，
  形成两条紧贴实线（其他文书直接用 `pt_doc_foot` 仅一条，故仅处方页存在）。
  删除多余独立横线，与其他文书版式统一。
- **条形码单号文字两端对齐**：单号文字直接内嵌条形码 SVG（`barcode128_svg`
  新增 `$withText` 参数）——SVG `<text>` 以 `textLength=黑条区宽度` +
  `lengthAdjust="spacing"` 在 viewBox 用户坐标系内强制文字渲染宽度精确等于
  条码黑条区宽度（首条左缘 → 末条右缘），文字与条码同为一个 SVG 整体等比
  缩放，任意显示尺寸/打印驱动下文字两端恒与条码左右边缘精确对齐（长单号
  收紧、短单号拉开字距、永不超出；黑条区每字符 11 模块恒大于 monospace
  字宽，仅拉伸不重叠）。零测量、零 JS，彻底消除度量基准差异、字体加载
  时序等不稳定因素；打印预览/只读预览/电子病历页头三端同构。
  `pt_barcode` 与电子病历页头条形码源启用内嵌文字，`emr.js` 移除外层文字
  节点；CSS svg 显示高 26→40px（条码部分仍 ≈26px 与医院名称齐平、文字
  ≈10.6px）。挂号凭条（居中带「门诊号:」前缀文字行）不在右上角文档条码
  范围，保持原样。
...
> 诊毕病历全链路快照封存强化：诊毕后知情同意书/诊断证明/诊断排序/护理文书等病历相关数据一律不可删改，前端隐藏删除/编辑按钮、后端硬拦截兜底。

### 新增
- **知情同意书诊毕封存（前端展示层同步）**：`emr_consent.js` 新增
  `isVisitFinished()`（就诊 `status=finished` 判定）与 `writeLockMsg()`
  统一锁定提示语（诊毕封存优先于跨科室只读）：
  1. 侧栏「知情同意书」列表项诊毕后不再渲染 🗑️ 删除按钮；
  2. 查看模态框诊毕后不显示「🗑️ 删除」与「✏️ 编辑」按钮（🖨️ 打印保留）；
  3. `del` / `delFromView` / `enterEdit` / `save` 全部写入口前端拦截兜底，
     提示「该患者已诊毕，病历已归档封存，知情同意书不可删改」。
  后端 `consent.php` save/delete 归档锁定此前已存在，本次补齐前端按钮隐藏与
  操作拦截，形成前后端双重封存。

### 安全
- **诊断证明诊毕封存**：诊毕后诊断证明（法律文书）一律不可删除——
  `record_read.php` 的 `certCanDelete` 后端权威计算增加 `status!=='finished'`
  判定（诊毕后大纲栏不再渲染删除按钮）；`record_cert.php` `certificate_delete`
  增加诊毕归档锁定，物理删除一律拒绝。归档补开证明（开具豁免）不受影响：
  可补开、可打印、不可删改。
- **诊断排序诊毕封存**：`record_save_diag_order.php` 增加诊毕归档锁定，
  补齐 `save_diags` 已有拦截之外的排序入口遗漏，诊毕后诊断聚合完全封存。
- **护士站诊毕封存**：`nurse.php` 三个写入口（`save_vitals` / `nursing_add` /
  `nursing_delete`）增加诊毕归档锁定——已诊毕就诊不可录入生命体征、不可新增/
  删除护理记录，与医生端 `record_save_vitals` 同口径。

### 说明
- 诊毕（`registrations.status='finished'`）后病历相关数据全链路封存现状盘点：
  病历文书保存/删除、诊断增删排序、生命体征、知情同意书增删改、开单提交/删除、
  会诊发起/接受/完毕/删除、诊断证明删除（补开除外）——前端按钮隐藏 + 后端
  `EmrContextResolver` 熔断/显式 `finished` 拦截双重保障，任何入口均不可删改。
...
### 修复
- **添加过敏史后再次点击打开模态框显示为空（需保存病历并刷新才正常）**：模态框保存只更新
  内存 `ALLERGY_HIST`（`ALLERGY_MODIFIED=true`），而患者主表 `patients.allergy_history`
  要等「保存病历」（`allergy_modified:1`）才同步；再次打开时 `openAllergyModal` 无条件从
  主表拉取，拿到旧值覆盖内存 → 弹窗空白。修复：`ALLERGY_MODIFIED=true` 时直接用内存最新
  值，不拉取主表；未修改过才拉取（保留护士侧皮试阳性等实时更新场景）。

### 说明
- 涉及文件：public/assets/js/components/emreditor.js。

---

## [7.28.14] - 2026-09-09

> 医生工作站科室胶囊仅在有会话选择时显示（未选隐藏，不再显示遗留默认科室）。

### 修复
- **首次进入医生工作站未选科室时，顶栏仍显示默认/遗留科室胶囊（如"外科门诊"）**：
  `doctor_tools.loadDepts` 之前会回退到持久化的 `users.current_dept_id`（上次遗留），
  无会话选择也显示。修复：`CUR_DEPT` 仅取本次会话记忆 `clinic_doc_dept`，未选择时保持
  0 → `renderDeptTitle` 将胶囊 `display:none` 隐藏，待医生显式选择后再显示。

### 说明
- 涉及文件：public/assets/js/components/doctor_tools.js。
...
### 新增
- **模板创建/编辑模态框（templates.php）**：模板正文等编辑区与病历区一致——右键屏蔽浏览器
  原生菜单；文本输入类（input/textarea/富文本字段）右击弹出自定义菜单
  （复制/剪切/粘贴/清空），其余区域（空白/勾选框等）一律屏蔽。
- **`emr_ctxmenu.js`**：`Clinic.emrMenu.show(ev, opts)` 新增可选 `opts.hideTemplate`——
  为 true 时嘱托字段（data-k="advice"）不显示「模板」菜单项（模板创建场景本就无需再选
  模板）；EMR 编辑页原有调用不变，嘱托字段仍保留「模板」项。

### 说明
- 覆盖全部模板类型（病历模板结构化编辑器 / 知情同意 / 护理 / 影像 / 嘱托 textarea）。

---

## [7.28.11] - 2026-09-09

> EMR 编辑控件 UI 统一：既往史下拉与过敏史按钮「否认」严格左对齐；所有可交互控件悬停统一 4px 微圆角。

### 修复
- **既往史下拉与过敏史按钮「否认」文字未左对齐（过敏史偏右约 8px）**：两控件盒模型不一致
  （字号 13vs14px、左内边距 6vs4px、外边距 0vs1px、固定高 vs 内容高），且过敏史行的空
  `.ef-cond` 条件包裹层作为 flex 子项多占一个 gap 把按钮推右。修复（`components-emr.css`）：
  1. `.ef-select` 统一为 14px / 行高 1.6 / 左内边距 4px / 外边距 0 1px / 内容撑开高，
     `vertical-align: baseline`——与 `.emr-item-link`（过敏史按钮）盒模型一致；
  2. `.ef-select-wrap` 去掉重复 1px 外边距；
  3. `.ef-cond` 改 `display: contents`——空条件包裹层不再产生 flex gap（过敏史按钮不再
     被推右），有内容（既往史=承认详情字段）时子项直接参与流式排列，显示不变。
- **编辑区可交互/可编辑项悬停背景圆角不一致**：`.ef-field`、`.ef-select`、`.ef-diag-item`
  统一 `border-radius: 4px`，与 `.emr-item-link`（过敏史按钮）一致。

### 说明
- 仅修改 `public/assets/css/components-emr.css`，不涉及保存/提交/数据结构逻辑。

---

## [7.28.10] - 2026-09-09

> A5 页脚下移并防止页脚挤到新页：内容区 187mm、页脚落在纸面 197mm（页脚下方留白约 13mm），sheet 增高余量恢复至 10mm——打印端正文略高时页脚不再被挤到一张空白新页。

### 变更
- **修复正式打印时页脚被挤到一张全新空白页顶部（7.28.9 引入）**：7.28.9 把正文预算提到
  189mm、sheet 最小高度提到 208mm 后，sheet 增高余量仅剩约 4mm——打印端正文比预览
  略高（实测约 5-6mm）即冲破 210mm 物理页高，页脚被浏览器分页到下一页，产生
  「第一页 + 页脚空页 + 原第二页」。
- `print.js`：分页正文可用高度基准 189mm → 187mm。
- `print.css`：打印 sheet 最小高度 208mm → 202mm（下边距 9→5mm），预览 sheet 下边距
  11mm → 13mm——页脚位置一致（纸面 197mm，比 7.28.8 的 189mm 低约 2 行），页脚下方
  留白约 13mm；sheet 从 202mm 长到 210mm 物理页高仍有约 10mm 增高余量（与 7.28.8
  已验证安全值一致），打印端正文略高时页脚不挤新页、签名不裁剪。

---

## [7.28.9] - 2026-09-09

> A5 页脚位下移尝试（未提交版本，已被 7.28.10 修正）：页脚压至纸面 199mm 后 sheet 增高余量不足，打印端正文略高即把页脚挤到新页。最终方案见 7.28.10。

---

## [7.28.8] - 2026-09-09

> A5 底部安全缓冲：正文区上移、页脚距纸底留 21mm——打印端正文比预览高出的偏差先被缓冲吸收，医生签名/页脚不再被打印机硬件边距裁掉。

### 修复
- **真实打印时页面底部医生签名被裁掉/消失（预览正确）**：预览与打印渲染存在约一行的
  高度偏差（打印端正文比预览略高）。此前页脚/签名距纸底仅约 13mm，打印端正文一高出
  即把签名顶进打印机硬件不可打印区被裁掉。修复（预览与打印同步调整，保持一致）：
  1. `print.js`：正文可用高度基准 186.5mm → **179mm**（对应打印 sheet 内容区 179mm）；
  2. `print.css`：打印 sheet 最小高度 209mm → **202mm**（页脚落在纸面 189mm 处，
     距纸底 210mm 留 **21mm 安全缓冲**）；预览 sheet 下边距 14mm → **21mm**（版心
     179mm 与打印一致，页脚位置一致）。
  净效果：打印端正文高出约一行（含 DPI 取整与硬件边距）时，先被 21mm 缓冲吸收，
  签名/页脚完整打印；仅极端超高才触发浏览器自然续页（内容永不丢失）。

---

## [7.28.7] - 2026-09-09

> A5 预览与打印同构：预览页面随内容自然增高、永不裁剪——预览和真实打印都不再丢内容；
> 升版本号强制浏览器刷新 print.js/print.css 缓存（此前 7.28.6 多轮改动共用同一版本号，
> 浏览器可能一直加载旧资源导致预览/打印行为错乱）。

### 修复
- **打印预览与真实打印仍不一致（预览丢【是否留观】/打印丢医生签名）**：预览 sheet 原为
  固定 210mm + overflow:hidden，内容比预算高出几像素即被裁掉；而打印端 sheet 已改为
  随内容增高不裁剪，两者行为不对称、同一内容两端表现不同。修复：
  `.a5-sheet`（屏幕预览）由固定高度+裁剪改为「最小 210mm、随内容自然增高、不裁剪」，
  与打印端完全同构——内容恰好排满时页面固定 210mm，渲染的细微高度差异由页面自然
  增高吸收，**预览与打印都不再丢正文**；`.a5-body` 同步放开裁剪。

### 说明
- 若此前多轮测试后页面行为仍错乱，请先强制刷新（Cmd/Ctrl+Shift+R）再打印。

---

## [7.28.6] - 2026-09-09

> A5 打印根治：页边距改由 sheet 内部 padding 呈现，打印文字区与预览逐像素一致——真实打印不吞行、换行点一致、底部无空白。

### 修复
- **电子病历/知情同意书真实打印与屏幕预览不一致（根因修复）**：预览与打印走浏览器的
  两条独立渲染管线（屏幕 GPU 96dpi vs 打印光栅化按打印机 DPI 逐像素取整），同一份
  HTML 打印端文字行高/换行点与屏幕测得略有偏差。此前打印页边距依赖
  `@page { margin:10mm }`——驱动实际可用区一旦与它不同，打印端每行可容纳字数变化、
  换行点漂移、整页行数变多，末尾文字被 sheet 的 overflow:hidden 裁掉（病历末行只
  显示一半）。修复（三层）：
  1. `@page` 边距归零（print.js 注入 `size:A5 portrait; margin:0`），页边距改由
     `.a5-sheet` 内部 padding（10/10/13mm）呈现——打印文字区宽高（128×186mm）由
     代码钉死，与预览/测高容器逐像素一致，换行点不再随驱动可用区漂移；驱动硬件
     不可打印区只会裁掉外围白边、永远碰不到正文。
  2. 打印 sheet 高度改为「最小 209mm、随内容自然增高、不裁剪」（原固定 187mm+
     overflow:hidden）：打印端文字比屏幕测得略高时先长入底部 13mm 白边区，
     再超出才触发浏览器自然续页——**内容永不丢失**；`.a5-body` 同步放开裁剪。
  3. 预览 `.a5-sheet` 下边距 10→14mm，版心高与打印 sheet 内容区（186mm）严格一致：
     预览页脚位置 = 打印页脚位置，正文排满、底部无空白行；打印字体栈显式固定为
     系统 PingFang 等，避免个别环境回退其他中文字体导致字宽变化。

### 说明
- 预览与真实打印的“所见即所得”受浏览器屏幕/打印双渲染管线制约：打印端文字经打印机
  DPI 逐像素取整，个别中文换行点可能相差一字，属渲染介质固有差异；本版保证页面
  完整不吞行、底部无空白、页脚位置与预览一致。

---

## [7.28.5] - 2026-09-08

> A5 分页余量回调：修复 7.28.4 余量归零导致的打印吞行，保留页尾悬挂记账收益。

### 修复
- **电子病历打印最底部一行被吞（7.28.4 引入）**：基准提至 187mm 足额 + 余量归零后，
  used 记账未覆盖打印引擎与屏幕渲染的系统性累计舍入差（整页 20+ 行逐行亚像素取整，
  打印介质下约一行）——打印实际高度超出 sheet 被 overflow 裁掉。修复：基准回调
  186.5mm（0.5mm 结构保护）+ 安全余量 8px（覆盖打印累计舍入）。

### 变更
- 保留 7.28.4 的页尾悬挂 marginBottom 记账（每页回收 3~10px 纯空白）。净效果：
  比历史版本（184mm+14px）每页仍多约 15px（约半行），打印安全、底部留白收敛。

---

## [7.28.4] - 2026-09-08

> A5 分页填充率再提升：每页底部可再多排约一行文字（知情同意书/电子病历等全部 A5 文书受益）。

### 变更
- **分页可用高度三处系统性留白回收**（`print.js`，判定与拆分统一以「真实占用」为基准，
  不裁字前提下尽量贴满页底）：
  1. 基准 186.5mm → **187mm 足额**（打印纸张内高，0.5mm 舍入差由 Math.floor 吸收）；
  2. 安全余量 4px → **0**（缓冲职责移交页尾悬挂记账）；
  3. **页尾悬挂 marginBottom 记账**：页面最末节点的下外边距视觉上是纯空白不占版心，
     但逐节点累加时被计入 used——改为以「真实占用 used−hangMb」判定与拆分，每页
     回收一个页尾 margin（约 3~10px，视内容而定）。
  合计每页回收约 13~20px ≈ 半行至一行，配合行级二分拆分正文可紧贴页底。

---

## [7.28.3] - 2026-09-08

> 知情同意书 A5 分页空白回退修复（撤销末页签名区错误预留，对齐电子病历分页模式）。

### 修复
- **每页下部约 1/3 空白（7.28.1 分页改动引入的回退）**：「末页签名区预留」对最后一个
  正文节点生效，而知情同意书正文是一段超长 print-split 段落——它跨页拆分后落到
  哪一页，哪一页就被整页扣除签名区高度（约 160px），造成每页下部大面积空白。
  修复：对齐电子病历分页模式（同一分页器、签名即普通正文流节点、从不预留）——
  1. print.js 撤销 isLastBody/effAvail 末页预留与空页死循环兜底；
  2. print_consent.php 签名区去掉 print-foot-sec 特殊标记（仅知情同意书使用、
     无 CSS 依赖），保留 page-break-inside:avoid 防从中间拆开；
  3. **保留 7.28.1 的有效收紧**（186.5mm 基准 + 4px 余量 + 页脚 margin-top 精确
     计入）——每页填充率优于最初版本且不再有大面积空白。
...
### 修复
- **文书标题显示「通用知情同意书」**：模板保存曾强制写入 content.name='通用'，
  开具时标题推导优先取 name 所致。修复：模板保存不再写入 name 键（content=
  {content,notice,sections}），开具标题一律取模板名称原文；迁移 v30 清理存量模板
  废弃 name 键（版本号 29 → 30）
- **编辑已开具文书无提示**：由内嵌提示条改为点击「✏️ 编辑」先弹出确认框
  （「编辑并保存后，正文内容与病情介绍快照将随当前病历更新重新固化，历史打印内容
  不再变化」），确认后进入编辑态
- **A5 分页每页底部 3-4 行留白**（影响知情同意书等全部 A5 文书）：分页器预留过大
  ——184mm 基准 + 14px 安全余量，且 .a5-foot 的 margin-top:8px（offsetHeight 不含
  外边距）未计入。修复：基准收紧为 186.5mm + 4px 余量 + 页脚边距精确计入；
  **末页签名区改为分配正文时即预留**（原为填满后放不下触发超长兜底），不可拆分
  整节点遇预留挤压时直接放置置 over 防死循环。勾选不同病历内容节导致的空间占用
  变化由逐节点测高分页动态吸收

### 变更
- **模态框移除「文书标题」只读输入框**：模态框标题即文书标题（新建取模板名称原文、
  查看取开具时固化标题）
- **区域顺序统一**（模板管理表单 / 审核预览 / 开具编辑模态框）：病情介绍显示内容 →
  正文内容 → 告知内容（告知内容显示在正文内容下方）

---

## [7.28.0] - 2026-09-08

> 知情同意书升级为「知情同意/告知文书」：标题完全自定义 + 告知内容可配置 + 病历内容勾选固化快照。

### 新增
- **文书标题完全自定义**（需求 1）：不再强制「XX知情同意书」——模板名称即文书抬头
  （门诊告知书 / 病重通知书 / 手术知情同意书等任意告知文书），开具后按原文显示；
  旧模板（content.name 非空）兼容旧逻辑 name+知情同意书。涉及模板管理表单、
  开具/编辑模态框、打印抬头与页脚提示语泛化
- **告知内容可配置**（需求 2）：模板表单与开具/编辑模态框均提供「告知内容」输入框，
  显示于打印签名区上方；占位符为默认话术（患者/委托人已知晓…配合治疗），留空保存
  默认话术，手动输入即替换；开具时固化随文书打印
- **病情介绍显示内容复选框**（需求 5）：模板表单与开具/编辑模态框支持按
  主诉/现病史/既往史/过敏史/主要症状/生命体征/意识状态/体格检查/初步诊断
  勾选展示（默认主诉+初步诊断与旧行为一致），打印按勾选节渲染、空内容自动不显示
- **病历内容快照固化**（需求 5）：开具时以首诊文书为锚点将勾选节文本固化至
  consents.emr_snapshot（迁移 v29 新增 notice/emr_snapshot 列），后期病历修改不影响
  已开具文书；**编辑重存时随当前病历重新快照**；打印优先用快照，旧数据（无快照）
  回退实时投影（旧行为兼容）
- 编辑已开具文书时显示提示条（需求 4：编辑态显示、查看态隐藏——保存后随当前病历
  重新固化，历史打印不再变化）

### 变更
- **开具/编辑模态框移除「就诊科室」只读展示**（需求 3）：就诊科室由后端开具时自动
  固化（dept_id/dept_name），无需人工查看占位
- **A5 分页时机动态精确**：病情介绍由单一大节点改为按勾选节【逐节独立节点】渲染
  （print_common pt_sec），分页器逐节点测高分配——勾选节增减/空节过滤时分页自然
  续页，正文 print-split 文本流与签名区预留机制不变
- 默认告知话术与 9 节白名单收敛为服务端单一来源（consent_default_notice /
  consent_section_keys，emr_formatter.php），模板/开具/打印三处共用
...
### 修复
- **打印病历存储型 XSS（安全）**：病历正文（主诉/现病史/既往史/过敏史/查体/诊断/意识状态/生命体征/
  留观/嘱托等）在打印 HTML 输出层统一转义（`print_record.php`），杜绝医生写入的
  HTML/脚本在打印预览中执行
- **就诊序号并发重复**：`next_visit_seq` 由 COUNT+1 改为事务内 MAX+1，并新增
  `registrations(first_dept_id, date(registered_at), visit_seq)` 唯一索引
  （迁移 v25 存量去重 + v26 唯一约束）
- **业务单号 TOCTOU 竞态**：申请单号/会诊单号/证明号新增唯一索引（迁移 v27 去重 +
  v28 唯一约束），订单/会诊/证明三处插入改为「唯一冲突重新生成单号重试（最多 3 次）」
- **MySQL 双驱动兼容**：候诊队列可见天数过滤与心跳保活检测去除 SQLite 专有语法
  （date 修饰符 / strftime / 标量 MAX MIN），改为按驱动生成方言；心跳注释修正
- **退费部分竞态**：退费资格判定移入事务内，未审批退费要求全部明细均成功置为
  refunded（affectedItems === count(items)），并发登记/发药导致的半退状态整单回滚

### 变更
- **前端大型文件拆分（低风险部分，行为不变，撤销/右键菜单等功能不受影响）**：
  `emr.js` 由 3472 行降至 2603 行：
  - 诊断证明模块 → `emr_cert.js`（`Clinic.emr.cert`，约 150 行）
  - 科室间会诊模块 → `emr_consult.js`（`Clinic.emr.consult`，约 320 行；顺带将
    `startConsult` 暴露到公共 API，修复「确认会诊」按钮潜在未定义）
  - 初步诊断模块 → `emr_diag.js`（`Clinic.emr.diag`，约 440 行；`clampPop` 跨模块
    共享改由 `_ctx.clampPop` 桥接）
  - 拆分遵循既有 `_ctx` 共享上下文先例：emr.js 保留同名桥接函数，内部调用与
    公共 API 语义不变；layout.php 与 nav.js（SPA 懒加载栈 + isLoaded）双路注册
  - 诊断证明弹窗内自定义 esc 收敛为 `Clinic.escHtml`（补转义单引号）
- **暂缓（高风险的 emr.js 后续拆分）**：左侧大纲栏（`renderLeftNav` 全文件约 15 处
  调用）、权限/打印门控（与 `emr_rules.js` 存在加载顺序依赖）——待本次改动测试稳定后
  再按需拆分
...
> 病历右键菜单新增撤销、嘱托模板（order_note）落地、模板应用交互统一（覆盖/续写/关闭）。

### 新增
- 病历输入框自定义右键菜单新增 **撤销**：字段每次输入/清空/粘贴前自动记录旧值
  （每字段独立历史栈，上限 50 步），误删内容可逐级回退恢复（`emreditor.js` 撤销
  历史 + `Clinic.emrEditor.canUndo/undo`；`emr_ctxmenu.js` 菜单项）
- **嘱托模板（order_note）** 全流程落地：
  - 后端开放医生角色 order_note 类型权限，保存内容为单段正文 `{ content }`
    （`api/template.php`，个人免审 / 科室全院经管理员审核）
  - 医生/管理员「模板管理」页启用「病历嘱托模板」Tab：新建/编辑为 名称+适用范围+
    嘱托正文 textarea（`templates.php`）；审核中心预览按纯文本展示
    （`admin/review.php`），审核结果标签按实际模板类型显示（`admin_audit.php`）
  - 病历嘱托字段右键「模板」弹出选择模态框：左侧搜索 + 模板列表，右侧预览内容，
    覆盖 = 清空后完全按模板填充 / 续写 = 保留原有内容、模板内容插入到后面 / 关闭
- **模板应用交互统一**：护理记录（`nurse/dashboard.php`）与影像报告
  （`imaging/dashboard.php`）模板列表由「点击即填入 / 二级替换追加弹窗」统一为与
  嘱托模板一致的「左侧列表选中 → 右侧预览 → 覆盖 / 续写 / 关闭」交互

### 修复
- 病历输入框内容被清空后撤销基线同步：新建字段初始化基线为空、程序化填充（set()）
  后同步基线，保证「撤销」可正确回退到用户编辑前状态

---

## [7.25.1] - 2026-09-08

> 病历字段清空后占位提示语不重新显示的修复。

### 修复
- 病历输入框（contenteditable 字段）将文字全部删除后，占位提示语（如
  【请填写辅助检查结果】）不重新出现：浏览器在 contenteditable 内文字删净后
  会残留 `<br>` 子节点，导致 `.ef-field:empty::before` 的 `:empty` 不再匹配。
  现于 input 事件检测「无实际文字但仍有子节点」时清空子节点，占位提示语
  按输入规则（空字段才显示）恢复。（`public/assets/js/components/emreditor.js`）

---

## [7.25.0] - 2026-09-08

> 病历输入框自定义右键菜单（替代原生菜单）+ 下拉区域右键漏网之鱼封堵。

### 新增
- 病历输入框自定义右键菜单组件（`emr_ctxmenu.js`）：替代浏览器原生右键菜单，
  普通输入框提供 **复制 / 剪切 / 粘贴 / 清空** 四项操作；嘱托输入框
  （`data-k="advice"`）额外提供「模板」项（预留，后期完善，当前点击提示建设中）
- 菜单样式走主题 CSS 变量，自动适配明亮 / 夜间主题；fixed 视口定位四边夹紧，
  点击菜单外部 / Esc / 滚动自动关闭；粘贴自动转为纯文本（与病历字段既有粘贴规则一致）

### 修复
- 病历编辑页下拉区域（`select`，如「请选择」占位）右击仍会弹出浏览器原生菜单的
  漏网之鱼：右键放行范围由「input/textarea/select/contenteditable」收窄为仅
  「input/textarea/contenteditable」，下拉与其余区域一律屏蔽原生右键菜单

---

## [7.24.0] - 2026-09-07

> 下拉交互反馈、占位符收敛、科室类型精简、表单控件主题色与下拉宽度自适应。

### 新增
- 自定义下拉宽度自适应：弹层宽度下限与触发 select 同宽、按最宽选项内容收缩
  （如主诉单位 1-2 字不再撑成 160px），上限 340px
- 各表单下拉补搜索 / 清空：检验检查项目所属分类、药品分类/包装单位/剂型/频次/途径、
  用户管理学历/学位/职称/职务、挂号职业、个人中心学历/学位

### 变更
- 下拉/纸面字段交互反馈：病历纸面字段悬停与打开态显示浅蓝底（ef-field:focus 同款），
  普通表单/开单组件仅主色边框（不改蓝色底）
- 科室类型下拉仅显示门诊/急诊（编辑既有医技/其他科室时保留其类型选项防误改）
- 占位符（请选择/请填写/单位/用药频次 等）默认不进候选列表（data-csd-keepempty=1 可保留）

### 修复
- 病历纸面下拉悬停误用 background 简写致右侧箭头消失 → 改 background-color 保留箭头

---

## [7.23.0] - 2026-09-07

> 全站自定义下拉升级（搜索/清空/按接口开关）+ 表单控件主题色 + 模板类型权限隔离 + 病历必填校验强化。

### 新增
- 自定义下拉组件 v2：弹层顶部按 select 的 `data-csd-search` / `data-csd-clear` 显示搜索栏与清空 X
  （两开关独立，按调用接口决定）；搜索按选项文本实时过滤；清空 X 置空并派发 change
- 开单处方频次/途径下拉：搜索 + 清空，占位【用药频次/使用途径】空值不进候选列表；
  药品数据自动回填、缺失回退占位
- 意识状态下拉、主诉/现病史单位下拉、供史者、来院途径：按需加搜索 / 清空（占位不进列表）

### 变更
- 表单控件主题色：复选框改为自定义主题样式（appearance:none，边框/勾选随主题变量），
  数字输入隐藏 WebKit 黑色上下箭头（Firefox/Edge 由 accent-color 上色），单选/范围 accent-color
- 模板类型-角色权限隔离（前后端双重）：医生仅病历/知情同意书，护士仅护理记录，影像仅影像报告，
  管理员全类型；/api/template list/get/save/delete 均校验防伪造 type

### 修复
- 主诉/现病史必填校验强化（前端 save 与后端 record_save 同步）：
  主诉=主要症状/持续时间/时间单位；现病史=具体内容/发病时间/时间单位/供史者/来院途径，任一为空判定未完善
- 处方提交必填：剂量/频次/途径（前端 submit + 后端 order_submit 兜底，子医嘱剂量必填）
- 修复双击 select 仍弹原生下拉（收起分支漏 preventDefault 的根因）
...
### 修复
- 护士站处置↔医嘱联动执行（一并执行）后，关联方区块状态未实时刷新（被点击方即时更新、
  关联方仍显示执行中，需手动刷新）：新增 refreshNurseSections 一次拉取同时重建多个区块，
  联动执行后处置与医嘱两侧状态同步更新为已完成/已执行

---

## [7.21.0] - 2026-09-07

> 医技工作台局部刷新、处方排序与进度数据化修正 + 未缴费状态补全。

### 修复
- 药房处方进度「缴费」步骤改为按真实缴费状态判定（open 待缴费显示「待缴费」而非绿色已完成）
- 医技工作台开单明细按开单先后排序（最早开单在上），药房/检验/影像按就诊先后顺序处理
- 护理/检验/检查/药房工作台补全 open（待缴费）状态：护士站过滤未缴费明细，检验/检查不再把
  未缴费项目误入「已完成」展示，药房状态徽章不再裸显英文
- 打印申请单表头 th 统一 white-space:nowrap（序号不再被挤成「序/号」两行拉高表头）

### 变更
- 药房/检验/影像工作台操作（审方/发药/登记/提交报告/撤回）改为 ajax 局部刷新：
  仅重建受影响区块 + 右栏 + 候诊数，不再整页重载（保持滚动位置）
- 明细行补充 order_id 混淆串（局部刷新定位所属申请单）

---

## [7.20.0] - 2026-09-07

> 病历处方展示与皮试处方排序修正 + 打印表头防换行。

### 修复
- 病历右侧处方列表：同一皮试开单拆出的皮试处方/正式处方同单号序调整（先建皮试再建正式，
  如已有 1-4 张处方时新皮试开单显示为 处方5=皮试、处方6=正式，不干扰其他处方相对顺序）；
  撤销上一版全局置前排序；处方导航保留「皮试」徽标区分
- 病历正文处方行整体应用 emr-item-link 样式（药名 + 剂量/频次/途径/×数量 含全角空格为一个
  整体可点击标签），子药行（├─/└─ 缩进）同理；撤销仅加粗后段的临时方案
- 打印申请单表头 th 统一 white-space:nowrap，修复「序号」被挤成「序/号」纵向两行拉高表头
  （处置/检验/检查申请单共用同一表头，一处修复全部覆盖；检验报告单表头原有 nowrap 不受影响）

### 变更
- 医技工作台右侧栏申请单展开子项目缩进 30px → 10px（更紧凑）

---

## [7.19.0] - 2026-09-07

> 皮试全流程改造：需皮试药品开单拆分、缴费钳制、护士联动执行与皮试结果闭环；处置/医嘱联动执行。

### 新增
- 需皮试药品开单自动拆分：皮试处方（皮试量/皮内注射/护士站执行）+ 正式处方（原量原途径）
  两个独立处方单号；皮试处置单独一张处置申请单、途径绑定处置另一张（共 4 单号）
- 缴费钳制：正式处方/处置在皮试阴性前不可缴费（收费处列表 🔒 禁用置灰 + pay_orders 后端硬拦截）
- 护士站处置↔医嘱双向联动执行（一并执行/单独执行/取消确认框）
- 皮试结果记录（阳性/阴性）：阴性自动解锁正式处方/处置缴费资格；阳性自动将该药品追加至患者
  过敏史（医生过敏史模态框实时拉取显示）
- 处置/医嘱执行完成后显示执行人 + 执行时间
- schema v24：orders.is_skin_test 标记 + skin_test_results 皮试结果表

### 变更
- 皮试单据标题统一为「门诊处方笺 / 门诊输液（注射）笺」（不再使用皮试专属标题）
- 右侧栏申请单展开箭头靠右对齐（为左侧内容留空间）；空白工作台右侧栏病历摘要在前

### 修复
- patient_allergy_append 中文分隔正则补 /u 修饰符（防按字节切分损坏既有过敏史）
- visit_detail 未缴费明细投影补齐皮试锁定字段（locked/order_no/locked_reason）

---

## [7.18.0] - 2026-09-07

> 药房审方/发药拆分 + 需皮试药品打印加单 + 护理工作台摘要版式优化。

### 新增
- 药房审方与发药拆分为两个独立步骤：审方通过置「待发药」（记录审方人），发药单独执行
  （记录发药人），审方人与发药人可为同一人或不同人；处方进度条分步展示两操作人
- 需皮试药品打印时前置「皮试处方」（途径改为皮内注射）+「皮试注射笺」（护士站执行时），
  常规处方/输液笺不变（单个需皮试的护士站执行药品 → 皮试处方 + 皮试注射笺 + 处方 + 输液笺 共 4 张）
- 医技工作台通用患者横条新增「📋 病历」按钮（护士/检验/检查/药房复用），点击弹出完整病历打印预览

### 变更
- 药房候诊页签：待审方 / 待发药 / 已发药 / 当日；队列按订单状态聚合
- 取药凭条（处方提示）仅在发药完成后显示（后端 rx_slip 仅 dispensed 放行）
- 护士站待执行医嘱仅在药房已发药后可执行（审方通过未发药显示「待药房发药」）
- 护理记录单病历摘要移至护理记录之前，改为纵向排列；右侧栏顺序同步（病历摘要→护理记录→生命体征→待办）
- 护理/医技工作台病历摘要初步诊断仅显示诊断名称（不再显示 ICD10 编码）
- 右侧申请单号整行点击展开/缩回项目明细（去掉「+」号，箭头 ▾/▸ 指示），默认展开并向右缩进一层
- 已发药处方退费：审批流放行后订单状态硬拦的「退费死锁」修复，退费库存恢复口径对齐

### 修复
- 已发药处方退费死锁（审批放行后 orders.status='dispensed' 无法退费）
- 药房编辑已审核药品绕过审核流（可篡改价格/库存影响计费）
- 检验/影像结果录入缺科室归属校验；再挂号覆盖既有患者姓名；护理记录删除无创建人校验
- 会话角色/科室快照不实时刷新（降权后既有会话仍保有原权限）
- 开单/审方/报告提交无防重入锁（双击重复建单/重复发药/重复生成报告）
- 候诊轮询与叫号/排队悬浮窗拖动监听泄漏（后台空轮询与内存累积）
- 开单/确认弹窗、诊室名渲染 XSS 转义（存储型）

### 重构
- 医技工作台抬头/状态徽章/申请单大纲、生命体征悬浮窗（Clinic.vitals）、候诊面板基础设施
  （queuepanel_core）、患者查询/退费取消挂号、打印报告快照回退、管理端列表等 13 处重复代码收敛
- 缴费凭条逐项执行流程抽公共 item_flow_steps；emr.js 内部等价收敛

### 文档
- README 目录结构 js/components 补充 vitals / queuepanel_core / admin_items

---

## [7.17.0] - 2026-09-07

> 整体复盘优化：修复 11 项后端逻辑/权限漏洞、6 项前端健壮性问题，收敛 13 处重复代码
> （低冗余、高解耦，行为与既有功能完全一致）。

### 修复
- 药房编辑已审核药品须重新走管理员审核流（防止直接篡改已生效药品价格/库存影响计费）
- 修复已发药处方退费死锁：审批放行后订单状态硬拦（orders.status='dispensed' 无法退费）；
  退费库存恢复口径同步对齐（dispensed 明细一并恢复，rejected/cancelled 不重复恢复）
- 禁止物理删除已退费订单（保留财务与病历追溯，前后端口径一致）
- 检验/影像结果录入（save_draft/save_result）补充科室归属校验，与登记环节口径一致
- 再挂号不再覆盖既有患者姓名（姓名/性别/出生日期锁定，与注释及患者档案修改口径一致）
- 护理记录删除校验创建人本人（管理员兜底），防止护士删除他人记录
- 会话角色/关联科室实时刷新：管理员降权/移除科室后既有会话即时生效，杜绝权限残留窗口
- 缴费批次强制同一就诊（禁止跨患者合并缴费凭条，避免退费批次整单错乱）
- 管理员退费代审批生效：非审批人时对其余全部待审行统一生效，意见不再被静默丢弃
- 护士站生命体征录入补齐生理区间校验（与医生端 record_save_vitals 同规则）
- 开单数量上限校验（处方单明细 ≤999、检验/检查/处置 ≤99，与前端输入框上限一致）
- 候诊轮询定时器离开病历页自动停止（消除后台空轮询）；叫号/排队悬浮窗拖动监听
  具名句柄成对清理（防反复开关累积泄漏）
- 开单/审方/报告提交防重入锁（双击不重复建单/重复审方发药/重复生成报告）；
  删除影像工作台死代码 doImgSave（读取从未渲染的 DOM，一旦调用即抛错）

### 重构
- 医技工作台抬头（headHtml）、状态徽章（statusBadge）、右侧申请单/处方大纲
  （renderOrderSide）收敛为 deptwork 公共组件（护士站/检验/影像/药房共用）
- 生命体征录入悬浮窗抽离为公共组件 Clinic.vitals（医生工作站与护士站共用）
- 打印报告「快照回退 + 患者信息 + 诊断回退」抽 pt_report_context；报告格抽 pt_cell 参数化
- 缴费凭条逐项执行流程抽公共 item_flow_steps（与整单 order_flow_steps 区分）
- 退费/取消挂号抽 Clinic.cashier.cancelVisit；患者查询抽 Clinic.patientSearch
- 候诊面板基础设施（定位/外部点击Esc关闭/高度钳制）抽 queuepanel_core 公共组件
- emr.js 续写默认骨架与只读段记录形状等价收敛（cleanProgressEmr / readOnlyRecShape）
- 零散 helper（pad3/money/nl2br/escHtml）收敛 Clinic 全局，消除多处重复实现
- 管理端检验/检查/药品列表的「分类 tab 构建/过滤/计数/分类管理/删除/深链」
  收敛 admin_items 公共组件（新组件经 nav.js adminItems 栈按页加载）

### 安全
- 开单删除确认、转科确认、诊室切换确认等弹窗中的库内名称统一转义（防存储型 XSS）；
  诊室列表由内联 onclick 改为 data 属性 + 事件委托（杜绝属性注入）

### 文档
- README 版本徽章更新至 v7.17.0；目录结构 js/components 补充 vitals、queuepanel_core、
  admin_items 组件说明
...
> 药房工作台优化：抬头急诊病历版式 + 按处方号整体审方。

### 变更
- 门诊处方笺抬头：医院名称 + 第二名称 + 门诊处方笺 + 患者信息两行
  （姓名 性别 年龄 出生日期 / 患者ID 流水号 首诊科室 首诊时间），急诊病历版式两端对齐
- 正文按处方号为一个整体：处方号可点击弹出通用打印预览（该处方全部处方笺，
  不含门诊输液注射笺）；状态进度条 开单→缴费→审方（点进患者即「审方中」）→发药/拒绝
- 右栏处方号前加「+」展开该处方全部药品名称
- 打印 order 接口新增 exclude_inject 参数：药房预览不含护士站输液笺

### 修复
- 修复药房审方通过后子药仍为「待发药」：audit 通过改为遍历全部待发药明细
  （含子药）按各自 is_nurse 发药/转交护士站，不再只更新主药
- 药房候诊列表明细去掉「执行中」：发药完成（dispensed + 已转交护士站 dispensing）统一显示
  「完成」，执行属护士站范畴与药房无关；全部护士站执行的处方也归入「完成」列表
- 存量数据修复：order 846 子药随主药置为转交护士站状态

### 文档
- README 全面更新：医技科室工作台功能说明细化（护理记录模板 / 检验草稿暂存与横向 A5
  报告 / 影像去写报告模板与 A4 报告 / 药房处方号整体审方）；功能表新增「医技报告打印」
  「模板系统」行；目录结构表数更新为 39 张

---

## [7.15.0] - 2026-09-07

> 影像检查报告单版式调整 + 影像报告模板系统。

### 变更
- 检查报告单抬头改为急诊病历版式：医院名称/第二名称两端对齐，标题显示在医院名称下方
  （不再与医院名称同行，检验报告单样式专属）
- 检查报告单标题动态化：按检查分类显示「XX检查报告单」（CT检查报告单 / DR检查报告单 /
  超声检查报告单…）
- 检查报告单文字字号随 A4 增大（页眉 13px、所见/诊断 14-15px、页脚 12.5px）

### 新增
- 影像报告模板系统（type=imaging_report：影像所见 + 影像诊断）：影像科左侧新增「影像模板」
  栏目；个人免审，科室/全院进审核（独立审核类型 imaging_template），审核期间仅本人可用，
  驳回自动降级为个人；管理员可管理全部模板
- 影像工作台书写报告改为「✍️ 去写报告」模态框：左侧模板搜索列表 + 右侧影像所见/影像诊断；
  点击模板弹出「替换 / 追加」选择（替换=直接替换影像所见全部内容，追加=另起一行顺延保留当前内容）

### 修复
- 检查报告单标题动态前缀：除申请单 category_name 外，增加检查项目分类回退
  （reports 新增 category_name 快照列，schema v22；申请单为空时取检查项目分类，
  如 DR（数字化X线）检查报告单）
...
> 影像检查报告单 A4 版式 + 检验/影像右侧申请单号展开项目。

### 新增
- 检查报告单（影像）A4 纵向固定画布：页眉 4×3 隐形表格（姓名/性别/年龄/出生日期，
  患者ID/申请科室/临床诊断/报告单号〔自动缩放单行〕，第三行合并显示检查项目逗号连接自动换行）；
  正文影像所见 2/3 + 影像诊断 1/3；页脚 3×3 表格（申请医生/报告医生/审核医生〔预留〕，
  申请时间/检查时间〔登记时间〕/报告时间，第三行前两列合并提示语 + 末列页码靠右）
- 检查报告快照固化（复用 reports 快照列：申请科室/医生/临床诊断/申请/检查时间）
- 检验科/影像科右栏申请单号前加「+」，点击展开该单全部项目（+↔−）

---

## [7.13.0] - 2026-09-07

> 影像科工作台优化：报告单抬头参照急诊病历版式 + 整张申请单统一登记/书写。

### 变更
- 影像诊断报告单抬头：医院名称 + 第二名称（如有）+ 影像诊断报告单 + 患者信息两行
  （姓名 性别 年龄 出生日期 / 患者ID 流水号 首诊科室 首诊时间），参照急诊电子病历
  版式（两端对齐），抬头宽度与下方记录区块等宽
- 检查登记改为整张申请单统一登记（register_order）；报告按申请单号组合书写，
  申请单号可点击预览检查申请单，右栏大纲按申请单分组

---

## [7.12.0] - 2026-09-06

> 检验科报告单版式细节优化（与护士站优化区分版本号）。

### 修复
- 修复检验报告单预览时患者信息与页脚空白：`paginateLabReport` 分页选择器仍引用旧的
  `.lr-patlines`/`.lr-meta*`/`.lr-tip` 类名（改名 `.lr-patgrid`/`.lr-footgrid` 后未同步），
  导致患者信息与页脚未被克隆进每张画布；现抬头/患者/页脚完整渲染
- 修复打印输出为两张 A4 纵向纸：检验报告单设置 `@page { size: 210mm 148mm }` 横向 A5
  打印纸张，与画布同尺寸；预览两侧改为透明底（同 A5 病历预览风格）
- 修复打印后出现两页 A5 横向纸且内容溢出、页脚第二三行丢失：补全 `@media print` 固定
  画布尺寸（横向 A5，页间强制分页、最后一页不追加），分页计算预留 5px 安全余量，
  保证预览与打印渲染一致、单页不溢出
- 修复患者信息上方重复显示医院第二名称：抬头仅医院名称 + 第二名称两端对齐 + 检验报告单一处

### 变更
- 患者信息改为隐形 2×4 表格（两行 8 字段），靠左对齐
- 页脚改为隐形 3×3 表格：申请/检验/报告时间 + 申请医生/检验者/审核者，
  第三行前两列合并显示提示语、末列页码靠左；报告时间靠右，其余靠左

### 变更（7.12.0 续）
- 患者信息第二行字段调整：患者ID 申请科室 临床诊断 报告单号（去掉申请医生）
- 页脚第二行改为 申请医生 / 检验者 / 审核者；第三行前两列合并显示
  「检验结果仅供临床诊疗参考，仅对送检标本负责！」+ 末列 第x/共x页（靠左）
- 报告单号自动缩放字号：始终单行显示，超出列宽时逐级缩小至刚好放下（fitReportNo）；
  患者信息网格列改为 minmax(0,1fr) 防被长内容撑宽

---

## [7.11.0] - 2026-09-06

> 检验报告单打印全新版式（横向 A5 固定画布）+ 检验结果录入体验优化。

### 新增
- 检验报告单独立抬头：医院名称 + 检验报告单同一行整体居中，第二名称两端对齐
- 患者信息两行：姓名 性别 年龄 出生日期 / 患者ID 申请科室 申请医生 临床诊断
- 横向 A5 固定画布（210×148mm）：每张报告单大小恒定，内容超出自动分页，每页一致
- 列头与智能分列：两条实线内列头，默认单列（含序号 1 起），单列放不下自动双列
  （虚线分隔、隐藏序号、第一列满后第二列从上往下）；双列仍超出则分页
- 页脚：检验备注（提交结果时确认弹窗可选填写，存 reports.content）+ 实线 +
  申请时间/检验时间/报告时间 + 检验者/审核者(留空)/第x页/共x页 + 提示语
- `order_items.registered_at`（schema 迁移 v20）：记录检验登记时间，报告单「检验时间」展示

### 变更
- 检验结果输入框：占位「请输入检验结果」（支持阳性/阴性）；输入框移至项目右侧
- 检验结果失焦自动临时保存（results draft，刷新不丢失），提交后才生成正式报告
- 检验登记按整张申请单统一登记（register_order）
- 修复检验结果提交后打印预览报「网络错误」（print.js 残留旧变量 `preview`）
- 修复候诊列表刷新后 tab 高亮与列表不一致（会话记忆筛选与首请求不一致时重新拉取，
  四个医技工作台共用修复）
- 检验报告单抬头：无第二名称时「医院名称 检验报告单」同行；有第二名称时两组名称
  两端对齐 + 检验报告单，医院名称与检验报告单垂直水平居中
- 检验报告单正文去除多余边框/行线与表头灰色背景，仅保留表格头两条实线
- 修复检验时间未显示：`order_items.registered_at` 记录登记时间并快照到报告
- 临床诊断仅显示首诊断且不含 ICD10 编码
- 检验报告快照固化（reports 新增 apply_dept/apply_doctor/clinical_diag/apply_time/
  reg_time 列，schema 迁移 v21）：生成时定格申请科室/医生/临床诊断/申请时间/检验时间，
  后期调阅不受病历/转科等后续变化影响（旧报告回退实时查询）

---

## [7.10.0] - 2026-09-06

> 护理记录模板系统 + 护士站护理记录交互优化。

### 新增
- 护理记录模板（`nursing_record` 类型）：护士侧边栏新增「护理模板」管理页
  （护士仅护理记录模板；管理员模板管理页可管理全部类型含护理模板）
- 模板审核扩展：护理模板用独立审核类型 `nursing_template`（审核中心识别「护理记录模板」），
  个人免审、科室/全院提交审核，审核期间仅本人可用，被驳回自动降级为个人模板，
  通知发送至护士角色并跳转护士模板页
- 护理记录添加改为模态框：左侧护理模板列表（搜索）+ 右侧自由输入，点击模板直接带入内容

### 变更
- 护理记录支持删除（✕ + 确认），后端新增 `nursing_delete`
- 患者信息横条「✕ 关闭」仅打开护理记录单时显示；关闭后返回空白工作台并自动弹出候诊列表
- 侧边栏无患者时显示占位符（参照病历空态）；打开护理记录单后侧边栏顶部新增「护理记录」锚点
- 修复处置单预览标题（显示「处置单预览」），处方预览仍为「输液（注射）笺」
- 护理记录按时间正序展示（最新显示在下方）
- 修复护士站叫号排队为空：排队悬浮窗数据源改为与候诊列表一致（患者级在办明细），
  并携带当前打开患者作为「当前处理中」
- 排队悬浮窗「当前处理中」始终显示当前打开的患者（含不在在办队列的已完成单），
  「下一位」随当前患者推进（其后首位在办患者）；切换患者立即刷新，不再等待轮询

### 变更（7.10.0 续 · 检验科）
- 检验科工作台抬头参照护理记录单版式：医院名称 + 第二名称 + 检验报告单 + 患者信息两行
- 检验结果按「申请单号」组合展示为独立区块，点击申请单号弹出通用打印预览（检验申请单）；
  右栏大纲按申请单分组
- 修复组合检验（如尿常规）无组内成员输入框：演示数据种子改为幂等并自动链接游离成员；
  界面在组未配置成员时给出明确提示并拦截提交
- 检验结果输入框：占位提示改为「请输入检验结果」（支持阳性/阴性等非数值）；输入框移至
  项目名称右侧（利用右侧空间）
- 检验结果失焦自动临时保存（results draft，刷新不丢失），提交后才生成正式报告
...
> 护士站工作台护理记录单全面优化 + 医技科室候诊列表交互与可见范围重构。

### 新增
- 通用只读打印预览模态框 `Clinic.print.preview(url)`：复用打印模板 HTML（含 A5 分页）
  在模态框内只读展示，仅关闭按钮、禁右键/选择/复制；完整病历 / 处置单 / 处方复用
- 医技角色（护士 / 检验 / 影像 / 药房）可预览 `record` / `order` 打印内容（print_guard 宽松校验）

### 变更
- 护士站护理记录单：抬头显示医院名称 + 第二名称；患者信息两行（急诊病历样式）；
  顶部状态位不再重复流水号；病历摘要去掉「只读」并新增「查看完整病历」预览入口；
  待执行医嘱去掉「护士站执行」文案；处置医嘱单号 / 医嘱处方号可点击预览处置单 / 处方
- 护士站操作后局部刷新：护理记录 / 生命体征 / 处置 / 医嘱各区块单独刷新，
  不再重建整页导致滚动回顶；生命体征改为「录入」按钮唤起悬浮窗输入
- 医技科室候诊列表页签：状态页签（待处置 / 待发药 / 检查中等）互斥单选，
  「当日」可叠加选择；去掉明细摘要列，状态列固定展示候诊 / 就诊中 / 诊毕
- 医技科室候诊可见天数跟随开单医生权限（users.queue_days 2-7）：每条明细按其
  开单医生天数过滤，多医生开单取各自天数并集（最长窗口），四科室统一适用

### 修复
- 修复 SPA 局部刷新下点击患者地址栏被写入 visit_id（地址栏为旧路径导致
  `/nurse/home?visit_id=…`），现地址栏完全保持不变
- 候诊列表日期 / 时间分列紧凑显示、姓名列收窄、找回明细摘要列；状态列改为
  项目状态徽章（待处置 / 完成 / 待发药…），不再显示候诊 / 就诊中
- 候诊列表时间改为「最后一次开具到本科室的处置 / 医嘱时间」（非挂号时间）；
  待处置按该时间正序（最新在下面）、完成按完成时间倒序（最新完成在上面）
- 生命体征录入改为与医生工作站一致的固定悬浮窗（点击处定位，含生理区间校验），
  移除模态框方案
- 护士站候诊列表明细列精准显示执行进度：处置1/2、医嘱2/3（某类全部完成显示
  「处置执行完成 / 医嘱执行完成」）；姓名列固定宽度不再占用弹性空间，剩余空间给明细列
- 护理记录单抬头字段优化：第一行 姓名/性别/年龄/出生日期，第二行 患者ID/流水号/
  首诊科室/首诊时间；抬头宽度与下方记录区块等宽
- 打印预览模态框宽度跟随纸张（A5 约 148mm，fit-content），纸张四周仅留 1-2mm，
  去掉过宽与上下大留白
- 候诊列表日期 / 时间列按内容自适应（minmax 保底，不再裁剪）；状态列加宽容纳徽章
- 生命体征悬浮窗每次全新录入，不再预填最后一次体征数据
- 修复打印预览模态框上方加载圈残留：内容挂载前先清空加载占位
- 科室工作台顶栏（固定栏）新增工具组：叫号按钮（科室排队悬浮窗）+ 工具箱下拉
  （患者查询），参照医生工作站；患者信息横条仅保留候诊按钮 + 关闭
- 患者信息横条新增「就诊状态」徽章（候诊 / 就诊中 / 诊毕）与「总费用」徽章
  （含悬浮明细，参照医生工作站）
- 候诊列表归类改为患者级：只要存在未办结项目即归入「待处置」，全部办结才进
  「完成」，不再同一患者同时出现在两个列表；明细精准描述部分完成
  （如「处置已完成，医嘱0/1」）
- 护士点击处方号只预览「门诊输液（注射）笺」（单号+Z），不再展示给药房的处方笺
- 患者横条右侧按钮改为「✕ 关闭」：点击关闭护理记录单返回空白工作台并自动弹出
  候诊列表，靠右对齐；去掉状态位的「就诊状态：xx」描述（已由徽章承担）
...
### 新增
- 知情同意书开具科室固化：`consents` 表新增 `dept_id` / `dept_name` 列
  （schema 迁移 v19），创建时固化就诊当前科室；打印模板与模态框
  「就诊科室」均显示开具时固化科室，不随转科 / 会诊变化（旧数据回退当前科室）

### 修复
- 修复转科后就诊状态被重置为「待就诊」：患者接诊中（已有病历）转科后仍保持
  「就诊中」，候诊列表状态不再来回跳变
- 修复续写病历打开时无法转科（提示「仅就诊中的患者可转科」）：转科保持
  `visiting` 状态，新科室医生打开续写病历即可直接转科 / 开单，无需先点保存
- 修复跨科室会诊被权限拦截：外科患者请内科会诊，内科医生进入会诊报
  「您无权查看该患者的当前就诊 / 该病历不在您的可查看时间范围内」——
  会诊医生（目标科室）对进行中 / 待处理会诊的就诊放行查看
- 修复医生切换科室后候诊列表不自动弹出（SPA 导航对同 URL 去重导致不刷新）：
  切换成功后立即刷新并弹出新科室候诊队列
- 优化 SPA 局部导航切换页面白屏闪烁：加载遮罩延迟 180ms 显示（快速切换不闪）、
  内容安装完成后再关闭遮罩
...
> 将残留的系统原生 `prompt()` 弹窗全部替换为内置模态框输入对话框，统一交互体验。

### 新增
- 模态框组件 `Clinic.modal.prompt(opts)` 输入对话框：支持单行 / 多行、必填校验（空值
  toast 拦截）、初始值、辅助说明、自动聚焦，替代原生 `prompt()`

### 变更
- 缴费管理页：退费 / 取消挂号原因、退费原因、整单退费原因由原生 `prompt()` 改为内置输入框
- 挂号管理页：退费 / 取消挂号原因由原生 `prompt()` 改为内置输入框
- 检验科 / 影像科：报告撤回原因由原生 `prompt()` 改为内置输入框（必填校验）

---

## [7.8.0] - 2026-09-06

> 护士站 / 检验科 / 影像科 / 药房工作台全面重构：采用与医生工作站一致的
> 「顶部患者信息横条 + 左侧候诊列表 + 主工作区」布局，点击候诊患者弹出
> 类似电子病历的患者工作台页，顶部工具栏含「叫号」科室排队悬浮窗。

### 新增
- 科室工作台共用后端接口 `app/api/deptwork.php`（患者级候诊队列 / 患者工作台
  聚合数据 / 科室排队悬浮窗），四个医技角色共用，避免重复实现
- 科室工作台公共布局骨架 `app/includes/dept_workbench.php`（顶部患者横条 + 候诊按钮 +
  主工作区 + 右栏大纲 + 操作按钮组）与公共前端组件 `deptwork.js`（候诊弹层面板 / 页签
  偏好会话记忆 / 排队悬浮窗 / URL 深链保持），高度解耦，各角色视图仅注入个性化渲染脚本
- 顶栏「叫号」按钮弹出科室排队悬浮窗（当前处理中 / 下一位 / 候诊队列，点击患者直接
  打开其工作台，10 秒轮询，可拖动），与医生叫号悬浮窗同风格
- 药房工作台顶栏保留「📦 库存」入口（入库 / 出库 / 低库存高亮）

### 变更
- 影像科工作台：候诊列表按「检查中 / 完成 / 当日」页签过滤；点击患者弹出所见即所得
  影像报告单页，可直接书写影像所见与影像诊断并提交生成报告
- 检验科工作台：候诊列表按「检验中 / 完成 / 当日」页签过滤；点击患者弹出检验值
  录入列表（含计量单位 / 正常范围 / 危急值提示），支持组合检验成员逐项录入
- 药房工作台：候诊列表按「待发药 / 完成 / 当日」页签过滤；点击患者弹出只读打印样式
  处方单，可直接「通过发药 / 拒绝」（拒绝填写理由，库存自动恢复）
- 护士站工作台：候诊列表按「待处置 / 完成 / 当日」页签过滤；点击患者弹出护理记录页
  （只读展示主诉 / 现病史 / 既往史 / 过敏史 / 查体 / 初步诊断 + 生命体征趋势图与手动录入
  + 待处理处置 / 待执行医嘱，均仅纳入「护士站执行」项目）
- 检查中 / 检验中页签 = 待登记 + 待出报告全流程在办项目；候诊可见天数与医生候诊一致

### 修复
- 护士站候诊队列原先包含非护士站执行的处置，现仅统计 is_nurse=1（护士站执行）项目

---

## [7.7.0] - 2026-09-06

> 全站 AJAX 局部刷新导航（SPA）：侧边栏菜单 / 站内链接 / 病历页切换患者均不再整页重载，
> 地址栏保持不变。

### 新增
- 页面局部渲染模式：`Router::render()` 支持 `_partial=1` 请求只输出视图内容 + 页面标题，
  不套框架布局；独立页（登录/安装/落地页/退出/叫号屏）始终整页加载
- 前端 SPA 导航核心 `nav.js`（`Clinic.nav`）：拦截站内链接点击 → fetch 拉取 partial →
  替换 `.content` 并重执行内联脚本 → 更新标题/导航高亮/滚动复位；会话失效自动回退整页刷新
- 内联脚本安全重执行机制：先注入内容再逐条同步执行，执行期间捕获视图新注册的
  DOMContentLoaded 回调并统一触发，避免重复触发全局旧回调
- 导航离开自动关闭模态窗；EMR 未保存修改统一确认拦截（原 queuepanel 仅 toast）

### 变更
- 侧边栏菜单 / 顶栏入口（模板管理 / 个人信息等）/ 站内消息跳转 / 会诊进入全部改为局部刷新
- 病历页切换患者（候诊列表 / 叫号下一位 / 重呼 / 会诊 / 消息跳转）不再整页重载：
  `Clinic.emr.init()` 重复调用重建病历状态，患者切换即时完成
- EMR 初始化幂等化：eventBus / patient 订阅先 off 再 on，SPA 重复进入不累积订阅；
  大纲轮询定时器全局句柄复用，不累积轮询与 beforeunload 监听

### 修复
- 修复局部导航进入病历页后候诊按钮/患者队列不出现：EMR 组件栈（emr.js/queuepanel/
  doctor_tools/room_heartbeat 等）原本只随完整页首屏注入，partial 局部导航缺脚本；
  现按 partial 响应 `data-needs` 声明按需动态加载缺失组件（按依赖顺序），
  依赖就绪后再安装内容并执行内联脚本
- 医生顶栏叫号/工具箱/科室切换工具改为**仅病历页显示**：SPA 局部导航下顶栏常驻，
  由 nav.js 依据页面顶栏补丁动态注入（进入病历页）与移除（离开病历页）；
  完整页与局部补丁共用同一份工具组 HTML，保证两种入口 DOM 一致
- 修复离开病历患者页后每 30s 弹「就诊记录不存在」toast：EMR 患者页的 30s 大纲
  轮询定时器在 SPA 局部导航离开后残留，以空 visit_id 请求开单接口；
  现已增加 #visitId 校验 + 离开患者页自动清除定时器
- 知情同意书新增「需有已保存首诊病历」约束：无首诊病历→前端隐藏「＋」并拦截，
  后端 save 新建分支硬校验（提示「请先书写并保存首诊病历后再创建知情同意书」）
- 修复右侧会诊栏「＋」在无可编辑病历时仍错误显示且未靠右：loadData 原逻辑
  在非会诊处理时无条件重新显示会诊「＋」，覆盖 syncNavAdds 的可编辑病历隐藏；
  现按「非会诊处理中且存在可编辑病历」双条件显示，隐藏时折叠箭头靠右

### 说明
- 因地址栏保持不变，页面 F5 刷新会回到浏览器地址栏对应入口页（非当前局部页），属预期行为

---

## [7.6.7] - 2026-09-05

> 叫号悬浮窗细节优化：大屏过号患者最新在最底部、绑定后下拉即收起、过号标记右对齐、
> 「解绑大屏」精简为「解绑」、切换患者时悬浮窗闪烁优化。

### 变更

- **大屏待就诊过号患者顺序**：改为正序排列——最早过号的在上、**最新过号的在最底部**。
- **绑定后收起绑定下拉**：医生绑定诊室成功后，顶栏绑定下拉立即收起，不再与叫号悬浮窗重叠。
- **过号标记右对齐**：完整版号源列表中过号患者「（过号·点重呼）」文字右对齐，不再受姓名
  长度影响。
- **解绑大屏 → 解绑**：完整版右下角按钮文案精简。
- **切换患者时悬浮窗闪烁优化**：悬浮窗缓存最近面板数据，切换患者页面重建后先用缓存立即
  渲染（不再出现「加载中」闪烁），再异步刷新；叫号/过号/重呼切换患者在导航前不再做多余
  刷新。页面本身的闪一下来自整页加载新患者病历，属正常导航行为。
  （`app/repositories/QueueRepository.php`、`public/assets/js/components/doctor_tools.js`、
...
- **完整版「再次叫号」→「重呼」**：与 mini 版文案统一。
- **候诊号源列表缩减**：标准模式号源区 max-height 降低至约 5 条可见（更多可滚动查看）。
- **悬浮窗宽度 200→220px**：完整版与 mini 版均改为 220px。
- **mini 版下一位换行**：根因是「下一位」标签（3字×11px=33px）超出 32px 标签宽导致
  「位」换行。修复：标签加宽至 36px + `white-space:nowrap`，值列 `flex:1;min-width:0`
  + nowrap/ellipsis——整行单行显示。
  （`public/assets/js/components/doctor_tools.js`、`public/assets/css/components.css`）

## [7.6.4] - 2026-09-05

> 医生叫号悬浮窗紧凑化：完整版与 mini 版宽度统一为 200px，按钮/文字尺寸同步优化；
> mini 版下一位省略「第」字。

### 变更

- **悬浮窗宽度 200px**：完整版（原 320px）与 mini 版（原 260px）统一改为 200px，并同步
  优化标题、按钮（重呼/过号/下一位）、当前就诊、下一位、号源列表、mini 行的文字与内边距
  尺寸，紧凑不占屏。
- **mini 版下一位简化**：显示为「张三（012号）」（省略「第」字）；完整版保持
  「张三（第012号）」。
  （`public/assets/js/components/doctor_tools.js`、`public/assets/css/components.css`）

## [7.6.3] - 2026-09-05

> 叫号大屏 UI 干净重做：三套布局（纵向/方屏/宽屏）自动切换；等待就诊列表五列按内容自适应
> 共享列宽——各行序号/姓名/性别/年龄全部纵向对齐、姓名后无超长空隙、列间距均匀适中；
> 竖屏下左右两块等宽均分；宽屏医生照片直接填满单元格；真实/预览文字均随画布等比缩放。

### 修复

- **等待就诊姓名后超长空隙 + 性别/年龄不对齐（同源问题一并解决）**：根因是各行是独立
  grid，且旧测量用 `scrollWidth` 在未溢出时返回整列宽（巨大）。修复：用同字号隐藏探针
  行测量五列各自内容最大自然宽度，设为共享列宽变量（`--wait-*-w`）——所有行共用同组列宽，
  序号/姓名/性别/年龄全部纵向对齐，同时列宽贴合内容、姓名后无超长空隙；实测竖屏预览
  列间隙 6/6/6、真实 1080 下 19/19/19、宽屏 8/8/8，均等且随画布缩放。
- **竖屏下方两块不等宽**：左侧「正在就诊+下一位」与右侧「等待就诊」改为等宽均分
  （`1fr:1fr`）。
- **真实浏览器文字未等比缩放**：`.call-body` 改用确定高度 `height:100vh` + `container-type:
  size`（cqmin 基准在 body），真实 1920×1080 下等待姓名 14→32px、就诊名 113px，横/竖/方屏
  全部正确等比缩放；预览模式同样正确。
- **宽屏医生照片限定正方形/卡片拉扁**：移除照片 `aspect-ratio` 与卡片比例限制，照片直接
  填满所在单元格（图片 `object-fit:contain` 保持原比例），卡片随列高正常撑满。
- **方屏双排分隔线**：JS 给第二列条目加 `wait-col2` 类，分隔线在第二列完整显示；
  两列间距、分隔线到内容间距与条目列间距统一（1.8cqmin）。
- **年龄「x岁x月」换行溢出**：年龄/性别列 `white-space:nowrap` + 省略号，单行显示不溢出。
- **过号「过」字徽标**：`box-sizing:border-box` 含边框正好占满预留列宽，不挤压序号。

### 变更

- **三套布局按宽高比自动切换**：纵向（宽<高）、方屏/近方屏（1:1~16:10，上医生+就诊、
  下等待双排）、宽屏/超宽屏（≥16:10，左医生｜中就诊｜右等待，列宽 `1.3fr/1fr/0.7fr`）。
- 过号患者恒居列表末尾并按顺序排列；等待显示数量按可用高度动态计算（末行不裁切）。
  （`public/assets/js/components/screen.js`、`public/assets/css/call.css`、
  `public/screen.php`、`app/api/screen.php`）
  （`public/assets/js/components/screen.js`、`public/assets/css/call.css`）

## [7.6.2] - 2026-09-05

> 叫号大屏改为「纵向 / 方屏(近方屏) / 宽屏(≥16:10)」三套布局自动切换；竖屏等待就诊
> 姓名后空白消除。

### 变更

- **三种屏型布局自动切换**（按画面宽高比）：
  - **纵向**（宽<高）：医生卡在上，主区「正在就诊/下一位 | 等待就诊」在下；
  - **方屏/近方屏**（1:1 ~ 16:10，如 600×600、4:3）：恢复原「上 医生卡+就诊信息、下 等待就诊
    双排 2 列」布局（不再使用三栏）；
  - **宽屏/超宽屏**（≥16:10，如 16:9、2:1）：左中右三等分三栏（医生卡 | 正在就诊/下一位 |
    等待就诊）。
- **竖屏等待就诊姓名后空白消除**：姓名列由 `1fr` 拉伸改为 `fit-content(16cqmin)` 按内容
  自适应，姓名后不再有约 2 个汉字的空白（实测姓名右端→性别左端间隙 28px→4px），各列仍
  纵向对齐。
- 方屏等待就诊同样按可用高度动态显示（过号恒居末尾），双排每行 2 位。
  （`public/assets/js/components/screen.js`、`public/assets/css/call.css`）

## [7.6.1] - 2026-09-05

> 叫号大屏启动提示语优化：分段展示 + 字号随屏幕缩放 + 呼吸动画，小尺寸屏幕不换行溢出。

### 变更

- **启动提示语分段美化**：自动播放解锁遮罩改为「🔊 图标（呼吸动画）+ 点击屏幕/启动叫号
  语音大屏系统（两行分段）+ 点击后自动开始播报叫号」三段式；字号、间距、内边距全部用
  vh/vmin 随屏幕缩放，小尺寸屏幕整句不再一行放不下而溢出。
  （`public/screen.php`）

## [7.6.0] - 2026-09-05

> 叫号大屏等待就诊全面优化：显示数量随画布高度动态计算、过号恒居末尾；列宽/间隙随画布
> 缩放不再挤压姓名；横屏改为「左医生｜中就诊｜右等待」三栏布局。

### 变更

- **等待就诊动态数量**：显示多少位由列表可用高度自动计算——屏幕小放不下就少显示
  （如 5/6/7 位），超长屏幕多显示；不再固定 8/16 位，末行不再被裁切。
- **过号患者恒居末尾**：无论显示数量如何调整，过号患者始终排在列表末尾，多个过号
  按顺序依次排列（末尾留位优先过号患者）。
- **列宽/间隙随画布缩放**：等待列表序号/姓名/性别/年龄列宽与列间隙全部改为 cqmin
  随画布同步缩放，姓名列自适应剩余宽度——竖屏小画布下不再被固定列挤压到看不见。
- **横屏（16:9/方屏）三栏布局**：由「上医生+就诊、下等待双排」改为「左 医生照片+信息 |
  中 正在就诊+下一位 | 右 等待就诊（满高单列）」。
- 服务端等待号源池扩至 51 位（首位为下一位），支持超长屏多显示。
  （`public/screen.php`、`public/assets/js/components/screen.js`、
  `public/assets/css/call.css`）

## [7.5.5] - 2026-09-05

> 修复：mini 叫号悬浮窗点击解绑时，确认弹窗被悬浮窗遮挡。

### 修复

- **解绑确认弹窗被悬浮窗遮盖**：悬浮窗 z-index(2000) 高于模态框遮罩(1000)，解绑确认框
  显示在悬浮窗之下被遮挡。修复：悬浮窗 z-index 降为 800（高于页面下拉、低于模态框），
  解绑/过号等确认弹窗正常显示在上层。
  （`public/assets/css/components.css`）
...
- **解绑按钮符号优化**：mini 版标题「解绑大屏」按钮由汉字「解」替换为纯字符符号「⊘」
  （圆圈斜杠，表断开/解除），与 `+` / `-` / `x` 保持统一字符风格。
  （`public/assets/js/components/doctor_tools.js`）

## [7.5.2] - 2026-09-05

> 叫号悬浮窗（完整版/精简版）右上角按钮统一优化：统一大小、统一字符标志（解 / + / - / x）。

### 变更

- **完整版标题按钮**：`-`（最小化，切换到精简版）+ `x`（关闭）。
- **精简版标题按钮**：`解`（解绑大屏）+ `+`（最大化，恢复完整版）+ `x`（关闭），
  解绑置于三按钮最左侧；所有按钮统一为等宽正方形、相同字号，字符标志一致，不再混用
  emoji 锁。
- 移除精简版原「最小化隐藏」按钮（关闭 `x` 即收起，点顶栏「叫号」重新打开）。
  （`public/assets/js/components/doctor_tools.js`、`public/assets/css/components.css`）

### 文档

- `AGENTS.md` 补充明确约定：**一处修改 = 一次提交**（每个独立改动点单独 commit）。

## [7.5.1] - 2026-09-05

> 大屏预览改为「固定设计尺寸 + 整体等比缩放」：只要比例一致，显示内容就完全一致，
> 所有文字随画布同步缩放、不再溢出。

### 修复

- **预览文字不随画布缩放/溢出**：预览原来按预览尺寸排版，字号 clamp 下限在 9:16 小画面下
  触发，导致院名/时钟/等待列表等文字比例失真、显示不全溢出。修复：预览时页面以「短边
  1080」的固定设计尺寸排版（字号 clamp 下限不触发），再由 JS 整体 `transform: scale`
  等比缩放到预览尺寸——任意预览尺寸下只要比例一致，显示内容就完全一致，所有文字随画布
  同步缩放、不溢出。
- **统一字号基准**：大屏样式单位由 `vmin` 统一为 `cqmin`（真实大屏无容器时回退
  svmin≈vmin，行为不变；预览时统一以 body 容器为基准）。
  （`public/screen.php`、`public/assets/js/components/screen.js`、
  `public/assets/css/call.css`）

## [7.5.0] - 2026-09-05

> 叫号悬浮窗新增「精简版（mini）」模式：可切换完整版/精简版并记忆所选版本，小屏电脑更省空间。

### 新增

- **叫号悬浮窗精简版（mini）**：精简窗口仅显示 4 项内容——标题（可拖动，带恢复完整版 /
  解绑大屏 / 最小化三个按钮，最小化后点顶栏「叫号」按钮重新打开）、当前就诊患者、
  下一位患者、一行三个小按钮（重呼 / 过号 / 下一位），小巧精炼不占屏幕。
- **完整版切换入口**：完整悬浮窗标题栏新增「切换精简模式」按钮（⤢），点击即进入精简版；
  精简版标题栏的「恢复」按钮可切回完整版。
- **版本记忆**：所选模式按医生（uid）保存到本地（localStorage），下次打开悬浮窗（含重新
  登录后）直接启动对应版本。（`public/assets/js/components/doctor_tools.js`、
  `public/assets/css/components.css`）
...
- **预览默认「自由」模式**：打开大屏预览时默认即「自由」尺寸（iframe 填满舞台、不限纵横比），
  需要锁定比例再手动切换。（`app/views/admin/callmanage.php`）

## [7.4.1] - 2026-09-05

> 大屏预览新增「自由」尺寸模式：不限纵横比，iframe 填满预览舞台并随舞台自动变化。

### 新增

- **大屏预览「自由」尺寸**：预览尺寸按钮新增「自由」模式，不限制纵横比，
  iframe 填满整个预览舞台（100% 宽高），随浏览器窗口/舞台尺寸自动变化显示，
  大屏按 iframe 视口自适应排版；与锁定比例模式（9:16 / 1:1 / 16:9）并存。
  （`app/views/admin/callmanage.php`）

## [7.4.0] - 2026-09-05

> 管理端叫号管理：大屏预览支持切换并锁定屏幕尺寸（纵向 9:16 / 方形 1:1 / 横向 16:9）。

### 新增

- **大屏预览尺寸切换**：叫号管理 → 预览大屏时，弹窗顶部提供「纵向 9:16 / 方形 1:1 /
  横向 16:9」三个尺寸按钮，点击即锁定 iframe 为对应比例的屏幕尺寸（在预览区域内等比
  缩放居中显示，窗口缩放自动重新适配），便于按实际摆放屏幕预览叫号大屏效果。
- **预览按预览尺寸排版（防止文字溢出）**：预览时向大屏传入 `pv_w/pv_h`，大屏画布严格
  按预览尺寸锁定排版（宽度/高度/最小高度），避免按浏览器窗口尺寸渲染导致文字溢出；
  真实大屏不受影响。
  （`app/views/admin/callmanage.php`、`public/screen.php`）

## [7.3.2] - 2026-09-05

> 修复：横屏/竖屏等待就诊空态提示「暂无候诊患者」未居中显示。

### 修复

- **等待就诊空态不居中**：上一轮为修等待列表列对齐，将等待面板内容区改为拉伸顶部对齐，
  导致无患者时的空态文字「暂无候诊患者」被顶到左上角。修复：无患者时改为渲染独立的
  居中容器（`.screen-wait-empty`），横屏/竖屏空态文字均整体居中显示；有患者时保持
  原有列对齐布局不变。（`public/assets/js/components/screen.js`、
  `public/assets/css/call.css`）

## [7.3.1] - 2026-09-05

> 横屏叫号大屏两处优化：等待就诊四列等宽均匀分布；医生空态卡分隔虚线居中。

### 变更

- **横屏等待就诊四列等宽均匀分布**：姓名列不再一味拉长，序号/姓名/性别/年龄四列改为
  等宽（`1fr`）均匀分布，观感更像规整表格。（`public/assets/css/call.css`）
- **横屏医生空态卡分隔虚线居中**：未绑定医生时的「医生出诊中」空态卡原为单列纵向布局
  （照片在上、信息在下），导致信息区左边框（分隔虚线）落在卡片最左沿。修复：横屏空态卡
  改用左右两列（照片 | 信息），分隔虚线居中；竖屏空态卡保持原纵向布局不变。
  （`public/assets/css/call.css`）

## [7.3.0] - 2026-09-05

> 叫号大屏竖屏/横屏双排版：横屏（含方屏）自动切换布局，等待就诊双排展示 16 位；修复
> 竖屏列对齐与过号徽标显示。

### 新增

- **横屏/方屏自动切换排版**：宽 ≥ 高时（横屏/方屏）自动切换为另一种布局——上半部分
  左侧医生照片+信息、右侧「正在就诊 + 下一位」（医生卡与右侧等高度），下半部分等待
  就诊双排（2 列，每列 8 位，共 16 位）；两列之间留间距并加纵向虚线分隔线，避免患者
  看错行。竖屏保持原排版，等待就诊显示 8 位。（`public/assets/js/components/screen.js`、
  `public/assets/css/call.css`、`app/api/screen.php`：号源池扩至 17 位供展示）

### 修复

- **竖屏等待就诊列不对齐**：等待就诊条目原在居中容器中按内容宽度居中导致各行错位。
  修复：等待就诊面板内条目拉满整行宽，序号/姓名/性别/年龄严格纵向对齐；
  姓名列 `minmax(0,1fr)` 自适应剩余宽度，竖屏窄屏自动收缩不溢出框外。
  （`public/assets/css/call.css`）
- **过号徽标不显示**：竖屏/横屏切片把队列末尾的过号患者裁掉。修复：等待列表改为
  「普通患者优先 + 过号患者恒居末尾」，过号患者始终显示在列表末尾（竖屏第 8 位 /
  横屏第 16 位）。（`public/assets/js/components/screen.js`）

## [7.2.1] - 2026-09-05

> 叫号管理与大屏细节优化：列表去除 Token 列；等待就诊区去掉表头、仅过号患者显示圆形「过」徽标。

### 变更

- **叫号管理列表去除 Token 列**：复制链接已自动携带 token，不再单独在列表展示（列表更简洁，
  `data-token` 属性保留供预览/复制链接使用）。（`app/api/parts/admin_call.php`）
- **等待就诊区去掉表头**：不再显示「号/姓名/性别/年龄」表头行，仅保留按列对齐的列表。
  （`public/assets/js/components/screen.js`）
- **仅过号患者显示圆形「过」徽标**：非过号患者左侧不再显示空圆圈占位（占位改为透明，
  保持列对齐的同时不显示多余圆圈）。（`public/assets/js/components/screen.js`、
  `public/assets/css/call.css`）
...
- **room_stats 接口「未知操作」**：`/api/admin` 动作白名单未收录 6.9.0 新增的 `room_stats`，
  导致叫号管理页新建/删除大屏后刷新科室统计失败（模态框统计无法实时更新）。
  修复：补入 `app/api/admin.php` 白名单。

## [7.1.0] - 2026-09-05

> 医生端可拖动叫号悬浮窗 + 病历联动（重大）：绑定诊室后，叫号按钮变为叫号操作台，
> 与叫号大屏实时联动，病历系统自动跟随叫号切换患者。

### 新增

- **医生端可拖动叫号悬浮窗**：绑定诊室后点击顶栏「叫号」按钮，弹出独立悬浮窗（替代原绑定
  下拉）。悬浮窗显示：诊室名称、当前就诊、下一位、再次叫号、过号、下一位、候诊号源池、
  解绑大屏；按住标题栏可在页面上任意拖动，位置与开合状态记忆在本登录会话，跨患者切换
  保持打开。未绑定诊室时保持原「下拉选择大屏」逻辑。
  （`public/assets/js/components/doctor_tools.js`、`public/assets/css/components.css`）
- **病历联动（叫号为主驱动、病历跟随）**：
  - 点「下一位」→ 自动认领号源池首位 → 自动跳转该患者病历页（等同手动点击候诊列表该患者）；
  - 点「过号」→ 当前患者标记过号 → 自动呼叫下一位并跳转其病历；
  - 当前病历有未保存修改（脏数据）时拦截并提示先保存（复用打印/开单同款脏数据校验）；
  - 手动在候诊列表点击患者不干预叫号系统运作。
- **过号标记**：过号患者不再进入任何医生的号源队列，叫号大屏候诊列表末尾以红色（过号）
  徽标显示，患者一目了然；再次叫号触发大屏重复语音播报当前患者（防止患者没听到）。
- **医技大屏（检验/影像/药房/护士站）保持原排队看板逻辑**；医生诊室大屏遵循
  「绑定医生才显示、数据回库校验」。

### 变更

- 医生顶栏「叫号」按钮行为：已绑定诊室 → 打开/收起叫号悬浮窗；未绑定 → 下拉选择大屏。

...
### 修复
- **审核中心/管理页预览已删除模板或套餐时闪退**：此前  收到「不存在」错误响应后 900ms 自动关闭模态框，导致「显示不存在然后消失」；现 /（及护士/影像模板预览）改为  手动加载——模板/套餐不存在时 toast 明确提示且不弹模态框，存在时正常打开只读预览；管理员审核预览跳转前先校验存在性。

### 文档
- **同步版本号至 v8.17.9**（README 徽章 +  + ）。

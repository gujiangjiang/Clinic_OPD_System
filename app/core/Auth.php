...
class Auth {
    ...
    public static function login($account, $password) {
        ...
        $u = DB::one('SELECT * FROM users WHERE username=? AND status=1', array($account));
        if (!$u) {
            $u = DB::one('SELECT * FROM users WHERE emp_no=? AND status=1', array($account));
        }
        if (!$u) {
            return '用户名/工号或密码错误';
        }
        ...
        // 登录成功：重置失败计数
        DB::exec('UPDATE users SET login_fail_count=0, login_locked_until=NULL, last_login=? WHERE id=?', array(now_str(), (int)$u['id']));
        // 防会话固定：登录后重置会话 ID

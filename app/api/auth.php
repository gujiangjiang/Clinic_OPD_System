...
if ($__act === 'login' || $__act === 'logout_page') {
    ...
    if ($__act === 'login') {
        ...
        if ($u['role'] === 'admin' && (int)UserRepository::val('SELECT pwd_changed FROM users WHERE id=?', array($u['id'])) === 0) {
            $exist = UserRepository::one("SELECT id FROM messages WHERE to_user_id=? AND title=? LIMIT 1",
                array((int)$u['id'], '修改管理员密码提醒'));
            if (!$exist) {
                UserRepository::insert("INSERT INTO messages(from_name, to_role, to_user_id, title, content, is_read, msg_type, link_url, created_at) VALUES(?,?,?,?,?,0,'system',?,?)",
...
switch ($action) {
    ...
    case 'forgot':
        $me = Auth::user();
        $row = UserRepository::one('SELECT id, username, name, emp_no, role FROM users WHERE id=?', array($me['id']));
        if (!$row) json_fail('用户不存在');
        // 防重复申请：已有待审核的密码重置申请时不再重复提交
        $pending = UserRepository::one("SELECT id FROM audits WHERE type='pwd_reset' AND ref_id=? AND status='pending'", array($row['id']));
        if ($pending) json_fail('您已提交过密码重置申请，请耐心等待管理员审核');
        // 已通过审核但尚未设置新密码时，引导直接设置
        $approved = UserRepository::one("SELECT id FROM audits WHERE type='pwd_reset' AND ref_id=? AND status='approved'", array($row['id']));
        if ($approved) json_fail('您的密码重置申请已通过审核，请直接在站内消息中点击【设置新密码】完成重置');
        UserRepository::insert("INSERT INTO audits(type, ref_id, title, content, status, proposer, proposer_id, created_at) VALUES(?,?,?,?,?,?,?,?)", array(
    ...
    case 'reset_password':
        ...
        $appr = UserRepository::one("SELECT id FROM audits WHERE type='pwd_reset' AND ref_id=? AND status='approved' ORDER BY id DESC", array(Auth::id()));
    ...
    case 'theme':
        ...
        UserRepository::exec('UPDATE users SET theme=? WHERE id=?', array($theme, Auth::id()));
    ...
    case 'sidebar':
        ...
        UserRepository::exec('UPDATE users SET sidebar=? WHERE id=?', array($sidebar, Auth::id()));
    ...
    case 'print_auto':
        ...
        UserRepository::exec('UPDATE users SET print_auto=? WHERE id=?', array($value, Auth::id()));
    ...
    case 'password':
        ...
        $u = UserRepository::one('SELECT * FROM users WHERE id=?', array(Auth::id()));
    ...
    case 'profile_submit':
        $me = Auth::user();
        // 防重复：已有待审核的个人资料申请
        $pending = UserRepository::one("SELECT id FROM audits WHERE type='profile_update' AND ref_id=? AND status='pending'", array($me['id']));
        if ($pending) {
        ...
        $meRow = UserRepository::one('SELECT emp_no FROM users WHERE id=?', array($me['id']));

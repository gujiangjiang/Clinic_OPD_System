...
switch ($action) {
    ...
    case 'home_stats':
        $today = date('Y-m-d');
        $todayItems = (int)DB::val('order', "SELECT COUNT(*) FROM order_items WHERE item_type='lab' AND date(created_at)=?", array($today));
        $todayFee = (float)DB::val('order', "SELECT COALESCE(SUM(total_amount),0) FROM orders WHERE order_type='lab' AND status NOT IN ('refunded','cancelled') AND paid_at IS NOT NULL AND date(paid_at)=?", array($today));
        $pendingReg = (int)DB::val('order', "SELECT COUNT(*) FROM order_items WHERE item_type='lab' AND status='paid'", array());
        $pendingRep = (int)DB::val('order', "SELECT COUNT(*) FROM order_items WHERE item_type='lab' AND status='registered'", array());
        $itemTotal = (int)DB::val('lab', "SELECT COUNT(*) FROM lab_items WHERE status='approved'", array());
        $pendingAudit = (int)DB::val('lab', "SELECT COUNT(*) FROM lab_items WHERE status='pending'", array());
        $labels = array(); $series = array();
        for ($i = 6; $i >= 0; $i--) {
            $day = date('Y-m-d', strtotime("-$i days"));
            $labels[] = substr($day, 5);
            $series[] = (int)DB::val('order', "SELECT COUNT(*) FROM order_items WHERE item_type='lab' AND date(created_at)=?", array($day));
        }
        json_ok(array(

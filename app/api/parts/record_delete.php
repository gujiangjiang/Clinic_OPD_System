...
function record_part_delete($action) {
    ...
    if ($action === 'delete_record') {
        ...
        $rec = DB::one('SELECT * FROM patient_records WHERE id=?', array($recordId));
        ...
        if ($rec['record_type'] === 'initial') {
            $hasProgress = (int)DB::val("SELECT COUNT(*) FROM patient_records WHERE visit_id=? AND record_type='progress' AND status<>'draft'", array($visitId));
            if ($hasProgress > 0) {
        ...
        if ((int)$rec['consultation_id'] > 0) {
            $delConsStatus = DB::one('SELECT status FROM consultations WHERE id=?', array((int)$rec['consultation_id']));
        ...
        if ($recConsultId > 0) {
            $cons = DB::one('SELECT * FROM consultations WHERE id=?', array($recConsultId));
            if ($cons && $cons['status'] === 'doing') {
                DB::exec("UPDATE consultations SET status='pending', accepted_by='', accepted_at='' WHERE id=?", array($recConsultId));
            }
        }
        // 3. 删除（物理删除 + 镜像清理）
        ...
        try {
            ...
            $mirrorOld = DB::one('SELECT id FROM records WHERE visit_id=? AND doctor_id=? AND patient_record_id=0 ORDER BY id DESC LIMIT 1', array($visitId, $u['id']));
            if ($mirrorOld) {
        ...
        if ($row['visit']['status'] === 'visiting') {
            $remainCurDept = (int)DB::val('SELECT COUNT(*) FROM patient_records WHERE visit_id=? AND dept_id=?',
                array($visitId, (int)$row['visit']['current_dept_id']));
            if ($remainCurDept === 0) {
                DB::exec('UPDATE registrations SET status=? WHERE id=?', array('paid', $visitId));
            }
        }
        $newStatus = (string)DB::val('SELECT status FROM registrations WHERE id=?', array($visitId));

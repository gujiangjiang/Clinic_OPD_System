...
function record_part_read($action) {
    ...
    if ($action === 'get') {
        ...
        if ($docDept <= 0) {
            $docDeptRow = DB::one('user', 'SELECT current_dept_id FROM users WHERE id=?', array((int)$u['id']));
        ...
        json_ok(array(
            ...
            }, DB::q('consultation', 'SELECT id, from_doctor_id, from_dept_name, target_dept_id, target_dept_name, status, record_id, accepted_by FROM consultations WHERE visit_id=? ORDER BY id ASC', array($visitId))),

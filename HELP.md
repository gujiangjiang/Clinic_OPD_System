...
- **多数据库备份**：配置备份库（SQLite / MySQL / PostgreSQL），支持立即备份、定时自动备份（如每晚 2 点，页面调度后台执行、当日去重）、以及双向实时同步（RAID1 式双写——每次写入实时镜像到备份库，仅同驱动可靠，失败自动降级不影响主库体验）。备份与双向为两个独立功能：备份=手动/定时全量同步；双向=写入实时镜像。
